...
function buildTeacher(summary, byReason, byEntryQuality, byPersistence, bySpread) {
  ...
  if (n(summary.winRate) < 45) {
    lessons.push({ type: "improve", text: "Winrate is te laag. Maak entries strenger en laat alleen sterkere setups door." });
  }
  if (timeout && n(timeout.avgPnlPct) < -1) {
    lessons.push({ type: "improve", text: "Timeout verliest gemiddeld te veel. Verkort timeout of verhoog minimale quality voor entry." });
  }
  if (stopLoss && stopLoss.count >= 3 && stopLoss.winRate === 0) {
    lessons.push({ type: "improve", text: "Stop-loss cluster aanwezig. Spread, OB score en entry confidence moeten strenger." });
  }
  if (thesisBreak && n(thesisBreak.totalPnlPct) > 0) {
    lessons.push({ type: "good", text: "Thesis-break exit werkt positief. Deze exit bewaart gemiddeld winst beter dan timeout." });
  }

  const bestEq = [...byEntryQuality].sort((a, b) => b.avgPnlPct - a.avgPnlPct)[0];
  if (bestEq) lessons.push({ type: "focus", text: `Beste entry-quality bucket nu: ${bestEq.key}.` });
  const bestPs = [...byPersistence].sort((a, b) => b.avgPnlPct - a.avgPnlPct)[0];
  if (bestPs) lessons.push({ type: "focus", text: `Beste persistence bucket nu: ${bestPs.key}.` });
  const bestSpread = [...bySpread].sort((a, b) => b.avgPnlPct - a.avgPnlPct)[0];
  if (bestSpread) lessons.push({ type: "focus", text: `Beste spread bucket nu: ${bestSpread.key}.` });
  ...
  if (worstEq && worstEq.key !== bestEq?.key) {
    lessons.push({ type: "warn", text: `Slechtste entry-quality bucket nu: ${worstEq.key}. Deze bucket moet strenger of uitgesloten worden.` });
  }
  if (worstPs && worstPs.key !== bestPs?.key) {
    lessons.push({ type: "warn", text: `Slechtste persistence bucket nu: ${worstPs.key}. Hier lekt waarschijnlijk kwaliteit weg.` });
  }
  if (worstSpread && worstSpread.key !== bestSpread?.key) {
    lessons.push({ type: "warn", text: `Slechtste spread bucket nu: ${worstSpread.key}. Deze spread-range kost waarschijnlijk rendement.` });
  }

  const score = Math.max(1, Math.min(10, 5 + (n(summary.winRate) - 40) / 10 + n(summary.avgPnlPct) / 2));
  return { score: pct(score), lessons };
}

function enrichClosedTrades(opened, closed) {
  const openById = new Map();
  for (const ev of safeArray(opened)) if (ev?.id) openById.set(ev.id, ev);
...
function analyzeGroup(rows, liveConfig) {
  ...
  const teacher = buildTeacher(summary, byReason, byEntryQuality, byPersistence, bySpread);
  return { summary, buckets: { byReason, byStage, byEntryQuality, byPersistence, bySpread, byObScore }, teacher, liveConfig };
}
  ...
  try {
    ...
    if (type === "main") {
      return {
        ...
        limits: { ENTRY_LIMIT: cfg?.ENTRY_LIMIT ?? null, ALMOST_LIMIT: cfg?.ALMOST_LIMIT ?? null, BUILDUP_LIMIT: cfg?.BUILDUP_LIMIT ?? null, RADAR_LIMIT: cfg?.RADAR_LIMIT ?? null, CG_TOP: cfg?.CG_TOP ?? null },
      };
    }
    if (type === "moon") {
      return {
...
export default async function handler(req, res) {
  try {
    ...
    const tradeFunnel = rows; // Alle trades
    res.status(200).json({
      ok: true,
      groups: {
        ...
        trade_funnel: analyzeGroup(tradeFunnel, { main_bull: liveConfig.main_bull, main_bear: liveConfig.main_bear, moon_bull: liveConfig.moon_bull, moon_bear: liveConfig.moon_bear }),
      },
      ts: Date.now(),
    });
  } catch (err) {
