...
async function buildUniverse(mode, whaleFlow, btc) {
  ...
  for (const coin of filtered) {
    ...
    if (tradeCandidate === true && isEliteStageForDesk && execution.score >= 62) {
    ...
    for (const coin of universe) {
      ...
      if (rawStage === "RADAR") {
        ...
        if (coin.tradeDeskStatus === "WATCH") {
          ...
          // kleine dip mag 1 scan overleven
...
export function buildMainExecutionDecision({ coin, btc, regime, mode, coinProfile }) {
  ...
  const qualityOk = entryQuality >= 64; // gewijzigd
  checklist.push({
    name: "Entry quality",
    ok: qualityOk,
  ...
  const persistenceOk = persistence >= 56; // gewijzigd
  checklist.push({
    name: "Persistence",
    ok: persistenceOk,
...
export function buildMoonExecutionDecision({ coin, btc, regime, mode, coinProfile }) {
  ...
  const entryQuality = n(coin.entryQuality, 0);
  const qualityOk = entryQuality >= 60; // gewijzigd
  checklist.push({
    name: "Entry quality",
    ok: qualityOk,
    value: entryQuality.toFixed(1),
  ...
  const persistence = n(coin.persistenceScore, 0);
  const persistenceOk = persistence >= 50; // gewijzigd
  checklist.push({
    name: "Persistence",
    ok: persistenceOk,
    value: persistence.toFixed(1),
