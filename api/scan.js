// api/main/scan.js (of jouw scan bestand) — VOLLEDIG aangepast met:
// ✅ buildAdaptiveThresholds()
// ✅ HEADWIND hard filter (marketScore < adaptive.market + 4 => no trade)
// ✅ computeMoonRisk krijgt performance mee (voor drawdown SL/TP logic)
// ✅ ensureFreshPerformance fix (geen keyMoonPositions referentie meer)
...
// ✅ NEW: adaptive engine thresholds
...
// ======================================================
// Force performance refresh als ouder dan 6h (FIXED)
// ======================================================
async function ensureFreshPerformance(prefix, mode) {
  ...
  if (!perf || now - (perf.updatedAt || 0) > MAX_AGE) {
    // ✅ Main-only: altijd keyMainPositions
    const positionsKey = keyMainPositions(mode);
...
// ======================================================
// tradePlan, funnel, portfolio, stage logic, etc.
// ======================================================
function buildTradePlan({ price, mode, confidence, range24, depthOk, tier, regime, persistenceScore, performance }) {
  const risk = computeMoonRisk({
    ...
    regime,
    persistenceScore,
    performance, // ✅ NEW: voor drawdown-based SL/TP
  });
  if (!risk) return null;
  return {
...
async function buildUniverse(mode, whaleFlow, btc, performance) {
  ...
  for (const coin of filtered) {
    ...
    // ✅ NEW: base gate via adaptive thresholds
    let tradeCandidate =
    ...
    // ✅ NEW: HEADWIND hard filter (tegen BTC trend)
    if (regime === "HEADWIND" && marketScore < adaptive.market + 4) {
    ...
    // Desk status logic blijft zoals jij had
    ...
    const nearEntryWatch =
      ...
      (isEliteStageForDesk || stage === "ALMOST");
    ...
    if (tradeCandidate === true && (isEliteStageForDesk || (stage === "ALMOST" && stableWatchReady))) {
  ...
  return {
    regime,
    coins: out,
    adaptive, // ✅ now returns {timing, quality, market}
  };
}
  ...
  try {
    ...
    for (const coin of universe) {
      ...
      if (!hasOpenPosition) {
        ...
        // ✅ entryReady nu ook “minstens adaptive thresholds”
        ...
        const marketNeed = Math.max(n(er.marketScore, 0), n(adaptive.market, 0) + (regime === "HEADWIND" ? 4 : 0));
        ...
        entryReady =
          ...
          (coin.qualityScore || 0) >= qualityNeed &&
          (coin.timingScore || 0) >= timingNeed &&
          (coin.liquidityScore || 0) >= er.liquidityScore &&
          (coin.marketScore || 0) >= marketNeed;
      }
    ...
    for (const candidate of toOpen) {
      ...
      if (coin.stage === "ELITE_IGNITION" || coin.stage === "ELITE_EXPANSION" || coin.stage === "ELITE_CASCADE") {
        await safeSendSignal({
          source: "main",
          stage: coin.stage,
          mode,
    ...
    const adaptiveMeta = {
      performance,
      positionSizeUsd: positionSize,
      adaptiveThresholds: adaptive, // ✅ now {timing,quality,market}
