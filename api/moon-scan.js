...
export default async function handler(req, res) {
  try {
    ...
    // Universe Bitget spot USDT symbols
    const symbolsSet = await getBitgetSpotUsdtSymbols();

    // CoinGecko top 1000 (cached)
    ...
    // cleanup: coins die niet meer in snapshot zitten
    const liveSyms = new Set(rawCoins.map((c) => c.symbol));
    for (const sym of Object.keys(state)) {
      if (!liveSyms.has(sym)) delete state[sym];
    }

    // 1) Eerst: alles dat door RADAR komt verzamelen
    const radarCoins = [];
    for (const c of rawCoins) {
      if (passRadarMoon(c, mode)) radarCoins.push(c);
      else delete state[c.symbol]; // valt uit radar -> weg uit state
    }

    // 2) BUILDUP selectie met rotatie:
    //    - hou een deel uit bestaande state
    //    - vul rest met nieuwe random uit radarCoins
    const stateSyms = Object.keys(state);
    const keepCount = Math.floor(MOON.RADAR_LIMIT * MOON.buildupKeepFromState);
    const keepSyms = shuffle(stateSyms).slice(0, keepCount);

    const newPool = radarCoins
      .map((c) => c.symbol)
      .filter((s) => !keepSyms.includes(s));

    const needNew = MOON.RADAR_LIMIT - keepSyms.length;
    const pickNewSyms = shuffle(newPool).slice(0, Math.max(0, needNew));

    const scanSyms = new Set([...keepSyms, ...pickNewSyms]);

    const buildup = [];
    const almost = [];
    const elite = [];

    for (const c of radarCoins) {
      const sym = c.symbol;
      if (!scanSyms.has(sym)) continue; // 🔥 hier gebeurt de rotatie
      ...
      if (desiredRank > prevRank) {
        if (stageScans >= MOON.minScansPerStage) nextStage = nextStageUp(curStage);
      ...
      const item = {
        ...
        why: { almost: almostGate.why, elite: eliteGate.why },
      };
    ...
    elite.sort(sortKey);
    almost.sort(sortKey);
    buildup.sort((a, b) => (Number(b.stageAt||0) - Number(a.stageAt||0)) || sortKey(a,b));
    ...
    const result = {
      ...
      mode,
      btc,
      counts: { elite: elite.length, almost: almost.length, buildup: buildup.length },
      funnel: { elite, almost, buildup: buildup.slice(0, MOON.RADAR_LIMIT) },
      debug: {
        radarPool: radarCoins.length,
        scannedThisRun: scanSyms.size,
      },
    };
