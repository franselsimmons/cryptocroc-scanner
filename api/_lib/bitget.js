...
const PRODUCTS_URL = "https://api.bitget.com/api/spot/v1/public/products";

export async function loadBitgetUsdtSpotMap(redis) {
  const key = "bitget:symbolmap:spot_usdt:v1";
  ...
  const j = await fetchJson(PRODUCTS_URL, 4);
  const list = j?.data || [];
  ...
  for (const p of list) {
    ...
    const sname = (p?.symbolName || p?.symbol || "").toString();
    if (!base || quote !== "USDT") continue;

    // Bitget spot v1 gebruikt vaak _spbl suffix in symbolName (zoals BTCUSDT_SPBL)
...
export async function fetchBitgetOrderbookSpot(symbolName, limit = 20) {
