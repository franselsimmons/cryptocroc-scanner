...
function isTemporalActivationRequest(req, body = {}) {
    const action = String(body.action || body.type || req?.query?.action || '').trim().toUpperCase();
    if (['ACTIVATE_TEMPORAL_GENERATION', 'TEMPORAL_ACTIVATE', 'ACTIVATE_TEMPORAL'].includes(action)) return true;
    if (body.temporal === true || String(req?.query?.temporal || '') === '1') return true;
...
async function activateTemporalGeneration({ redis, requestedGenerationId = null }) {
    ...
    return {
        ok: true,
        skipped: false,
        type: 'LONG_TEMPORAL_GENERATION_ACTIVATED',
        generationId,
        expectedOldGenerationId,
...
async function handlePost(req, res) {
  ...
  if (isTemporalActivationRequest(req, body)) {
    const requestedGenerationId = body.generationId || req?.query?.generationId || null;
    ...
    return res.status(ok ? 200 : 409).json({
      ...
      source: 'LONG_TEMPORAL_ROOT_GENERATION_ACTIVATION',
      ...flags(),
      ...temporalResult,
      longKeys: {
        namespace: LONG_NAMESPACE,
