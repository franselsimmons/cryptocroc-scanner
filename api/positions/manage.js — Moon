...
function lockKey(mode) {
  return `moon:positions:lock:${String(mode || "bull").toLowerCase()}`;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await kv.set(keyMoonLatest(mode), latestNext, { ex: 60 * 60 });
    ...
    console.error("Moon position manager error:", err);
