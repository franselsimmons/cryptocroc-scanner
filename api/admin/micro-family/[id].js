...
function inferTradeSide(input = {}) {
    if (typeof input === 'string') {
         ...
         if (direct === TARGET_TRADE_SIDE || direct === OPPOSITE_TRADE_SIDE) return
direct;
    ...
    if (microText.includes('MICRO_LONG_')) return TARGET_TRADE_SIDE;
    if (microText.includes('MICRO_SHORT_')) return OPPOSITE_TRADE_SIDE;
...
function isExplicitShort(input = {}) {
    ...
    if (typeof input === 'string') {
         ...
         if (isScannerFingerprintId(value)) return false;
         if (isExecutionFingerprintId(value)) return false;
         if (parseLongTaxonomyMicroId(value).valid) return false;
         return hasShortSignal(value) || upper(value).startsWith('MICRO_SHORT_');
    }
    if (input.shortOnly === true || input.longDisabled === true) return true;
...
function normalizeTaxonomyIdentity(row = {}, fallbackId = '') {
    ...
    const childId = ids.find(isFixedLongChildMicroId);
    const parentId = ids.find(isFixedLongParentMicroId);
    const anyLongId = ids.find((id) => parseLongTaxonomyMicroId(id).valid);
    const parsed = parseLongTaxonomyMicroId(childId || parentId || anyLongId ||
'');
...
function isTargetSide(row = {}) {
    ...
    if (isExplicitShort(row)) return false;
    const identity = normalizeTaxonomyIdentity(row, id);
    if (identity.trueMicroFamilyId &&
parseLongTaxonomyMicroId(identity.trueMicroFamilyId).valid) {
         return true;
    }
...
const slHit =
    ...
    return {
         ...
         denominator,
         validGeometry,
         longTpHit: Boolean(tpHit),
         longSlHit: Boolean(slHit),
         longGrossR,
         longCurrentR,
...
function getLongCurrentFit(row = {}) {
    ...
    const biasText = collectSideText({
    ...
    const bullish = hasLongSignal(biasText);
    const bearish = hasShortSignal(biasText);
    ...
    return outcomes.reduce(
      (acc, outcome) => {
          const source = upper(outcome.source || outcome.outcomeSource || 'VIRTUAL');
              ...
              if (outcome && typeof outcome === 'object' && !isTargetSide({ ...row,
...outcome })) return acc;
...
function getVirtualCompleted(row = {}) {
    ...
    return Math.max(
         num(row.virtualCompleted, 0),
         num(row.virtualWins, 0) + num(row.virtualLosses, 0) + num(row.virtualFlats,
0),
         0
    );
}
...
function getLearningProfitFactor(row = {}) {
    if (hasValue(row.longNetProfitFactor)) return num(row.longNetProfitFactor, 0);
    if (hasValue(row.netLongProfitFactor)) return num(row.netLongProfitFactor, 0);
    if (hasValue(row.netProfitFactor)) return num(row.netProfitFactor, 0);
    if (hasValue(row.profitFactor) && aggregateBucketsAreLearningSafe(row)) return
...
function getLearningCountMetric(row = {}, aggregateCountKey, virtualCountKey =
null, shadowCountKey = null) {
...
function getLearningPctMetric(row = {}, aggregatePctKey, aggregateCountKey,
virtualCountKey = null, shadowCountKey = null) {
...
function getDashboardBalancedScore(row = {}) {
    ...
    if (winrateMeta.outcomeSample <= 0 && winrateMeta.observationSample > 0) {
         ...
         const reliabilityComponent = sampleReliability(winrateMeta.observationSample)
* 18;
         ...
         const currentFitBonus = Math.max(0, getLongCurrentFit(row)) * 2;
         return Math.max(
              1,
...
const currentFit = Math.max(0, getLongCurrentFit(row));
...
function getLearningTier(row = {}) {
    ...
    const score = num(row.dashboardBalancedScore ?? getDashboardBalancedScore(row),
0);
    ...
    if (gate.status === 'PASSED' && score > 0 && (avgR > 0 || totalR > 0)) return
'HARD';
...
row.currentR, 4),
      ...
      currentFit: round(longCurrentFit, 4),
      longCurrentFit: round(longCurrentFit, 4),
      bullCurrentFit: round(longCurrentFit, 4),
      bearishCurrentFit: round(-longCurrentFit, 4),
...
row.trueMicroFamilyId || row.microFamilyId,
     ...
     winsLossesFlatsSource: 'netR',
     winrateDefinition: 'netR > 0',
     validLongGeometry: Boolean(row.validLongGeometry),
     longTpHit: Boolean(row.longTpHit),
     longSlHit: Boolean(row.longSlHit),
     longGrossR: row.longGrossR,
     longCurrentR: row.longCurrentR,
     currentR: row.currentR,
     currentFit: row.currentFit,
     longCurrentFit: row.longCurrentFit,
...
0), 0);
    ...
    const totalCostR = targetRows.reduce((sum, row) => sum + num(row.totalCostR, 0),
0);
    const seen = targetRows.reduce((sum, row) => sum + num(row.seen, 0), 0);
    const observations = targetRows.reduce((sum, row) => sum + num(row.observations,
0), 0);
...
function findRawRow(micros = {}, id) {
    ...
    const candidates = uniqueStrings([id, decodedId])
         ...
         .filter((candidateId) => !idLooksShort(candidateId));
...
function buildActiveLongRows(activeRotation, activeSet, activeParentSet) {
...
function extractActiveParentIds(activeRotation) {
    ...
    return uniqueStrings(ids)
         .filter(validLearningId)
         .filter((id) => isFixedLongParentMicroId(id) || idLooksLong(id));
}
...
export default async function handler(req, res) {
    ...
    res.setHeader('X-Admin-Micro-Family-Mode', 'long-only-75-child-true-micro-detail-temporal-v2');
...
PERSISTENT_LEARNING_KEY
     ...
     const relatedLimit = toSafeLimit(firstQueryValue(req.query?.relatedLimit,
100), 100);
     if (!id) {
     ...
     if (!validLearningId(id) || isExplicitShort(id)) {
   ...
   const requestedTaxonomy = parseLongTaxonomyMicroId(id);
   if (!requestedTaxonomy.valid && !idLooksLong(id)) {
        return res.status(404).json({
          ok: false,
          reason: 'NOT_A_LONG_TRUE_MICRO_FAMILY_ID',
          id,
          weekKey,
          currentWeekKey,
          previousWeekKey,
   ...
   const activeRows = buildActiveLongRows(activeRotation, activeSet,
activeParentSet);
...
|| id;
       ...
       return res.status(404).json({
           ...
           reason: requestedTaxonomy.isParent
             ? 'LONG_PARENT_TRUE_MICRO_FAMILY_HAS_NO_CHILD_ROWS_YET'
             : 'LONG_75_CHILD_TRUE_MICRO_FAMILY_NOT_FOUND',
           ...commonResponse,
           availableCount: allRows.length,
           rawAvailableCount: Object.keys(micros || {}).length,
           activeRotationId: activeRotation?.rotationId || null
