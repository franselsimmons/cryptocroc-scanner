...
function emptyText(){
  return "<p style='color:#94a3b8'>Geen bear runner-kandidaten</p>";
}
...
async function load(){
  try{
    ...
    const candidatesBear = num(data?.candidatesBear);

    el("statusLine").innerText =
      `BTC: ${btcState}${btc24} | Regime: ${regime} | Bear runner candidates: ${candidatesBear} | TS input: ${inputCount}`;

    const f = data?.funnel?.bear || {
      entry: [],
      almost: [],
      buildup: [],
      radar: []
    ...
    console.error("Bear runner fetch error:", e);

    if(el("statusLine")){
      el("statusLine").innerText = "Bear runner data kon niet geladen worden";
    }
  }
}
