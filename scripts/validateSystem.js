...
const REQUIRED_TEMPORAL_RAW_ACCUMULATORS = Object.freeze([
  'observations',
  'completed',
  'wins',
  'losses',
...
const REQUIRED_GENERATION_MANIFEST_FIELDS = Object.freeze([
  'generationId',
  'side',
  'generationCutoffTs',
...
function exportedNames(source) {
    ...
    const direct =
        /export\s+(?:async\s+)?(?:function|const|let|var|class)\s+([A-Za-z_$]
[\w$]*)/g;
...
function hasWeekendUtcLogic(source) {
    ...
    const commonDirect =
         /(?:day|dayUtc|utcDay)\s*===\s*0[\s\S]{0,160}
(?:day|dayUtc|utcDay)\s*===\s*6/.test(source) ||
         /(?:day|dayUtc|utcDay)\s*===\s*6[\s\S]{0,160}
(?:day|dayUtc|utcDay)\s*===\s*0/.test(source);
    const commonArray =
...
function forbiddenLongRiskPatterns() {
    return [
         ...
         },
         {
              ...
              regex: /\(\s*entry\s*-\s*exitPrice\s*\)\s*\/\s*\(\s*(?:initialSl|sl)\s*-
\s*entry\s*\)/gi
         },
         {
              code: 'SHORT_CURRENT_R_FORMULA',
              regex: /\(\s*entry\s*-\s*currentPrice\s*\)\s*\/\s*\(\s*(?:initialSl|sl)\s*-
\s*entry\s*\)/gi
         },
         {
...
/(?:parentFamilyFormat|parentFormat|exampleParent|exampleParentTrueMicroFamilyId)\s*:\s*
['"]MICRO_SHORT_/g,


/(?:selectableChildFamilyFormat|selectableChildFormat|exampleSelectableChild|exampleSelectableT
['"]MICRO_SHORT_/g
    ];
...
function checkTemporalContracts(sourceByFile, productionSource, errors) {
  const projectWideRequired = [
    ...
    ...TEMPORAL_GENERATION_LIFECYCLE,
    ...TEMPORAL_ACTIVE_DECISIONS,
    ...TEMPORAL_CANDIDATE_DECISIONS,
  ...
  if (scannerGroup.existing.length === 0) {
    errors.push({
      ...
      expectedAnyOf: ['api/scanner/run.js', 'src/market/scanner.js']
    });
  } else {
    addMissingTokenErrors(
      errors,
      ...
      [
        ...
        'sessionOverlap',
        'offHours',
        'scannerTemporalMetadataOnly',
        'scannerTemporalContextAuthoritative',
        'entryTemporalContextMustBeBuiltAtPositionOpen'
      ],
      `SCANNER_TEMPORAL_METADATA:${scannerGroup.existing.join(',')}`
    );
  ...
  if (tradeGroup.existing.length === 0) {
    ...
    addMissingTokenErrors(
      ...
      `TRADE_IMMUTABLE_ENTRY_CONTEXT:${tradeGroup.existing.join(',')}`
    );
    addMissingTokenErrors(
      errors,
      tradeGroup.source,
    ...
    addMissingTokenErrors(
      ...
      [
        ...
        'DISCORD_ENTRY_PUBLICATION_ONLY'
      ],
      `TRADE_TEMPORAL_DECISION:${tradeGroup.existing.join(',')}`
    );
  }
  ...
  if (analyzeGroup.existing.length === 0) {
    ...
    addMissingTokenErrors(
      ...
      analyzeGroup.source,
      [
        ...REQUIRED_TEMPORAL_RAW_ACCUMULATORS,
        'dayType',
        'dayOfWeek',
        'session',
        'canonicalOutcomeId',
      ...
      `ANALYZE_TEMPORAL_AGGREGATION:${analyzeGroup.existing.join(',')}`
    );
  }

  const freezeGroup = sourceForFiles([
    'src/analyze/rotationEngine.js',
    'api/analyze/weekly-freeze.js',
    'scripts/freezeWeekly.js'
  ], sourceByFile);
  addMissingTokenErrors(
    errors,
    freezeGroup.source,
    [
      ...REQUIRED_GENERATION_MANIFEST_FIELDS,
      ...TEMPORAL_GENERATION_LIFECYCLE,
      '50',
      '180',
      '35',
    ...
    `TEMPORAL_FREEZE_GENERATION:${freezeGroup.existing.join(',')}`
  );

  const productionUpper = productionSource.toUpperCase();
  const forbiddenTemporalFamilyPatterns = [
    /MICRO_LONG_[A-Z0-9_]+_(?:MONDAY|TUESDAY|WEDNESDAY|THURSDAY|FRIDAY|SATURDAY|SUNDAY)\b/,
    /MICRO_LONG_[A-Z0-9_]+_(?:ASIA|EUROPE|US|OFF_HOURS)\b/,
    /MICRO_LONG_[A-Z0-9_]+_(?:WEEKDAY|WEEKEND)\b/,
    /MICRO_LONG_[A-Z0-9_]+_[A-Z0-9]+USDT\b/
  ];
  for (const pattern of forbiddenTemporalFamilyPatterns) {
    if (pattern.test(productionUpper)) {
      errors.push({
        type: 'TEMPORAL_BUCKET_OR_SYMBOL_LEAKED_INTO_FAMILY_ID',
        pattern: String(pattern)
      });
    }
  }

  if (productionSource.includes('LONG:SHORT:') || productionSource.includes('SHORT:LONG:')) {
    errors.push({
      type: 'CROSS_ROOT_PREFIX_CONTAMINATION',
      forbidden: ['LONG:SHORT:', 'SHORT:LONG:']
    });
  }
}
    ...
    const longRiskAlternatives = [
        ...
        },
        {
         ...
         },
         {
              name: 'LONG_GROSS_R',
              patterns: [
                  /\(\s*exitPrice\s*-\s*entry\s*\)\s*\/\s*\(\s*entry\s*-\s*
(?:initialSl|sl)\s*\)/
              ]
         },
         {
              name: 'LONG_CURRENT_R',
              patterns: [
                  /\(\s*currentPrice\s*-\s*entry\s*\)\s*\/\s*\(\s*entry\s*-\s*
(?:initialSl|sl)\s*\)/
              ]
         }
    ];
    ...
    for (const contract of longRiskAlternatives) {
         const present = contract.patterns.some((pattern) =>
...
async function findTemporalBuilder(files, sourceByFile, exportsByFile) {
    for (const file of files) {
        ...
        for (const name of TEMPORAL_EXPORT_CANDIDATES) {
            ...
            if (!source.includes('getUTCHours') || !source.includes('getUTCDay'))
continue;
...
if (errors.length > 0) {
    ...
    console.log(JSON.stringify({
      ...
      temporalFamilyProfiling: 'PASS',
      temporalGeneration: 'PASS',
      weekendPolicy: 'PASS',
      sessionPolicy: 'PASS'
    }, null, 2));
}
