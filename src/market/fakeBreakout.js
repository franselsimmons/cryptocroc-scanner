...
if (volumeExpansion >= EXHAUSTION_VOLUME_EXPANSION) return
'BULL_VOLUME_EXPANSION';
...
const fake = sweptHigh && wickReject && (volumeExhaustion || btcAgainst ||
weakBody);
...
const validBreakout = closedAboveRange && !wickReject && (btcWith ||
...
fakeBreakdownReason: null,
breakoutType: fake ? 'FAKE_BREAKOUT' : validBreakout ? 'VALID_BREAKOUT' :
'NONE',
breakdownType: 'NONE',
...
return emptyResult('SHORT_DISABLED_LONG_ONLY', now(), temporalMetadata);
}
if (normalizedSide !== TARGET_SCANNER_SIDE) {
return emptyResult('UNKNOWN_OR_NON_BULL_SIDE', now(), temporalMetadata);
}
