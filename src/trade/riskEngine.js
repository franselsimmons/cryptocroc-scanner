...
function tradeConfig() {
  return {
    minRR: safeNumber(CONFIG.long?.trade?.minRR ?? CONFIG.trade?.minRR, 0.5),
    defaultRR: safeNumber(CONFIG.long?.trade?.defaultRR ?? CONFIG.trade?.defaultRR, 1.5),
    maxSpreadPct: safeNumber(CONFIG.long?.trade?.maxSpreadPct ?? CONFIG.trade?.maxSpreadPct, 0.015),

    minRiskPct: safeNumber(CONFIG.long?.trade?.minRiskPct ?? CONFIG.trade?.minRiskPct, 0.004),
    maxRiskPct: safeNumber(CONFIG.long?.trade?.maxRiskPct ?? CONFIG.trade?.maxRiskPct, 0.025),
    fallbackRiskPct: safeNumber(CONFIG.long?.trade?.fallbackRiskPct ?? CONFIG.trade?.fallbackRiskPct, 0.005),

    atrRiskMult: safeNumber(CONFIG.long?.trade?.atrRiskMult ?? CONFIG.trade?.atrRiskMult, 1.2),
    spreadRiskMult: safeNumber(CONFIG.long?.trade?.spreadRiskMult ?? CONFIG.trade?.spreadRiskMult, 5),
...
function inferTradeSideFromReason(row = {}) {
  ...
  if (hasShortSignal(reason) && !hasLongSignal(reason)) return OPPOSITE_TRADE_SIDE;
...
function modeFlags(row = {}) {
  ...
  return {
    ...
    trueMicroFamilyId: identity.exact75Child ? identity.childTrueMicroFamilyId : identity.trueMicroFamilyId,
    microFamilyId: identity.exact75Child ? identity.childTrueMicroFamilyId : identity.trueMicroFamilyId,
...
function btcRelation(side, btcState) {
  ...
  if (tradeSide === TARGET_TRADE_SIDE && ['BULLISH', 'STRONG_BULL', 'BULL'].includes(btc)) {
    return 'BTC_WITH';
  }
