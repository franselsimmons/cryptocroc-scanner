...
// - Explicit SHORT/BEAR/SELL input wordt geweigerd en produceert geen learnable
// outcome.
//
// Architectuur:
...
'LONG_COST_OPEN_VS_FINALIZED_PROJECTION_V2';
...
function calcLongCurrentR({ entry, initialSl, currentPrice } = {}) {
const e = safeNumber(entry, 0);
const s = safeNumber(initialSl, 0);
...
outcomeMeasurementGateMode: 'STRICT_EXACT_VERSION',
...
function emptyCostResult(reason = 'NON_LONG_COST_MODEL_SKIPPED', source =
DEFAULT_SOURCE) {
return {
...baseLongOnlyMeta({
skipped: true,
...
const explicitGrossR = Number(grossR);
const hasExplicitGrossR =
grossR !== null &&
grossR !== undefined &&
grossR !== '' &&
...
const grossR = calcLongGrossR({ entry: e, initialSl: s, exit: x });
const currentR = calcLongCurrentR({ entry: e, initialSl: s, currentPrice: p });
