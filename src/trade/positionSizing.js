...
//
// LONG-only virtual position sizing.
// Risk contribution = fraction of equity lost if a virtual LONG position hits
// initial SL.
// Required LONG risk geometry: sl < entry < tp.
// No real orders. No SHORT sizing. Exact 75-child trueMicroFamilyId is the
// selectable identity.
...
1,
safeNumber(
CONFIG.long?.rotation?.priorTrades ??
CONFIG.rotation?.longPriorTrades ??
...
familyGateDoesNotBlockVirtualLearning: true,
...
normalized.includes('TRADESIDE_LONG')) return TARGET_TRADE_SIDE;
if (normalized.includes('TRADE_SIDE_SHORT') ||
normalized.includes('TRADESIDE_SHORT')) return OPPOSITE_TRADE_SIDE;
...
if (fromIds === TARGET_TRADE_SIDE || fromIds === OPPOSITE_TRADE_SIDE) return
fromIds;
...
const id = extractTrueMicroFamilyId(row);
if (!id) return false;
return !isExactLongChildTrueMicroId(id);
}
...
const avgR = safeNumber(row.avgR ?? row.sampleAdjustedAvgR, 0);
if (completed <= 0) return 'OBSERVING';
if (completed < MIN_COMPLETED_ACTIVE_LEARNING) return 'EARLY_OUTCOMES';
if (completed < MIN_COMPLETED_FAMILY_GATE) return 'ACTIVE_LEARNING';
...
if (relation === 'BULLISH' || relation === 'STRONG_BULL' || relation === 'BULL'
|| relation === 'UP') {
return 'BTC_WITH';
}
if (relation === 'BEARISH' || relation === 'STRONG_BEAR' || relation === 'BEAR'
|| relation === 'DOWN') {
return 'BTC_AGAINST';
}
return 'BTC_UNKNOWN';
...
return entry > 0 && sl > 0 && tp > 0 && sl < entry && entry < tp;
}
...
trueMicroFamilyId: taxonomy.exactChild ? taxonomy.childTrueMicroFamilyId :
taxonomy.trueMicroFamilyId,
microFamilyId: taxonomy.exactChild ? taxonomy.childTrueMicroFamilyId :
taxonomy.trueMicroFamilyId,
...
if (!validLongRiskGeometry(weeklyStats)) {
return 0;
}
if (!cfg.enabled) {
...
if (!validLongRiskGeometry(position)) {
...
if (!validLongRiskGeometry(requestRow)) {
return {
ok: false,
reason: 'LONG_RISK_GEOMETRY_INVALID_SL_LT_ENTRY_LT_TP_REQUIRED',
side: TARGET_DASHBOARD_SIDE,
tradeSide: TARGET_TRADE_SIDE,
riskFraction: 0,
want,
