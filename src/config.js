...
const completedStatusRules = Object.freeze({
     ...
     EARLY_OUTCOMES: 'completed >= 1 && completed <= 19',
     ACTIVE_LEARNING: 'completed >= 20 && completed <= 34',
     PASSED: 'completed >= 35 && avgR > 0',
     EMPIRICAL_VETO: 'completed >= 35 && avgR <= 0'
});


const longRiskRules = Object.freeze({
...
const temporalConfig = Object.freeze({
  ...
  timezone: 'UTC',
  asiaActiveHoursUtc: [0, 8],
  europeActiveHoursUtc: [7, 16],
  usActiveHoursUtc: [13, 22],
  sessionBuckets: Object.freeze([
    'ASIA',
    'EUROPE',
    'US',
    'ASIA_EU_OVERLAP',
...
const redisConfig = Object.freeze({
  ...
  persistentLearningKey: PERSISTENT_LEARNING_KEY,
  redisKeysSeparatedFromShortRoot: true,
  maxRequestBytes: positiveInt(env.REDIS_MAX_REQUEST_BYTES, 9_500_000, 1_000_000,
50_000_000),
  shortRootTouched: false
});
...
const bitgetConfig = Object.freeze({
  ...
  minRequestIntervalMs: positiveInt(env.BITGET_MIN_REQUEST_INTERVAL_MS, 80, 0,
10_000),
...
const scannerConfig = Object.freeze({
  ...
  analyzeMaxCandidates: positiveInt(env.SCANNER_ANALYZE_MAX_CANDIDATES, 500, 1,
1000),
  analyzeMaxSymbols: positiveInt(env.SCANNER_ANALYZE_MAX_SYMBOLS, 500, 1, 1000),
  maxAnalyzeCandidates: positiveInt(env.SCANNER_MAX_ANALYZE_CANDIDATES, 500, 1,
1000),
  ...
  softMinQuoteVolume24h: boundedNum(env.SCANNER_SOFT_MIN_QUOTE_VOLUME_24H, 10_000,
0),
  ...
  snapshotTtlSec: positiveInt(env.SCANNER_SNAPSHOT_TTL_SEC, 30 * 60, 60, 24 *
3600),
  candleLimit: positiveInt(env.SCANNER_CANDLE_LIMIT, 100, 30, 500),
  fakeBreakoutLookback: positiveInt(env.SCANNER_FAKE_BREAKOUT_LOOKBACK, 24, 5,
200),
...
const tradeConfig = Object.freeze({
  ...
  maxCandidatesPerSnapshot: positiveInt(env.TRADE_MAX_CANDIDATES_PER_SNAPSHOT,
500, 1, 1000),
  analyzeMaxCandidatesPerSnapshot:
positiveInt(env.TRADE_ANALYZE_MAX_CANDIDATES_PER_SNAPSHOT, 500, 1, 1000),
  maxAnalyzeCandidatesPerSnapshot:
positiveInt(env.TRADE_MAX_ANALYZE_CANDIDATES_PER_SNAPSHOT, 500, 1, 1000),


  maxSnapshotAgeSec: positiveInt(env.TRADE_MAX_SNAPSHOT_AGE_SEC, 8 * 60, 30, 24 *
3600),
  ...
  requireScannerGateForLiveEntries:
bool(env.TRADE_REQUIRE_SCANNER_GATE_FOR_LIVE_ENTRIES, false),
  blockDiscoveryOnlyLiveEntries: bool(env.TRADE_BLOCK_DISCOVERY_ONLY_LIVE_ENTRIES,
false),
  allowFakeBreakoutLiveEntries: bool(env.TRADE_ALLOW_FAKE_BREAKOUT_LIVE_ENTRIES,
true),
  allowLowConfidenceLiveEntries: bool(env.TRADE_ALLOW_LOW_CONFIDENCE_LIVE_ENTRIES,
true),
...
bool(env.TRADE_ALLOW_STANDARDIZED_LEARNING_RISK_FALLBACK, true),
  allowStandardizedLearningRiskVirtualEntries:
bool(env.TRADE_ALLOW_STANDARDIZED_LEARNING_RISK_VIRTUAL_ENTRIES, true),
  standardizedLearningRiskRequiresScannerGatePassed:
bool(env.TRADE_STANDARDIZED_LEARNING_RISK_REQUIRES_SCANNER_GATE_PASSED, false),
  standardizedLearningRiskRequiresAnalyzeEligible:
bool(env.TRADE_STANDARDIZED_LEARNING_RISK_REQUIRES_ANALYZE_ELIGIBLE, false),
  standardizedLearningRiskRequiresSpreadGatePassed:
bool(env.TRADE_STANDARDIZED_LEARNING_RISK_REQUIRES_SPREAD_GATE_PASSED, false),
...
const analyzeConfig = Object.freeze({
  ...
  refineExecutionMicroIdsEnvRequested:
bool(env.ANALYZE_REFINE_EXECUTION_MICRO_IDS, false),
  ...
  shadowHorizonMin: positiveInt(env.ANALYZE_SHADOW_HORIZON_MIN, 6 * 60, 5, 7 * 24
* 60),
  shadowWeight: boundedNum(env.ANALYZE_SHADOW_WEIGHT, 0.35, 0, 1),
  obsDedupeTtlSec: positiveInt(env.ANALYZE_OBS_DEDUPE_TTL_SEC, 14 * 24 * 3600, 60,
90 * 24 * 3600),
  shadowDedupeTtlSec: positiveInt(env.ANALYZE_SHADOW_DEDUPE_TTL_SEC, 3 * 3600, 60,
14 * 24 * 3600),


  maxShadowMonitorsPerRun: positiveInt(env.ANALYZE_MAX_SHADOW_MONITORS_PER_RUN,
120, 1, 1000),


  freezeLockTtlSec: positiveInt(env.ANALYZE_FREEZE_LOCK_TTL_SEC, 600, 30, 3600),
  activateLockTtlSec: positiveInt(env.ANALYZE_ACTIVATE_LOCK_TTL_SEC, 600, 30,
3600),


  weekMicrosCompressionEnabled: bool(env.ANALYZE_WEEK_MICROS_COMPRESSION_ENABLED,
true),
  weekMicrosCompressionLevel:
positiveInt(env.ANALYZE_WEEK_MICROS_COMPRESSION_LEVEL, 6, 0, 9),
  maxMicroRowSetBytes: positiveInt(env.ANALYZE_MAX_MICRO_ROW_SET_BYTES, 250_000,
50_000, 5_000_000),


  weekMicrosTtlSec: positiveInt(env.ANALYZE_WEEK_MICROS_TTL_SEC, 21 * 24 * 3600,
24 * 3600, 365 * 24 * 3600),
  weekMetaTtlSec: positiveInt(env.ANALYZE_WEEK_META_TTL_SEC, 90 * 24 * 3600, 24 *
3600, 365 * 24 * 3600),
  ...
  topMicrosSnapshotLimit: positiveInt(env.ANALYZE_TOP_MICROS_SNAPSHOT_LIMIT, 300,
1, 5000),
  maxFullReadMicroRows: positiveInt(env.ANALYZE_MAX_FULL_READ_MICRO_ROWS, 1_500,
100, 50_000),
  fullReadSoftTimeoutMs: positiveInt(env.ANALYZE_FULL_READ_SOFT_TIMEOUT_MS, 2_400,
250, 30_000),
  preferTopSnapshotOnLargeIndex:
bool(env.ANALYZE_PREFER_TOP_SNAPSHOT_ON_LARGE_INDEX, true),


  maxExamplesPerMicro: positiveInt(env.ANALYZE_MAX_EXAMPLES_PER_MICRO, 8, 1, 100),
  maxRecentOutcomesPerMicro:
positiveInt(env.ANALYZE_MAX_RECENT_OUTCOMES_PER_MICRO, 8, 1, 100),
  maxDefinitionPartsPerMicro:
positiveInt(env.ANALYZE_MAX_DEFINITION_PARTS_PER_MICRO, 64, 8, 512),
  maxParentDefinitionPartsPerMicro:
positiveInt(env.ANALYZE_MAX_PARENT_DEFINITION_PARTS_PER_MICRO, 48, 8, 512),
  maxCounterKeysPerMicro: positiveInt(env.ANALYZE_MAX_COUNTER_KEYS_PER_MICRO, 18,
1, 100),
  maxCounterValuesPerCounter:
positiveInt(env.ANALYZE_MAX_COUNTER_VALUES_PER_COUNTER, 24, 1, 250),
  maxStoredStringLength: positiveInt(env.ANALYZE_MAX_STORED_STRING_LENGTH, 480,
64, 5000),
...
const rotationConfig = Object.freeze({
  ...
  minWeightedCompleted: boundedNum(env.ROTATION_MIN_WEIGHTED_COMPLETED,
MIN_COMPLETED_ACTIVE_LEARNING, 0, 100),
  activeLearningMinCompleted: MIN_COMPLETED_ACTIVE_LEARNING,
  minCompletedForActiveLearning: MIN_COMPLETED_ACTIVE_LEARNING,
  empiricalVetoMinCompleted: MIN_COMPLETED_EMPIRICAL_GATE,
  empiricalVetoMaxAvgR: 0,
  ...
  measurementFixVersion: MEASUREMENT_FIX_VERSION,
  outcomeMeasurementGateMode: OUTCOME_MEASUREMENT_GATE_MODE,
  requirePassedGateForManualActivation: true,
  learningStatusRules: completedStatusRules,
  ...
  minPrimaryRowsForPreviousMerge:
positiveInt(env.ROTATION_MIN_PRIMARY_ROWS_FOR_PREVIOUS_MERGE, 25, 0, 10_000),


  allowLegacyMacroActivation: false,


  allowSoftRotationFallback: bool(env.ROTATION_ALLOW_SOFT_ROTATION_FALLBACK,
true),
  allowObservationRotationFallback:
bool(env.ROTATION_ALLOW_OBSERVATION_ROTATION_FALLBACK, true),
  allowRawRotationFallback: bool(env.ROTATION_ALLOW_RAW_ROTATION_FALLBACK, true),


  allowManualUnknownTrueMicroIds: false,
  allowManualBelowMinCompleted: false,
  ...
  sampleReliabilityCap: boundedNum(env.ROTATION_SAMPLE_RELIABILITY_CAP, 50, 1,
10_000),
  avgRCap: boundedNum(env.ROTATION_AVG_R_CAP, 5, 0.1, 100),
  avgRSampleExponent: boundedNum(env.ROTATION_AVG_R_SAMPLE_EXPONENT, 1.35, 0.1,
5),
...
const discordConfig = Object.freeze({
  ...
  manualSelectionMatchMode: 'EXACT_TRUE_MICRO_FAMILY_ID',
  alertRule:
'CANDIDATE_TRUE_MICRO_FAMILY_ID_EQUALS_MANUALLY_SELECTED_75_CHILD_ID',
  ...
  requirePassedFamilyGate: true,
  requireCurrentMeasurementVersion: true,
  measurementFixVersion: MEASUREMENT_FIX_VERSION,
  outcomeMeasurementGateMode: OUTCOME_MEASUREMENT_GATE_MODE,
  blockNewWeekendEntries: true,
  allowWeekendExitAlerts: true,
  sessionMode: SESSION_MODE,
  sessionDiscordEntryAllowed: true,
  neverThrow: true,
...
const resetConfig = Object.freeze({
  ...
  learningConfirmText: str(env.LONG_RESET_LEARNING_CONFIRM_TEXT,
'RESET_LEARNING_LONG'),
  rotationConfirmText: str(env.LONG_RESET_ROTATION_CONFIRM_TEXT,
'RESET_ROTATION_LONG'),
...
const costConfig = Object.freeze({
  ...
  marketSlippagePct: boundedNum(env.COST_MARKET_SLIPPAGE_PCT ||
env.COST_SLIPPAGE_PCT, 0.0002, 0, 0.05),


  marketImpactPct: boundedNum(env.COST_MARKET_IMPACT_PCT, 0.0003, 0, 0.05),
  impactPct: boundedNum(env.COST_IMPACT_PCT || env.COST_MARKET_IMPACT_PCT, 0.0003,
0, 0.05),
...
export const CONFIG = Object.freeze({
  strategyVersion: str(env.STRATEGY_VERSION,
'LONG_FIXED_TAXONOMY_75_VIRTUAL_NET_V1'),
