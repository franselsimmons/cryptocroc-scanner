...
export const WEEKEND_POLICY_VERSION = 'LONG_WEEKEND_OBSERVE_DISCORD_BLOCK_V1';
export const SESSION_POLICY_VERSION = 'LONG_SESSION_OBSERVE_V1';
...
export function normalizeTemporalTimestamp(value = Date.now()) {
  const n = Number(value);
...
export function buildTemporalContextUtc(value = Date.now()) {
  const contextTs = normalizeTemporalTimestamp(value);
...
export function entryTemporalFields(source = {}, fallbackTs = Date.now()) {
  ...
  const entryIsWeekend = source.entryIsWeekend ?? source.isWeekend ?? context.isWeekend;
  return {
    ...
    weekendMode: WEEKEND_MODE,
    sessionMode: SESSION_MODE,
    entryTs: normalizeTemporalTimestamp(rawTs),
    ...
    entryIsWeekend,
    entrySessionTags: Array.isArray(source.entrySessionTags)
    ...
    weekendLearningAllowed: true,
    weekendVirtualEntryAllowed: true,
    weekendDiscordEntryAllowed: !entryIsWeekend,
    weekendExitMonitoringAllowed: true,
    weekendOutcomeRecordingAllowed: true,
    sessionLearningAllowed: true,
    sessionVirtualEntryAllowed: true,
...
export function exitTemporalFields(value = Date.now()) {
...
export function deriveLongLearningStatus(completedValue, avgRValue = 0) {
  const completed = Math.max(0, Math.floor(safeNumber(completedValue, 0)));
  const avgR = safeNumber(avgRValue, 0);
  if (completed === 0) return 'OBSERVING';
  if (completed < 20) return 'EARLY_OUTCOMES';
  if (completed < 35) return 'ACTIVE_LEARNING';
  return avgR > 0 ? 'PASSED' : 'EMPIRICAL_VETO';
}
...
export function filterAllowedTradeSides(rows = [], sideGetter = (row) =>
row?.tradeSide || row?.side) {
...
export function buildLongChildTrueMicroFamilyId(setup, regime,
confirmationProfile) {
...
export function stripSymbolTokensFromFamilyId(id = '', row = {}) {
    ...
    if (!raw) return raw;
    if (isSelectableLongTrueMicroFamilyId(raw) ||
isLongParentTrueMicroFamilyId(raw)) {
        return upper(raw);
    }
...
export function getObRelation(side, obBias) {
    ...
    if (!['BEARISH', 'BULLISH'].includes(ob)) return 'NEUTRAL';
...
export function bucketStep(value, step, prefix, decimals = 0) {
    ...
    return `${p}_${bucketClean(lower, decimals)}_${bucketClean(upperBound,
decimals)}`.toUpperCase();
}
