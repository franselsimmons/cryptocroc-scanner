...
export function webhookMoonPortfolio() {
  return process.env.DISCORD_WEBHOOK_PORTFOLIO_MOON || process.env.DISCORD_WEBHOOK_ELITE_MOON || "";
}
...
export async function fetchBTCGateCached() {
  ...
  // Fallback naar oude cache als row leeg is
  if (!row) {
    if (prev) return prev;
...
export async function fetchCoinGeckoTopCached() {
  ...
  const allMap = new Map(); // deduplicate op id
  ...
  // Omzetten naar array en velden normaliseren
  const all = [];
  ...
  // Fallback naar oude cache als er niets nieuws is
