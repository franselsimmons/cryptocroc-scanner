...
export async function sendDiscord(webhookUrl, content) {
  ...
  // FIX: Controleer of de url een geldig http(s) schema heeft
  if (!url.startsWith("http://") && !url.startsWith("https://")) {
    console.error("[MOON sendDiscord] Ongeldige webhook URL (geen http/https):", url);
    return { ok: false, error: "Invalid webhook URL (no http/https)" };
  }

  console.log("[MOON sendDiscord] url=", url); // FIX: tijdelijk voor debugging
