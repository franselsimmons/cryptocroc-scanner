...
export const MAIN_V2 = {
  bull: {
    minVol24h: 200_000, // Versoepeld
    minVmRadar: 0.02, // Versoepeld
    minVmBuildup: 0.10, // Versoepeld
    minVmAlmost: 0.15, // Versoepeld
    minVmElite: 0.20, // Versoepeld

    minCh1hRadar: -1.0, // Versoepeld
    minCh1hBuildup: 0.15,
    minCh1hAlmost: 0.5,
    minCh1hIgnition: 0.8,
    minCh1hExpansion: 2.0,

    minCh24Radar: -1.0, // Versoepeld
    minCh24Buildup: 1.5,
    minCh24Almost: 4.0,
    minCh24Ignition: 4.0,
    minCh24Expansion: 12,

    minObBull: 0.005, // Versoepeld
    minObStrong: 0.015,
    spreadMaxRadar: 3.0, // Versoepeld
    spreadMaxElite: 2.5, // Versoepeld

    maxExhaust24: 100, // Versoepeld
    minVelocity: 0.04, // Versoepeld
    strongVelocity: 0.07,
    explosiveVelocity: 0.12,

    maxMcapRadar: 1_200_000_000, // Versoepeld
    maxMcapBuildup: 800_000_000,
    maxMcapAlmost: 500_000_000,
    maxMcapElite: 350_000_000,
  },
  ...
  bear: {
    ...
    maxCh1hRadar: 1.0, // Versoepeld
    maxCh1hBuildup: -0.15,
    maxCh1hAlmost: -0.5,
    maxCh1hIgnition: -0.8,
    maxCh1hCascade: -2.0,

    maxCh24Radar: 1.0, // Versoepeld
    maxCh24Buildup: -1.5,
    maxCh24Almost: -4.0,
    maxCh24Ignition: -4.0,
    maxCh24Cascade: -12,

    minObBearAbs: 0.005, // Versoepeld
    minObStrongAbs: 0.015,
    spreadMaxRadar: 3.5, // Versoepeld
    spreadMaxElite: 3.0, // Versoepeld

    maxBounce1h: 4.0, // Versoepeld
    minVelocity: 0.04, // Versoepeld
    strongVelocity: 0.07,
    explosiveVelocity: 0.12,
...
export function computeBreakoutPressure(priceHist = []) {
  ...
  if (arr.length < 8) {
    return { breakoutPct: 0, ready: false };
  }
...
export function computeLiquidityScore({ ob, depthOk, spreadPct, depthMinUsd1p }) {
  if (!ob) return 30; // laag bij geen data
  ...
  if (Math.abs(obScore) > 0.08) score += 10; // sterke scheefheid = goede liquiditeit voor richting
...
export function computeMarketScore({ btc, mode, regime, whaleFlow }) {
  ...
  // whaleFlow: aantal actieve grote coins
...
export function computePerfectCandidateScore({ qualityScore, liquidityScore, timingScore, marketScore }) {
  // weging: kwaliteit 40%, timing 30%, liquiditeit 20%, markt 10%
  const total =
  ...
  try {
    const r = await fetch(
      ...
      new URLSearchParams({
        vs_currency: "usd",
        order: "volume_desc",
        per_page: "400", // Mee verhoogd voor prestaties
        page: "1",
        sparkline: "false",
