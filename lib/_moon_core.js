...
export const MAIN_V2 = {
  bull: {
    ...
    maxExhaust24: 80,
    minVelocity: 0.075,
    strongVelocity: 0.105,
    explosiveVelocity: 0.17,
...
const BLOCKED_SYMBOLS = [
  "USDT","BUSD","DAI","USDC","TUSD","PAX","UST","LUNC","LUNA","USTC","LUNA2","MIM","FRAX",
  "WBTC","WETH","RENBTC","3X","BEAR","BULL","DOWN","UP","HEDGE",
];
...
// ======================================================
// ✅ FIX 1: Bitget symbols cache + fallback
// ======================================================
const KV_BITGET_SYMBOLS_KEY = "bitget:spot:usdt:symbols:v1";
const KV_BITGET_SYMBOLS_TTL_SEC = 60 * 60 * 6; // 6 uur
...
export async function getBitgetSpotUsdtSymbols() {
  ...
  try {
    ...
    if (String(json?.code || "") !== "00000") throw new Error("bad_code");
    ...
    if (set.size > 0) {
      await kv.set(KV_BITGET_SYMBOLS_KEY, Array.from(set), { ex: KV_BITGET_SYMBOLS_TTL_SEC });
      return set;
    }
  } catch (e) {
    console.error("getBitgetSpotUsdtSymbols live error:", e?.message || e);
  }

  // 2) fallback naar KV
  try {
    const cached = await kv.get(KV_BITGET_SYMBOLS_KEY);
    const arr = Array.isArray(cached) ? cached : [];
    const set = new Set(arr.map((x) => up(x)).filter(Boolean));
    if (set.size > 0) {
      console.warn("Bitget symbols fallback -> using cached set:", set.size);
      return set;
    }
  } catch (e) {
    console.error("getBitgetSpotUsdtSymbols cache error:", e?.message || e);
  }

  // 3) laatste redmiddel: empty set (caller beslist)
  console.warn("Bitget symbols unavailable -> returning EMPTY set");
  return new Set();
}

// ======================================================
// ✅ FIX 2: CoinGecko pagination tot N coins (CG_TOP)
// ======================================================
const KV_CG_CACHE_KEY = "coingecko:markets:volume_desc:v2";
const KV_CG_CACHE_TTL_SEC = 45; // kort, maar genoeg om rate limit te verminderen
...
export async function fetchCoinGeckoTopCached(top = 250) {
  const want = Math.max(1, Math.min(2500, Number(top || 250)));
  // 0) KV cache
  try {
    const cached = await kv.get(KV_CG_CACHE_KEY);
    if (Array.isArray(cached) && cached.length > 0) {
      return cached.slice(0, want);
    }
  } catch {}

  const pages = Math.max(1, Math.ceil(want / 250));
  const all = [];

  for (let p = 1; p <= pages; p++) {
    try {
      const data = await fetchCoinGeckoPage(p);
      for (const c of data || []) {
      ...
      console.error("CoinGecko page fetch failed:", p, e?.message || e);
      break;
    }
    await sleep(250); // kleine delay tegen rate-limit
  }

  const sliced = all.slice(0, want);

  try {
    await kv.set(KV_CG_CACHE_KEY, sliced, { ex: KV_CG_CACHE_TTL_SEC });
  } catch {}
...
/**
 * ✅ FIX: softReady toegevoegd + ready iets makkelijker
 */
export function computeBreakoutPressure(priceHist = []) {
  ...
  if (arr.length < 8) return { breakoutPct: 0, pressure: 0, ready: false, softReady: false };
  ...
  const ready = pressure >= 60 && breakoutPct <= 6.0;
  const softReady = pressure >= 54 && breakoutPct <= 7.2;
...
export function adjustMoonConfigForRegime(baseCfg, regime) {
  ...
  if (r === "DRY") {
    ...
    cfg.maxCh24Almost = Math.min(0, n(cfg.maxCh24Almost, 0) + 1.0);
...
export function computeMoonProbabilities({ mode, coin, moveScore, velocity, compression, persistenceScore = 0 }) {
  ...
  const vmScore = vm >= 1.50 ? 100 : vm >= 0.80 ? 82 : vm >= 0.40 ? 65 : vm >= 0.20 ? 40 : 15;
...
export function computeQualityScore({ coin, moveScore, entryQuality, persistenceScore, velocity, compression, breakout }) {
  ...
  else if (breakout?.softReady) score += 6;
...
export function computeTimingScore({ mode, stage, breakout, volAcc, strongScans = 0, eliteScans = 0, lateEntry = false, exhausted = false, bounceTrap = false }) {
  ...
  else if (breakout?.softReady) score += 7;
...
export function computePerfectCandidateScore({ qualityScore, liquidityScore, timingScore, marketScore }) {
  const total =
    n(qualityScore, 0) * 0.38 +
    n(timingScore, 0) * 0.32 +
    n(liquidityScore, 0) * 0.22 +
    n(marketScore, 0) * 0.08;
