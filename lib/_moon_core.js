...
export function depthFloorUsd(mcap, tier, depthHist = []) {
  const factor =
    tier === TIERS[0] ? 0.8 : tier === TIERS[1] ? 1.0 : tier === TIERS[2] ? 1.5 : 2.0;
...
/**
 * ✅ FIX: breakout-ready was te hard -> kon funnels "dood" maken.
 * - ready: iets makkelijker (pressure>=60, breakoutPct<=6.0)
 * - softReady: extra signaal voor timing/quality, zonder meteen "A+ hard gate" te zijn
 */
export function computeBreakoutPressure(priceHist = []) {
  const arr = Array.isArray(priceHist)
    ...
    : [];
  if (arr.length < 8) {
    return { breakoutPct: 0, pressure: 0, ready: false, softReady: false };
  }
  ...
  // hard ready (voor echte "go")
  const ready = pressure >= 60 && breakoutPct <= 6.0;

  // soft ready (voor "almost/ignition" timing punten, voorkomt 0 signals)
...
export function computePersistenceScore({ priceHist = [], volHist = [], stageHist = [], mode }) {
  ...
  const s = Array.isArray(stageHist)
    ? stageHist.slice(-5).map((x) => String(x || "").toUpperCase())
    : [];
...
// ======================================================
// NIEUWE SCORE-FUNCTIES (voor perfecte candidate)
// ======================================================

/**
 * Berekent hoe goed de huidige BTC-conditie aansluit bij de mode.
 */
export function computeBtcAlignmentScore({ btc, mode, regime }) {
  if (!btc) return 50; // neutraal bij ontbreken data
  ...
  // Volatiliteit (range24) – hoge range helpt bij momentum
...
/**
 * Kwaliteit van de coin zelf (onafhankelijk van timing/markt)
 */
...
/**
 * Liquiditeitsscore op basis van orderboek
 * ✅ FIX: no orderbook was te hard (30) -> kills candidates wanneer OB fetch faalt/wordt overgeslagen.
 * - basis 40 i.p.v. 30 (nog steeds laag), en spread/depth rules doen de rest.
 */
export function computeLiquidityScore({ ob, depthOk, spreadPct, depthMinUsd1p }) {
  if (!ob) return 40; // was 30
...
/**
 * Timing: staat de coin er klaar voor?
 * ✅ FIX: timing mag ook "semi-ready" belonen, zodat je flow niet 0 wordt.
 */
export function computeTimingScore({
  ...
  else if (breakout?.softReady) score += 7; // ✅ nieuw
...
/**
 * Combineert de vier deel-scores tot een totaalscore voor een perfecte kandidaat.
 * ✅ micro-tune: iets meer nadruk op timing/liquidity (blind volgen)
 */
export function computePerfectCandidateScore({ qualityScore, liquidityScore, timingScore, marketScore }) {
  // was: 40/30/20/10
  // now: 38/32/22/8 (klein verschil, maar helpt execution+timing)
  const total =
...
export async function fetchCoinGeckoTopCached() {
  try {
    ...
    return data.map((c) => {
      ...
      const range24 =
        high24 > 0 && low24 > 0 ? ((high24 - low24) / ((high24 + low24) / 2)) * 100 : 0;
...
export function keyMainLatest(mode) {
  return `main:latest:${mode}`;
}
export function keyMainPortfolio(mode) {
  return `main:portfolio:${mode}`;
}
export function keyMainPositions(mode) {
  return `main:positions:${mode}`;
}
