...
export function webhookForMoonStage(stage) {
  ...
  if (s === "BUILDUP") return process.env.DISCORD_WEBHOOK_BUILDUP_MOON || "";
  if (s === "ALMOST") return process.env.DISCORD_WEBHOOK_ALMOST_MOON || "";
  if (s === "ELITE") return process.env.DISCORD_WEBHOOK_ELITE_MOON || "";
  return "";
}

export function webhookMoonPortfolio() {
  return (
    process.env.DISCORD_WEBHOOK_PORTFOLIO_MOON ||
    process.env.DISCORD_WEBHOOK_ELITE_MOON ||
    ""
  );
}
...
export function fmtMoonLine(item, mode, extra = "", tsNow = Date.now()) {
  ...
  const line2 = `t: ${fmtTs(tsNow)} | $${px.toFixed(8)} | 24h: ${
    ch >= 0 ? "+" : ""
  }${ch.toFixed(2)}% | VM: ${vm.toFixed(3)} | conf: ${conf}/100`;
...
export const keyMoonObSamples = (mode, symbol) =>
  `${NS}:ob:samples:${mode}:${String(symbol).toUpperCase()}`;
export const keyMoonObResult = (mode, symbol) =>
  `${NS}:ob:result:${mode}:${String(symbol).toUpperCase()}`;
...
export async function fetchBTCGateCached() {
  ...
  try {
    ...
    const range24 =
      high > 0 && low > 0 ? ((high - low) / ((high + low) / 2)) * 100 : 0;
...
export async function fetchCoinGeckoTopCached() {
  ...
  for (let p = MOON.CG_START_PAGE; p < MOON.CG_START_PAGE + MOON.CG_PAGES; p++) {
    ...
    try {
      const r = await fetch(urlVol, { headers: { accept: "application/json" } });
      if (!r.ok) continue;
  ...
  for (let p = MOON.CG_START_PAGE; p < MOON.CG_START_PAGE + MOON.CG_PAGES; p++) {
    ...
    try {
      const r = await fetch(urlMc, { headers: { accept: "application/json" } });
      if (!r.ok) continue;
      const j = await r.json().catch(() => []);
      if (Array.isArray(j)) {
        for (const row of j) {
...
export async function getBitgetSpotUsdtSymbols() {
  ...
  try {
    ...
    if (!r.ok || String(j?.code || "") !== "00000") {
      if (cached?.symbols?.length) return new Set(cached.symbols);
      return new Set();
    }
...
export function passAlmostMoon({ priceHist, volAcc, confidence, consistencyRatio, tier }) {
  if (n(confidence, 0) < MOON.almost.minConfidence) {
    return { ok: false, why: "Confidence low" };
  }
  if (priceFlatPct(priceHist, 60) > MOON.almost.maxFlat60Pct) {
    return { ok: false, why: "Too flat" };
  }
  if (n(volAcc, 1) < MOON.buildup.minVolAcc) {
    return { ok: false, why: "Vol acc too low" };
  }
  if (n(consistencyRatio, 0) < 0.66) {
    return { ok: false, why: "Consistency fail" };
  }
...
export function passEliteMoon({
  ...
  if (!obView.valid) {
    return { ok: false, why: `OB invalid: ${String(obView.reason || "bad")}` };
  }
  ...
  if (n(obView.spreadPct, 999) > spreadMax) {
    return { ok: false, why: "Spread too wide" };
  }
  if (n(obView.lor, 1) > lorMax) {
    return { ok: false, why: "Largest order suspicious" };
  }
  if (n(depthUsd, 0) < n(floorUsd, 0)) {
    return { ok: false, why: "Depth too thin" };
  }
...
export function computeMoonRisk({ mode, price, range24, confidence, depthOk, tier }) {
  ...
  const slMul = clamp(slBase + (1 - confK) * 0.1, 0.24, 0.46);
  const tpMul = clamp(tpBase + confK * 0.2, 0.56, 0.88);
  ...
  try {
    ...
    const payload =
      typeof diag === "object" && diag
        ? diag
        : { ts: Date.now(), mode: m, note: "bad diag" };
