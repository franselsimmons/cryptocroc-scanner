...
// ======================================================
// ✅ Universe settings (CoinGecko)
// ======================================================
const CG_TOP = 1500;          // 6 * 250
const CG_PER_PAGE = 250;
const CG_PAGES = Math.ceil(CG_TOP / CG_PER_PAGE);
const CG_CACHE_SEC = 90;      // KV cache TTL
const CG_KEY = "universe:coingecko:top:v2";
...
const BLOCKED_SYMBOLS = [
  "USDT","BUSD","DAI","USDC","TUSD","PAX","UST","LUNC","LUNA","USTC","LUNA2",
  "MIM","FRAX","WBTC","WETH","RENBTC","3X","BEAR","BULL","DOWN","UP","HEDGE",
];
...
/**
 * ✅ FIX: ready was te hard -> funnels konden “dood”
 */
...
// ======================================================
// ✅ Score functies (ongewijzigd uit jouw versie met softReady tweaks)
// ======================================================
...
// ======================================================
// ✅ New score functions (zelfde als jouw versie)
// ======================================================
...
export async function fetchCoinGeckoTopCached() {
  // 1) KV cache
  try {
    const cached = await kv.get(CG_KEY);
  ...
  try {
    ...
    for (let p = 1; p <= CG_PAGES; p++) {
      ...
      // tiny delay: reduce rate-limit
      await sleep(220);
    }

    const sliced = all.slice(0, CG_TOP);
    ...
    // 3) store KV
    try {
      await kv.set(CG_KEY, mapped, { ex: CG_CACHE_SEC });
    } catch {}
