...
// Escape Discord markdown-gevoelige tekens
function esc(x) {
  return String(x ?? "")
    .replace(/\\/g, "\\\\")
    .replace(/\*/g, "\\*")
...
// Formatteer Discord bericht op basis van funnel + item/event
function formatDiscordMessage(funnel, item) {
  const t = item?.tradePlan || {};
  const mode = esc(String(item?.mode || "").toUpperCase());
  const stage = esc(String(item?.stage || "").toUpperCase());
  const prevStage = esc(String(item?.prevStage || "").toUpperCase());
  const btcState = esc(String(item?.btcState || "-").toUpperCase());
