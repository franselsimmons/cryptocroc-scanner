...
export async function sendDiscord(webhook, title, description, color = 3066993) {
  ...
  const chunks = [];
  const max = 3800; // veilig onder Discord embed description limiet
...
// ======================================================
// Deduplicatie keys – Discord/events betrouwbaarder maken
// ======================================================
function dedupeKey(funnel, item) {
  ...
  if (f.startsWith("trade_")) {
    const entryPrice = fmtNum(item?.entryPrice, 8);
    const exitPrice = fmtNum(item?.exitPrice, 8);
    const barsOpen = String(item?.barsOpen ?? "");
    const reason = String(item?.exitReason || item?.reason || "").slice(0, 120);
    return `cc:dedupe:${f}:${symbol}:${entryPrice}:${exitPrice}:${barsOpen}:${reason}`;
  }
  ...
  const reason = String(item?.reason || "").slice(0, 120);
  return `cc:dedupe:${f}:${symbol}:${reason}`;
}
...
// ======================================================
// Titel + kleur per funnel
// ======================================================
function getDiscordMeta(funnel, item) {
  const f = String(funnel || "").toLowerCase();
  const symbol = String(item?.symbol || "?").toUpperCase();

  switch (f) {
    case "scan_transition":
      return { title: `Stage change • ${symbol}`, color: 3447003 };
    case "scan_entry":
      return { title: `Entry • ${symbol}`, color: 15158332 };
    case "scan_hold":
      return { title: `Hold • ${symbol}`, color: 5763719 };
    case "scan_sell":
      return { title: `Sell • ${symbol}`, color: 15548997 };
    case "scan_almost":
      return { title: `Almost • ${symbol}`, color: 15844367 };
    case "scan_buildup":
      return { title: `Buildup • ${symbol}`, color: 16776960 };
    case "scan_radar":
      return { title: `Radar • ${symbol}`, color: 3447003 };
    case "trade_tp":
      return { title: `Take Profit • ${symbol}`, color: 5763719 };
    case "trade_sl":
      return { title: `Stop Loss • ${symbol}`, color: 15548997 };
    case "trade_exit":
      return { title: `Exit • ${symbol}`, color: 9807270 };
    default:
      return { title: `Event • ${symbol}`, color: 3066993 };
  }
}

// ======================================================
// Formattering per funnel
// ======================================================
...
export async function pushEvent(funnel, eventData) {
  ...
  try {
    ...
    if (webhook) {
      const description = formatDiscordMessage(funnel, eventData);
      const meta = getDiscordMeta(funnel, eventData);
      await sendDiscord(webhook, meta.title, description, meta.color);
    }
  } catch (discordErr) {
