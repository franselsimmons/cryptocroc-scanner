...
/**
 * PURE BEAR MOON CORE - SUPER LOS
 * - veel meer intake
 * - bearish moon sneller zichtbaar
 * - zwakke coins mogen veel sneller door de funnel
 */
...
export function decideMoonStage({
  ...
  if (mode === "bear") {
    if (
      ...
      n(coin.change1h, 0) <= Math.min(-0.12, n(cfg.maxCh1hAlmost, 0) + 0.3) &&
      n(coin.change24, 0) <= Math.min(-0.6, n(cfg.maxCh24Almost, 0) + 1.2) &&
