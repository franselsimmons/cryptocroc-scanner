...
/**
 * PURE BEAR MOON CORE
 * - defensiever op slechte shorts
 * - ALMOST en ELITE duidelijk strenger
 */
export const MOON_V2 = {
  ...
  minPersistenceIgnition: 50,
  minPersistenceExpansion: 60,
};
...
export function adjustMoonConfigForRegime(baseCfg, regime) {
  ...
  if (r === "HEADWIND") {
    ...
    cfg.maxCh24Ignition = n(cfg.maxCh24Ignition, 0) - 0.8;
  }
  return cfg;
}
...
export function isBearBounceTrap(coin) {
  ...
  if (ch24 <= -45 && ch1h >= 4.5) return true;
  if (ch24 <= -60 && ch1h >= 2.5 && velocity < 0.04) return true;
  return false;
}
...
export function isLateBearEntry(coin) {
  ...
  if (ch24 <= -35 && ch1h <= -8) return true;
  if (ch24 <= -25 && vm < 0.12) return true;
...
export function computeBtcAlignmentScore({ btc, mode, regime }) {
  ...
  if (mode === "bear") {
    if (state === "BEAR") score += 18;
    else if (state === "BULL") score -= 12;
...
export function decideMoonStage({
  ...
  if (mode === "bear") {
    if (
      ...
      entryQuality >= 80 &&
      persistenceScore >= n(cfg.minPersistenceExpansion, 60)
    ) {
      stage = "ELITE_CASCADE";
      eliteType = "cascade";
    } else if (
      ...
      entryQuality >= 72 &&
      persistenceScore >= n(cfg.minPersistenceIgnition, 50)
    ) {
      stage = "ELITE_IGNITION";
      eliteType = "ignition";
    } else if (
      n(coin.change1h, 0) <= Math.min(-0.40, n(cfg.maxCh1hAlmost, 0) + 0.2) &&
      n(coin.change24, 0) <= Math.min(-2.0, n(cfg.maxCh24Almost, 0) + 0.8) &&
      n(coin.vm, 0) >= Math.max(0.14, n(cfg.minVmAlmost, 0) - 0.02) &&
      velocity >= Math.max(0.08, n(cfg.strongVelocity, 0) - 0.02)
    ) {
      stage = "ALMOST";
    } else if (
  ...
  if (isMoonEliteStage(stage) && !breakout.ready && entryQuality < 72) {
    stage = "ALMOST";
    eliteType = null;
  }
