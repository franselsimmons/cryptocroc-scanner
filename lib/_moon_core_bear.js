...
export function isBullExhausted() { return false; }
...
export function isLateBullEntry() { return false; }
...
export function computeMoonProbabilities({ mode, coin, moveScore, velocity, compression, persistenceScore = 0 }) {
  ...
  const dumpProbability = mode === "bear"
...
export function computeMoonRisk({ mode, price, range24, confidence, depthOk, tier, regime = "TREND", persistenceScore = 50 }) {
  ...
  // Bear: short positie -> SL boven, TP onder
...
const DESK_THRESHOLDS_MOON = {
  watchConfirmScans: 1, openConfirmScans: 1,
  watchMinHoldMs: 20 * 60 * 1000, openMinHoldMs: 12 * 60 * 1000,
  watchEnterEQ: 68, watchEnterPS: 54, watchEnterPressure: 50, watchEnterObScore: 0.004,
  watchStayEQ: 60, watchStayPS: 48,
  openEnterEQ: 72, openEnterPS: 56, openEnterPressure: 52,
  openStayEQ: 64, openStayPS: 50,
};
...
export function computeDeskGate({ mode, stage, entryQuality, persistenceScore, breakout, obScore, tradePlan, now, prevGate, prevMeta, isEliteStageForDesk }) {
  const aPlus = tradePlan && entryQuality >= 72 && persistenceScore >= 56 && breakout?.ready && Math.abs(obScore) >= 0.025;
...
export function decideMoonStage({
  ...
  if (mode === "bear") {
    if (n(coin.change1h,0) <= n(cfg.maxCh1hCascade,0) && n(coin.change24,0) <= n(cfg.maxCh24Cascade,0) &&
        n(coin.vm,0) >= n(cfg.minVmElite,0) && Math.abs(n(obx.score,0)) >= n(cfg.minObStrongAbs,0) &&
        n(obx.score,0) <= 0 && velocity >= n(cfg.explosiveVelocity,0) && entryQuality >= 76 &&
        persistenceScore >= n(cfg.minPersistenceExpansion,70)) {
      stage = "ELITE_CASCADE"; eliteType = "cascade";
    } else if (n(coin.change1h,0) <= n(cfg.maxCh1hIgnition,0) && n(coin.change24,0) <= n(cfg.maxCh24Ignition,0) &&
               n(coin.vm,0) >= n(cfg.minVmElite,0) && Math.abs(n(obx.score,0)) >= n(cfg.minObStrongAbs,0) &&
               n(obx.score,0) <= 0 && velocity >= n(cfg.strongVelocity,0) && entryQuality >= 66 &&
               persistenceScore >= n(cfg.minPersistenceIgnition,60)) {
      stage = "ELITE_IGNITION"; eliteType = "ignition";
    } else if (n(coin.change1h,0) <= Math.min(-0.9, n(cfg.maxCh1hAlmost,0)+0.3) &&
      ...
      stage = "ALMOST";
    } else if (n(coin.change1h,0) <= n(cfg.maxCh1hBuildup,0) && n(coin.change24,0) <= n(cfg.maxCh24Buildup,0) &&
