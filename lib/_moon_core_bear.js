 ...
 * MOON CORE BEAR
 * Doel:
 * - alleen echte downside momentum shorts
 * - bounce traps harder eruit
 * - strengere ALMOST / ELITE / desk gate
 */
...
export function adjustMoonConfigForRegime(baseCfg, regime) {
  ...
  if (r === "DRY") {
    ...
    cfg.minVmElite += 0.03;
    cfg.maxCh1hAlmost -= 0.15;
    cfg.maxCh24Almost -= 0.8;
  ...
  if (r === "EXPANSION") {
    ...
    cfg.maxCh24Ignition = Math.min(0, n(cfg.maxCh24Ignition, 0) + 0.8);
  ...
  if (r === "HEADWIND") {
    cfg.minVmAlmost += 0.02;
    cfg.minVmElite += 0.03;
    cfg.maxCh1hIgnition -= 0.20;
    cfg.maxCh24Ignition -= 1.0;
...
export function computeEliteQuality({
  ...
  if (absOb >= 0.02) score += 4;
  if (absOb >= 0.04) score += 6;
  if (absOb >= 0.07) score += 6;
...
export function isBearBounceTrap(coin) {
  ...
  if (ch24 <= -18 && ch1h >= 1.6) return true;
  if (ch24 <= -26 && ch1h >= 1.0 && velocity < 0.08) return true;
  if (ch24 <= -34 && ch1h >= 0.6) return true;
...
export function computeLiquidityScore({ ob, depthOk, spreadPct, depthMinUsd1p }) {
  ...
  const absOb = Math.abs(n(ob.score, 0));
  if (absOb > 0.03) score += 5;
  if (absOb > 0.06) score += 7;
  ...
  const wantWatchFinal =
    ...
    (isEliteStageForDesk ||
      (
        ...
        Math.abs(n(obScore, 0)) >= T.watchEnterObScore
      ));
...
export function decideMoonStage({
  ...
  const moveScore = CORE.computeBearMoveScore(coin, obx);
  ...
  const timingScore = CORE.computeTimingScore({
    ...
    lateEntry: CORE.isLateBearEntry(coin),
    exhausted: false,
    bounceTrap: CORE.isBearBounceTrap(coin),
  });
  ...
  if (
    ...
    n(obx.score, 0) <= -n(cfg.minObStrongAbs, 0) &&
    ...
    entryQuality >= 78 &&
    persistenceScore >= n(cfg.minPersistenceExpansion, 74) &&
    ...
    n(obx.score, 0) <= -n(cfg.minObBearAbs, 0) &&
    n(obx.spreadPct, 999) <= 1.35 &&
