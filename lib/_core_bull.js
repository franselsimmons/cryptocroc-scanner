...
export const SETTINGS = {
  ...
  radar: {
    ...
    mcapMax: 650_000_000,
    volMin: 1_800_000,          // was 1.2M
    vmMin: 0.10,                // was 0.08

    maxAbsChg24: 40,            // was 45
    maxRange24: 60,             // was 70
  ...
  // ===== BUILDUP (versoepeld) =====
  buildup: {
    minVolAcc: 1.02,            // was 1.04
  },

  // ===== ALMOST (versoepeld) =====
  almost: {
    minConfidence: 32,          // was 34
    maxFlat60Pct: 10.0,         // was 8.5
  },
  ...
  entry: {
    ...
    minAgree: 1,
    minConfidence: 36,          // was 38
    obScoreMin: 0.019,          // was 0.020
    spreadMaxPct: 1.90,         // was 1.80
    depthMinUsd1p: 10_000,      // was 11k

    adaptiveTiers: [
      { maxMc: 60_000_000,  minConf: 42, spreadMax: 1.90, depth1pMin: 9_500,  obScoreMin: 0.021 },
      { maxMc: 200_000_000, minConf: 40, spreadMax: 1.70, depth1pMin: 14_000, obScoreMin: 0.023 },
      { maxMc: 650_000_000, minConf: 38, spreadMax: 1.45, depth1pMin: 22_000, obScoreMin: 0.029 },
    ],
    ...
    dyn: {
      ...
      obScoreHardMin: 0.019,    // was 0.020
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  spreadMaxPct = clamp(spreadMaxPct, Number(dyn.spreadHardMinPct ?? 0.40), Number(dyn.spreadHardMaxPct ?? 2.6));
  ...
  depthMinUsd1p = Math.round(
    clamp(depthMinUsd1p, Number(dyn.depthHardMinUsd ?? 6_000), Number(dyn.depthHardMaxUsd ?? 240_000))
  );
