...
export const SETTINGS = {
  ...
  radar: {
    mcapMin: 5_000_000,
    mcapMax: 4_000_000_000, // Verhoogd
    volMin: 80_000, // Verlaagd
    vmMin: 0.005, // Verlaagd
    maxAbsChg24: 200, // Meer speling
    maxRange24: 200, // Meer speling
    dir1hMinBull: -0.20, // Versoepeld
    dir24MinBull: -0.50, // Versoepeld
    dir1hMaxBear: 0.50, // Versoepeld
    dir24MaxBear: 1.50, // Versoepeld
  },

  buildup: {
    minVolAcc: 0.65, // Versoepeld
  },

  almost: {
    minConfidence: 5, // Flink lager
    maxFlat60Pct: 60.0, // Meer range toegestaan
  },
  ...
  entry: {
    ...
    minConfidence: 8, // Versoepeld
    obScoreMin: 0.0005, // Versoepeld
    spreadMaxPct: 6.5, // Versoepeld
    depthMinUsd1p: 400, // Versoepeld
    ...
    dyn: {
      spreadHardMaxPct: 8.0,
      spreadHardMinPct: 0.10,
      depthHardMinUsd: 300,
      depthHardMaxUsd: 260_000,
      obScoreHardMin: 0.0002,
      obScoreHardMax: 0.05,
    },
  },
  ...
  btc: {
    ...
    bullMinChg24: 0.3, // Versoepeld
    bearMaxChg24: -0.3, // Versoepeld
  },
};
...
export function computeBtcState(btc, settings = SETTINGS) {
  ...
  const bullMin = Number(settings?.btc?.bullMinChg24 ?? 0.3);
  const bearMax = Number(settings?.btc?.bearMaxChg24 ?? -0.3);
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBear = Number(R.dir1hMaxBear ?? 0.50);
  const base24Bear = Number(R.dir24MaxBear ?? 1.50);

  const bull1h = clamp(base1hBull * (0.70 + 0.55 * s), -0.50, 0.50);
  const bull24 = clamp(base24Bull * (0.70 + 0.55 * s), -1.50, 2.0);
  const bear1hAbs = clamp(Math.abs(base1hBear) * (0.65 + 0.55 * s), 0.02, 0.80);
  const bear24Abs = clamp(Math.abs(base24Bear) * (0.65 + 0.55 * s), 0.05, 2.0);

  const baseMaxRange = Number(R.maxRange24 ?? 200);
  const dynMaxRange = clamp(baseMaxRange + s * 12, 70, 240);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  const mcScore = clamp(
    (Math.log10(mc + 1) - Math.log10(5_000_000 + 1)) /
      (Math.log10(4_000_000_000 + 1) - Math.log10(5_000_000 + 1)),
    0,
    1
  );
  const vmScore = clamp((vmr - 0.005) / (0.60 - 0.005), 0, 1);

  const liq = clamp(0.55 * volScore + 0.30 * vmScore + 0.15 * mcScore, 0, 1);

  const spreadBase = Number(thr.spreadMaxPct ?? entry.spreadMaxPct ?? 6.5);
  const spreadAdj = (1 - liq) * 0.25 - liq * 0.08;
  let spreadMaxPct = spreadBase * (1 + spreadAdj);
  spreadMaxPct = clamp(
    spreadMaxPct,
    Number(dyn.spreadHardMinPct ?? 0.10),
    Number(dyn.spreadHardMaxPct ?? 8.0)
  );

  const depthBase = Number(thr.depthMinUsd1p ?? entry.depthMinUsd1p ?? 400);
  const depthAdj = 0.75 + 0.55 * liq;
  let depthMinUsd1p = Math.round(depthBase * depthAdj);
  depthMinUsd1p = Math.round(
    clamp(
      depthMinUsd1p,
      Number(dyn.depthHardMinUsd ?? 300),
      Number(dyn.depthHardMaxUsd ?? 260_000)
    )
  );

  const scoreBase = Number(thr.obScoreMin ?? entry.obScoreMin ?? 0.0005);
  const scoreAdj = (1 - liq) * 0.20 - liq * 0.08;
  let obScoreMin = scoreBase * (1 + scoreAdj);
  obScoreMin = clamp(
    obScoreMin,
    Number(dyn.obScoreHardMin ?? 0.0002),
