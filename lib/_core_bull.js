...
export const SETTINGS = {
  ...
  entry: {
    ...
    dyn: {
      spreadHardMaxPct: 5.0,
      spreadHardMinPct: 0.25,
      depthHardMinUsd: 1_000,
      depthHardMaxUsd: 240_000,
      obScoreHardMin: 0.002,
      obScoreHardMax: 0.050,
    },
  },
...
export function computeBtcState(btc, settings = SETTINGS) {
  ...
  const bearMax = Number(settings?.btc?.bearMaxChg24 ?? -1.8);
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBear = Number(R.dir1hMaxBear ?? -0.04);
  const base24Bear = Number(R.dir24MaxBear ?? -0.12);

  const bull1h = clamp(base1hBull * (0.65 + 0.55 * s), -0.35, 0.35);
  const bull24 = clamp(base24Bull * (0.65 + 0.55 * s), -1.20, 1.20);
  const bear1hAbs = clamp(Math.abs(base1hBear) * (0.70 + 0.55 * s), 0.02, 0.35);
  const bear24Abs = clamp(Math.abs(base24Bear) * (0.70 + 0.55 * s), 0.05, 1.20);

  const baseMaxRange = Number(R.maxRange24 ?? 120);
  const dynMaxRange = clamp(baseMaxRange + (s * 12), 70, 140);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  const volScore = clamp(
    (Math.log10(vol + 1) - Math.log10(250_000 + 1)) /
      (Math.log10(60_000_000 + 1) - Math.log10(250_000 + 1)),
    0,
    1
  );
  const mcScore = clamp(
