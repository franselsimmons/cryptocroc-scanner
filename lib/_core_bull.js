...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBull = Number(R.dir1hMinBull ?? 0.00);
  ...
  const bull1h = clamp(base1hBull + s * 0.06, -0.02, 0.22);
  const bull24 = clamp(base24Bull + s * 0.12, 0.02, 1.15);
  const bear1hAbs = clamp(Math.abs(base1hBear) * (0.62 + 0.55 * s), 0.02, 0.40);
  const bear24Abs = clamp(Math.abs(base24Bear) * (0.62 + 0.55 * s), 0.05, 1.30);
