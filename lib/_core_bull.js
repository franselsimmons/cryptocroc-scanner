...
// ======================================================
// ✅ BULL CONFIG (alle tuning voor LONG / bull mode)
// ======================================================
...
export function computeConfidence({ vm, change24, range24, obValid }) {
  let c = 0;
  c += Math.max(0, Math.min(40, (Number(vm || 0) / 0.30) * 40));
  c += Math.max(0, Math.min(25, (Math.abs(Number(change24 || 0)) / 12) * 25));
  c += Math.max(0, 20 - Math.min(20, Number(range24 || 0) / 2));
  if (obValid) c += 15;
