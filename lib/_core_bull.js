...
export function computeBtcState(btc, settings = SETTINGS) {
  ...
  const bullMin = Number(settings?.btc?.bullMinChg24 ?? 0.55);
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base24Bull = Number(R.dir24MinBull ?? -0.08);
  ...
  const baseMaxRange = Number(R.maxRange24 ?? 150);
  const dynMaxRange = clamp(baseMaxRange + s * 18, 80, 175);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  spreadMaxPct = clamp(
    ...
    Number(dyn.spreadHardMaxPct ?? 6.0)
  );
