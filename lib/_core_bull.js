...
export const SETTINGS = {
  CG_TOP: 1500,
  RADAR_LIMIT: 80, // was 60

  radar: {
    mcapMin: 10_000_000,     // was 12M
    mcapMax: 650_000_000,    // was 500M
    volMin: 1_200_000,       // was 1.8M
    vmMin: 0.085,            // was 0.10

    maxAbsChg24: 40,         // bull iets minder ruimer dan bear
    maxRange24: 60,
  ...
  buildup: { minVolAcc: 1.05 },
  almost: { minConfidence: 37, maxFlat60Pct: 7.5 },
  ...
  entry: {
    ...
    adaptiveTiers: [
      { maxMc: 60_000_000,  minConf: 46, spreadMax: 1.70, depth1pMin: 11_000, obScoreMin: 0.024 },
      { maxMc: 200_000_000, minConf: 44, spreadMax: 1.50, depth1pMin: 16_000, obScoreMin: 0.026 },
      { maxMc: 650_000_000, minConf: 42, spreadMax: 1.35, depth1pMin: 22_000, obScoreMin: 0.030 },
    ],
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const dynMaxRange = clamp(baseMaxRange + (s * 6 - 4), 24, 40);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  spreadMaxPct = clamp(spreadMaxPct, Number(dyn.spreadHardMinPct ?? 0.50), Number(dyn.spreadHardMaxPct ?? 2.0));
  ...
  depthMinUsd1p = Math.round(clamp(depthMinUsd1p, Number(dyn.depthHardMinUsd ?? 10_000), Number(dyn.depthHardMaxUsd ?? 160_000)));
