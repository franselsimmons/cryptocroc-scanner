...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  // ✅ default aligned with SETTINGS.radar.maxRange24 (=60)
  const baseMaxRange = Number(R.maxRange24 ?? 60);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  // ✅ default aligned with SETTINGS.entry.spreadMaxPct (=1.90)
  const spreadBase = Number(thr.spreadMaxPct ?? entry.spreadMaxPct ?? 1.90);
  ...
  // ✅ default aligned with SETTINGS.entry.depthMinUsd1p (=10000)
  ...
  // ✅ default aligned with SETTINGS.entry.obScoreMin (=0.019)
  ...
  obScoreMin = clamp(
    obScoreMin,
    Number(dyn.obScoreHardMin ?? 0.019),
