...
export const SETTINGS = {
  ...
  entry: {
    samplesNeed: 2,                // ⬅ gewijzigd van 3 naar 2
    ...
    minAgree: 1,                    // ⬅ gewijzigd van 2 naar 1
    minConfidence: 50,
...
export function checkObSlopeGate({ stage, mode, obSamples, settings = SETTINGS }) {
  const m = String(mode || "bull").toLowerCase(); // let op: default bull i.p.v. bear
  ...
  // ⬅ gewijzigd: Math.max(2, need) i.p.v. Math.max(3, need)
