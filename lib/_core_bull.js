...
export const SETTINGS = {
  ...
  radar: {
    mcapMin: 12_000_000,
    mcapMax: 500_000_000,
    volMin: 2_500_000,           // ⬅ 2.5M (was 2.0M)
    vmMin: 0.12,                 // ⬅ 0.12 (was 0.10)

    maxAbsChg24: 35,
    maxRange24: 55,              // ⬅ 55 (was 60)

    dir1hMinBull: 0.12,          // ⬅ 0.12 (was 0.10)
    dir24MinBull: 0.27,          // ⬅ 0.27 (was 0.25)
    dir1hMaxBear: -0.12,         // ⬅ -0.12 (was -0.10)
    dir24MaxBear: -0.27,         // ⬅ -0.27 (was -0.25)
  },

  // ========== BUILDUP – iets strenger ==========
  buildup: {
    minVolAcc: 1.05,             // ⬅ 1.05 (was 1.00)
    // eventueel andere buildup‑parameters, bijv.:
    // minConfidenceBuildup: 32, // als die bestond (was 30)
  },

  // ========== ALMOST – iets strenger ==========
  almost: {
    minConfidence: 37,           // ⬅ 37 (was 35)
    maxFlat60Pct: 7.5,           // ⬅ 7.5 (was 8)
  },

  // ========== ENTRY – soepeler ==========
  entry: {
    samplesNeed: 2,                // al versoepeld (was 3)
    ...
    minAgree: 1,                   // al versoepeld (was 2)
    minConfidence: 48,             // ⬅ 48 (was 50)

    obScoreMin: 0.032,             // ⬅ 0.032 (was 0.034)
    spreadMaxPct: 1.35,            // ⬅ 1.35 (was 1.30)
    depthMinUsd1p: 20_000,         // ⬅ 20k (was 22k)

    adaptiveTiers: [
      // kleine marktkapitalisatie (0 - 60M)
      { maxMc: 60_000_000,  minConf: 50, spreadMax: 1.55, depth1pMin: 13_000, obScoreMin: 0.030 },
      // middel (60 - 200M)
      { maxMc: 200_000_000, minConf: 48, spreadMax: 1.37, depth1pMin: 20_900, obScoreMin: 0.032 },
      // groot (200 - 500M)
      { maxMc: 500_000_000, minConf: 46, spreadMax: 1.18, depth1pMin: 30_000, obScoreMin: 0.038 },
    ],
    ...
    dyn: {
      spreadHardMaxPct: 2.2,      // ⬅ 2.2 (was 2.0)
      spreadHardMinPct: 0.45,
      depthHardMinUsd: 7_000,     // ⬅ 7k (was 8k)
      depthHardMaxUsd: 200_000,   // ⬅ 200k (was 180k)
      obScoreHardMin: 0.026,      // ⬅ 0.026 (was 0.028)
      obScoreHardMax: 0.075,      // ⬅ 0.075 (was 0.070)
    },
  },
...
export function checkObSlopeGate({ stage, mode, obSamples, settings = SETTINGS }) {
  ...
  const minPts = Math.max(2, need); // ⬅ aangepast (was 3)
