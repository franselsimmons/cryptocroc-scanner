...
export const SETTINGS = {
  ...
  almost: { minConfidence: 30, maxFlat60Pct: 11.5 },
  ...
  btc: { softOpenNeutral: true, bullMinChg24: 0.9, bearMaxChg24: -1.0 },
};

export function getCfg() { return SETTINGS; }

export function keyLatest(mode) { return `latest:${String(mode || "bull").toLowerCase()}`; }
export function keyState(mode) { return `state:${String(mode || "bull").toLowerCase()}`; }
export function keyReset(mode) { return `reset:${String(mode || "bull").toLowerCase()}`; }
export function keyObSamples(mode, sym) { return `ob:samples:${String(mode || "bull").toLowerCase()}:${String(sym).toUpperCase()}`; }
export function keyObResult(mode, sym) { return `ob:result:${String(mode || "bull").toLowerCase()}:${String(sym).toUpperCase()}`; }
export function keyObResultMapTs(mode) { return `ob:mapts:${String(mode || "bull").toLowerCase()}`; }
export function keyDiagList(mode) { return `diag:list:${String(mode || "bull").toLowerCase()}`; }
export function keyDiagSnap(mode) { return `diag:snap:${String(mode || "bull").toLowerCase()}`; }
export const keyEntryLog = `logs:entry`;

export function computeVm(volume, marketCap) {
  const v = Number(volume || 0), m = Number(marketCap || 0);
  return (v > 0 && m > 0) ? v / m : 0;
}

export function computeRangePct(high24, low24) {
  const hi = Number(high24 || 0), lo = Number(low24 || 0);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  const mcScore = clamp((Math.log10(mc + 1) - Math.log10(15_000_000 + 1)) / (Math.log10(650_000_000 + 1) - Math.log10(15_000_000 + 1)), 0, 1);
  ...
  depthMinUsd1p = Math.round(clamp(depthMinUsd1p, Number(dyn.depthHardMinUsd ?? 5_000), Number(dyn.depthHardMaxUsd ?? 240_000)));
