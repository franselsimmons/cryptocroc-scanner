...
export const SETTINGS = {
  ...
  entry: {
    // Swing: je wil minder ruis => iets minder samples nodig,
    // maar wél binnen een groter tijd-window.
    samplesNeed: 6,
    ...
    resultTtlSec: 60 * 45, // 45 min (mag langer bij 30m scan)
    ...
    // OB slope sniper (per minuut)
    obSlopeEnabled: true,
    obSlopeMinBull: 0.0007,
    obSlopeMinBear: 0.0007, // alleen voor shape
  },
...
function filterFreshSamples(samples, windowSec) {
  ...
  // keep original shape as much as possible
  return fresh;
}
