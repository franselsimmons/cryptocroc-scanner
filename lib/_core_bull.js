...
/**
 * BULL core:
 * - Richting filters zijn bullish (positieve 1h/24h)
 * - BTC gate is bullish georiënteerd
 * - Entry thresholds zijn relatief soepel (zoals jij wil “eerst los, dan strenger”)
 */
export const SETTINGS = {
  // ✅ MAXIMALE UNIVERSE (CG_TOP bepaalt slice)
  CG_TOP: 1500,
  ...
  radar: {
    ...
    // bull direction baseline (dynamicRadarThresholds schaalt dit mee)
    dir1hMinBull: 0.06,
    dir24MinBull: 0.18,

    // bear fields blijven bestaan voor shared code, maar bull gebruikt ze niet
    dir1hMaxBear: -0.06,
    dir24MaxBear: -0.18,
  },
  ...
  // ========== BUILDUP ==========
  buildup: {
    // volume acceleration: now / 5 samples ago
    minVolAcc: 1.04,
  },
  ...
  entry: {
    ...
    obScoreMin: 0.020,
    spreadMaxPct: 1.80,
    depthMinUsd1p: 11_000,

    // marketcap tiers (override baseline)
    adaptiveTiers: [
      { maxMc: 60_000_000, minConf: 44, spreadMax: 1.80, depth1pMin: 10_000, obScoreMin: 0.022 },
      { maxMc: 200_000_000, minConf: 42, spreadMax: 1.60, depth1pMin: 15_000, obScoreMin: 0.024 },
      { maxMc: 650_000_000, minConf: 40, spreadMax: 1.35, depth1pMin: 23_000, obScoreMin: 0.030 },
    ],
    ...
    // dynamic clamps (hard safety)
    dyn: {
      spreadHardMaxPct: 2.6,
      spreadHardMinPct: 0.40,
      depthHardMinUsd: 6_000,
      depthHardMaxUsd: 240_000,
      obScoreHardMin: 0.020,
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  // scale factor based on volatility (range24)
  ...
  const baseMaxRange = Number(R.maxRange24 ?? 55);
  const dynMaxRange = clamp(baseMaxRange + (s * 10 - 6), 40, 90);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  const spreadBase = Number(thr.spreadMaxPct ?? entry.spreadMaxPct ?? 1.8);
  ...
  spreadMaxPct = clamp(
    ...
    Number(dyn.spreadHardMaxPct ?? 2.6)
  );

  const depthBase = Number(thr.depthMinUsd1p ?? entry.depthMinUsd1p ?? 11_000);
  ...
  depthMinUsd1p = Math.round(
    clamp(
      depthMinUsd1p,
      Number(dyn.depthHardMinUsd ?? 6_000),
      Number(dyn.depthHardMaxUsd ?? 240_000)
    )
  );

  const scoreBase = Number(thr.obScoreMin ?? entry.obScoreMin ?? 0.020);
  const scoreAdj = (1 - liq) * 0.25 - liq * 0.10;
  let obScoreMin = scoreBase * (1 + scoreAdj);

  obScoreMin = clamp(obScoreMin, Number(dyn.obScoreHardMin ?? 0.020), Number(dyn.obScoreHardMax ?? 0.070));
