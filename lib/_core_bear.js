...
export function computeBtcState(btc, settings = SETTINGS) {
  ...
  const bullMin = Number(settings?.btc?.bullMinChg24 ?? 0.8);
  const bearMax = Number(settings?.btc?.bearMaxChg24 ?? -0.8);
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBear = Number(R.dir1hMaxBear ?? -0.02);
  const base24Bear = Number(R.dir24MaxBear ?? -0.05);

  const bull1h = clamp(base1hBull * (0.65 + 0.55 * s), -0.35, 0.35);
  const bull24 = clamp(base24Bull * (0.65 + 0.55 * s), -1.2, 1.2);
  const bear1hAbs = clamp(Math.abs(base1hBear) * (0.70 + 0.55 * s), 0.01, 0.35);
  const bear24Abs = clamp(Math.abs(base24Bear) * (0.70 + 0.55 * s), 0.02, 1.2);
