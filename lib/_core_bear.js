...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBull = Number(R.dir1hMinBull ?? 0.04);
  const base24Bull = Number(R.dir24MinBull ?? 0.10);
  const base1hBear = Number(R.dir1hMaxBear ?? 0.20);
  const base24Bear = Number(R.dir24MaxBear ?? 0.50);

  const bull1h = clamp(base1hBull * (0.70 + 0.55 * s), 0.02, 0.35);
  const bull24 = clamp(base24Bull * (0.70 + 0.55 * s), 0.05, 1.20);
