...
export function computeBtcState(btc, settings = SETTINGS) {
  ...
  const bearMax = Number(settings?.btc?.bearMaxChg24 ?? -0.45);
...
export function computeConfidence({ vm, change24, range24, obValid }) {
  ...
  // Hard penalty voor dode / stable assets
  if (ch24 < 0.60) c -= 16;
  if (ch24 < 0.25) c -= 12;
  if (r24 < 1.20) c -= 16;
  if (r24 < 0.60) c -= 10;
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const bull1h = clamp(base1hBull * (0.60 + 0.55 * s), -0.30, 0.30);
  const bull24 = clamp(base24Bull * (0.60 + 0.55 * s), -1.0, 1.0);
  const bear1hAbs = clamp(Math.abs(base1hBear) * (0.62 + 0.62 * s), 0.01, 0.25);
  const bear24Abs = clamp(Math.abs(base24Bear) * (0.62 + 0.62 * s), 0.05, 0.95);

  const baseMaxRange = Number(R.maxRange24 ?? 135);
  const dynMaxRange = clamp(baseMaxRange + s * 14, 70, 155);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  spreadMaxPct = clamp(
    ...
    Number(dyn.spreadHardMaxPct ?? 6.4)
  );
