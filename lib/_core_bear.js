...
export function computeBtcState(btc, settings = SETTINGS) {
  ...
  const bearMax = Number(settings?.btc?.bearMaxChg24 ?? -0.55);
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBear = Number(R.dir1hMaxBear ?? -0.01);
  const base24Bear = Number(R.dir24MaxBear ?? -0.03);

  const bull1h = clamp(base1hBull * (0.55 + 0.65 * s), -0.35, 0.35);
  const bull24 = clamp(base24Bull * (0.55 + 0.65 * s), -1.2, 1.2);
  const bear1hAbs = clamp(Math.abs(base1hBear) * (0.58 + 0.70 * s), 0.005, 0.28);
  const bear24Abs = clamp(Math.abs(base24Bear) * (0.58 + 0.70 * s), 0.015, 0.90);

  const baseMaxRange = Number(R.maxRange24 ?? 145);
  const dynMaxRange = clamp(baseMaxRange + s * 18, 75, 170);
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  const mcScore = clamp(
    (Math.log10(mc + 1) - Math.log10(3_000_000 + 1)) /
      (Math.log10(3_500_000_000 + 1) - Math.log10(3_000_000 + 1)),
    0,
    1
  );
  const vmScore = clamp((vmr - 0.008) / (0.50 - 0.008), 0, 1);

  const liq = clamp(0.52 * volScore + 0.30 * vmScore + 0.18 * mcScore, 0, 1);

  const spreadBase = Number(thr.spreadMaxPct ?? entry.spreadMaxPct ?? 5.4);
