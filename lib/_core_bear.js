...
export const SETTINGS = {
  ...
  radar: {
    ...
    // Baseline caps (worden dynamisch bijgesteld per coin via helpers hieronder)
    maxAbsChg24: 25,
    maxRange24: 28,

    // Baseline direction (wordt dynamisch bijgesteld per coin via helpers hieronder)
    dir1hMinBull: 0.20,
    dir24MinBull: 0.50,
    dir1hMaxBear: -0.20,
    dir24MaxBear: -0.50,
  ...
  entry: {
    ...
    // Baseline OB thresholds (worden dynamisch bijgesteld per coin)
    obScoreMin: 0.05,
    spreadMaxPct: 0.95,
    ...
    // ✅ nieuw: limieten voor dynamische OB tuning (veiligheidsrails)
    dyn: {
      // spread mag nooit boven dit (hard cap)
      spreadHardMaxPct: 1.6,
      // spread mag nooit lager dan dit (te streng voor illiquide coins)
 ...
 *
 * Idea:
 * - Rustige coins (range < ~10) → lagere dir drempels
 * - Volatiele coins (range > ~25) → hogere dir drempels
 */
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  // dynamic maxRange: allow a bit more for naturally volatile coins, but cap it
  // baseline maxRange24 ~ 28; volatile coins can go to 34, calm coins ~ 24
...
/**
 * Make OB thresholds adapt per coin using liquidity proxies:
 * - volume (24h) and marketCap
 * - vm = volume/marketCap
 *
 * Returns thresholds that your scan.js can use instead of static ones.
 */
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  // Baseline from tiers (already computed by your adaptive tiers)
  const thr = { ...(baseThr || {}) };

  // Liquidity score 0..1 using log-ish compression
  // - vol 3.5M .. 60M maps ~0..1
  const volScore = clamp((Math.log10(vol + 1) - Math.log10(3_500_000 + 1)) / (Math.log10(60_000_000 + 1) - Math.log10(3_500_000 + 1)), 0, 1);
  // - mc 20M .. 400M maps ~0..1
  const mcScore = clamp((Math.log10(mc + 1) - Math.log10(20_000_000 + 1)) / (Math.log10(400_000_000 + 1) - Math.log10(20_000_000 + 1)), 0, 1);
  // - vm 0.14 .. 0.60 maps ~0..1
  const vmScore = clamp((vmr - 0.14) / (0.60 - 0.14), 0, 1);

  // overall liquidity score: volume matters most, then vm, then mc
  const liq = clamp(0.55 * volScore + 0.30 * vmScore + 0.15 * mcScore, 0, 1);

  // Spread: illiquid coins get a bit more tolerance; liquid coins get stricter
  // Move within [-0.20 .. +0.35] relative factor
  const spreadBase = Number(thr.spreadMaxPct ?? entry.spreadMaxPct ?? 0.95);
  const spreadAdj = (1 - liq) * 0.35 - liq * 0.10; // illiquid:+, liquid:-
  ...
  // Depth: liquid coins must have more depth; illiquid coins can pass with less
  // But never below a hard min.
  ...
  // OB score: for illiquid coins demand a bit stronger signal; for liquid coins slightly less
  const scoreBase = Number(thr.obScoreMin ?? entry.obScoreMin ?? 0.05);
  const scoreAdj = (1 - liq) * 0.25 - liq * 0.10; // illiquid:+, liquid:-
