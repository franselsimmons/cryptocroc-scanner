...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  // ✅ default aligned with SETTINGS.radar.maxRange24 (=70)
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  ...
  // ✅ default aligned with SETTINGS.entry.spreadMaxPct (=2.05)
  ...
  // ✅ default aligned with SETTINGS.entry.depthMinUsd1p (=9000)
  ...
  depthMinUsd1p = Math.round(
    clamp(
      depthMinUsd1p,
      Number(dyn.depthHardMinUsd ?? 5_500),
      Number(dyn.depthHardMaxUsd ?? 260_000)
    )
  );

  // ✅ default aligned with SETTINGS.entry.obScoreMin (=0.018)
  ...
  obScoreMin = clamp(
    obScoreMin,
    Number(dyn.obScoreHardMin ?? 0.018),
...
// FIX: slope calculation now works with as few as 2 points
