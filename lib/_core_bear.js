...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBear = Number(R.dir1hMaxBear ?? -0.005);
  const base24Bear = Number(R.dir24MaxBear ?? -0.015);
  ...
  const dynMaxRange = clamp(baseMaxRange + s * 18, 75, 190);
