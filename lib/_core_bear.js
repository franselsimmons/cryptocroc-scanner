...
export const SETTINGS = {
  ...
  radar: {
    ...
    dir1hMinBull: -0.50, // Versoepeld
    dir24MinBull: -1.50, // Versoepeld
    dir1hMaxBear: 0.20, // Versoepeld
    dir24MaxBear: 0.50, // Versoepeld
  },
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBull = Number(R.dir1hMinBull ?? -0.50);
  const base24Bull = Number(R.dir24MinBull ?? -1.50);
  const base1hBear = Number(R.dir1hMaxBear ?? 0.20);
  const base24Bear = Number(R.dir24MaxBear ?? 0.50);

  const bull1h = clamp(base1hBull * (0.65 + 0.55 * s), -0.80, 0.50);
  const bull24 = clamp(base24Bull * (0.65 + 0.55 * s), -2.0, 2.0);
  const bear1hAbs = clamp(Math.abs(base1hBear) * (0.70 + 0.55 * s), 0.01, 0.80);
  const bear24Abs = clamp(Math.abs(base24Bear) * (0.70 + 0.55 * s), 0.02, 2.0);

  const baseMaxRange = Number(R.maxRange24 ?? 180);
  const dynMaxRange = clamp(baseMaxRange + s * 12, 70, 220);
