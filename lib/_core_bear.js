...
export const SETTINGS = {
  ...
  buildup: { minVolAcc: 1.01 },
  almost: { minConfidence: 30, maxFlat60Pct: 11.0 },
  ...
  entry: {
    ...
    dyn: {
      ...
      obScoreHardMin: 0.018,
      obScoreHardMax: 0.070,
    },
  },

  btc: { softOpenNeutral: true, bullMinChg24: 1.0, bearMaxChg24: -0.9 },
};

export function getCfg() { return SETTINGS; }

// keys en helpers zijn identiek aan bull, alleen met "bear" default
export function keyLatest(mode) { return `latest:${String(mode || "bear").toLowerCase()}`; }
export function keyState(mode) { return `state:${String(mode || "bear").toLowerCase()}`; }
export function keyReset(mode) { return `reset:${String(mode || "bear").toLowerCase()}`; }
export function keyObSamples(mode, sym) { return `ob:samples:${String(mode || "bear").toLowerCase()}:${String(sym).toUpperCase()}`; }
export function keyObResult(mode, sym) { return `ob:result:${String(mode || "bear").toLowerCase()}:${String(sym).toUpperCase()}`; }
export function keyObResultMapTs(mode) { return `ob:mapts:${String(mode || "bear").toLowerCase()}`; }
export function keyDiagList(mode) { return `diag:list:${String(mode || "bear").toLowerCase()}`; }
export function keyDiagSnap(mode) { return `diag:snap:${String(mode || "bear").toLowerCase()}`; }
export const keyEntryLog = `logs:entry`;

export function computeVm(volume, marketCap) { const v = Number(volume || 0), m = Number(marketCap || 0); return (v > 0 && m > 0) ? v / m : 0; }
export function computeRangePct(high24, low24) { const hi = Number(high24 || 0), lo = Number(low24 || 0); return (hi > 0 && lo > 0) ? ((hi - lo) / lo) * 100 : 0; }
...
function clamp(x, a, b) { const v = Number(x); return Number.isFinite(v) ? Math.max(a, Math.min(b, v)) : a; }
...
export function dynamicRadarThresholds(range24Pct, settings = SETTINGS) {
  ...
  const base1hBull = Number(R.dir1hMinBull ?? 0.06);
  const base24Bull = Number(R.dir24MinBull ?? 0.18);
  ...
  const baseMaxRange = Number(R.maxRange24 ?? 70);
  const dynMaxRange = clamp(baseMaxRange + (s * 12 - 6), 50, 95);
  return {
...
export function dynamicEntryThresholds({ marketCap, volume, vm }, baseThr, settings = SETTINGS) {
  const entry = settings?.entry || {}, dyn = entry?.dyn || {};
  const mc = Math.max(0, Number(marketCap || 0));
  const vol = Math.max(0, Number(volume || 0));
  const vmr = Math.max(0, Number(vm || 0));
  const thr = { ...(baseThr || {}) };
  const volScore = clamp((Math.log10(vol + 1) - Math.log10(2_500_000 + 1)) / (Math.log10(60_000_000 + 1) - Math.log10(2_500_000 + 1)), 0, 1);
  const mcScore = clamp((Math.log10(mc + 1) - Math.log10(15_000_000 + 1)) / (Math.log10(750_000_000 + 1) - Math.log10(15_000_000 + 1)), 0, 1);
  ...
  const depthBase = Number(thr.depthMinUsd1p ?? entry.depthMinUsd1p ?? 9_000);
  const depthAdj = 0.72 + 0.70 * liq;
  let depthMinUsd1p = Math.round(depthBase * depthAdj);
  depthMinUsd1p = Math.round(clamp(depthMinUsd1p, Number(dyn.depthHardMinUsd ?? 5_500), Number(dyn.depthHardMaxUsd ?? 260_000)));
  const scoreBase = Number(thr.obScoreMin ?? entry.obScoreMin ?? 0.018);
  const scoreAdj = (1 - liq) * 0.25 - liq * 0.10;
  let obScoreMin = scoreBase * (1 + scoreAdj);
  obScoreMin = clamp(obScoreMin, Number(dyn.obScoreHardMin ?? 0.018), Number(dyn.obScoreHardMax ?? 0.070));
  return { ...thr, spreadMaxPct, depthMinUsd1p, obScoreMin, liqScore: liq };
}

// slope helpers (zelfde als bull, met min 2 punten)
function filterFreshSamples(samples, windowSec, field = "score") {
  const arr = Array.isArray(samples) ? samples : [];
  const win = Math.max(60, Number(windowSec || 0)) * 1000;
  const now = Date.now();
  return arr.map(s => ({ ts: Number(s?.ts || 0), y: Number(s?.[field]) }))
...
export function calcSlopeFromSamplesPoints(points) {
  ...
  const sorted = pts.map(p => ({ t: Number(p?.ts || p?.t || 0), y: Number(p?.y) }))
...
export default {
  SETTINGS, getCfg, keyLatest, keyState, keyReset, keyObSamples, keyObResult, keyObResultMapTs,
  keyDiagList, keyDiagSnap, keyEntryLog, computeVm, computeRangePct, computeBtcState, computeConfidence,
  dynamicRadarThresholds, dynamicEntryThresholds, checkObSlopeGate,
};