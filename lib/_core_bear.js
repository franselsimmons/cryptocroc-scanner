...
export const SETTINGS = {
  ...
  entry: {
    // ✅ 30m cadence: met 3h window heb je max 6 samples.
    // 6 nodig is vaak te streng → naar 4.
    samplesNeed: 4,
  ...
  btc: {
    ...
    // ✅ harde drempels voor BTC state (zelfde als bull)
    bullMinChg24: 1.0,
    bearMaxChg24: -1.0,
  },
};
...
/**
 * ✅ BTC state: 1 logica voor bull + bear
 * - chg24 >= +1.0 → BULL
 * - chg24 <= -1.0 → BEAR
 * - anders → NEUTRAL (en als softOpenNeutral aan staat: treat als "open")
 */
 ...
 * ✅ 30m cadence: 4 punten is al bruikbaar → niet meer 6 eisen.
 */
...
export function checkObSlopeGate({ stage, mode, obSamples, settings = SETTINGS }) {
  ...
  // ✅ was: Math.max(6, need) → nu: Math.max(4, need)
