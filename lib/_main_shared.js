...
export async function fetchBTCGateFromUniverse() {
  try {
    ...
    const btc = Array.isArray(arr) ? arr[0] : arr;
...
export async function fetchContractConfigs() {
  try {
    ...
    const map = new Map();
    res?.data?.forEach(c => map.set(c.symbol, c));
    return map;
  } catch {
    return new Map();
  }
...
export async function fetchFuturesTickers() {
  try {
    ...
    const res = await fetchJsonWithTimeout(url);
    const map = new Map();
    res?.data?.forEach(t => map.set(t.symbol, t));
    return map;
  } catch {
    return new Map();
  }
...
export function calculateFuturesSize(entry, sl, riskUsd, cfg) {
  if (!entry || !sl || entry === sl || !cfg) return 0;
  const riskPerUnit = Math.abs(entry - sl);
  const rawSize = riskUsd / riskPerUnit;

  const step = parseFloat(cfg.sizeMultiplier || 0.01);
  const precision = parseInt(cfg.volumePlace || 2);

  const steps = Math.floor(rawSize / step);
