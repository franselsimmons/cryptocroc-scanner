...
export async function fetchBTCGateFromUniverse() {
  try {
    ...
    const btc = arr;
    return {
      price: Number(btc.current_price |

| 0),
      chg24: Number(btc.price_change_percentage_24h |

| 0),
      chg1h: Number(btc.price_change_percentage_1h |
...
// Bitget Futures Specifieke functies
...
| 0);
  ...
  return { 
    bestAsk: ask, bestBid: bid, spreadPct, depthMinUsd1p: 0, score: 0, 
    valid: ask > 0 && bid > 0, fresh: true, stale: false, reason: "shallow" 
  };
}
...
| entryPrice === slPrice ||!contractConfig) return 0;
...
// Wrapper om oude Main core naadloos te laten integreren
export function preScoreMainCoin(coin, CORE) {
