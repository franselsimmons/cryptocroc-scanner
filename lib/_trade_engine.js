...
//
// Extra hardening (jouw request: "bijna perfect"):
// - Missing/failed OB snapshot mag NOOIT emergency-close veroorzaken
// - entryAgeMinutes wordt automatisch afgeleid uit entryAt als het niet is meegegeven
// - PnL velden defaulten safe (0) als position manager ze niet zet
//
...
//   lastExitAt: 0,
//
//   entryAgeMinutes: 12, // optioneel; wordt anders afgeleid uit entryAt
// }
//
// scannerGate:
...
function isMissingObSnapshot(coin) {
  const reason = String(coin?.ob?.reason || "").toLowerCase();
  const status = String(coin?.ob?.status || "").toLowerCase();
  const valid = coin?.ob?.valid === true;
  const fresh = coin?.ob?.fresh === true;

  // meest voorkomende scanner fallback: reason=missing_snapshot, status=none, valid=false, fresh=false
  if (reason === "missing_snapshot") return true;
  if (status === "none") return true;
  if (!valid && !fresh) return true;
...
function buildBaseChecklist({ coin, btc, regime, mode }) {
  ...
  const obMissing = isMissingObSnapshot(coin);

  // depth/spread kunnen "999/0" zijn als snapshot faalt -> dat mag niet als hard fail tellen
  const depthOkRaw = !!coin.thresholds?.depthOk;
  const depthOk = obMissing ? true : depthOkRaw;

  const spreadRaw = n(coin.ob?.spreadPct, 999);
  const spread = obMissing ? Math.min(spreadRaw, 0.75) : spreadRaw; // clamp voor scoring/softDamage, niet voor echte OB logic
  ...
  checklist.push({
    name: "Fresh OB",
    ok: freshOb || obMissing, // als OB missing is, behandel dit niet als negatief in checklist
    value: obMissing ? "missing_snapshot" : freshOb ? "ja" : "nee",
...
function getEmergencyExitReason({
  ...
  // HARDENING: als OB snapshot ontbreekt, NOOIT sluiten op spread/depth
  if (!obMissing) {
    if (!depthOkRaw) return "depth_failed";
    if (spreadRaw >= emergencySpread) return "spread_explosion";
  }
...
function isSoftDamage({
  ...
  // HARDENING: spread niet gebruiken als snapshot ontbreekt
  if (!obMissing && spread >= config.weakHoldSpreadMax) return true;
...
function shouldReduce({
  ...
  if (
    trailingActive &&
    maxPnlPct >= config.reduceAfterPnlPct &&
    (maxPnlPct - currentPnlPct) >= config.reduceDrawbackPct
  ) {
    return true;
  }
...
function buildExecutionDecision({
  ...
  const adaptiveConfig = getAdaptiveConfig(config, coin, coinProfile, coinProfile?.systemType);
  ...
  // PnL/state velden: safe defaults
  ...
  // HARDENING: entryAgeMinutes afleiden uit entryAt als niet meegegeven
  const entryAt = n(positionState.entryAt, 0);
  const now = Date.now();
  const derivedAgeMinutes =
    entryAt > 0 ? Math.max(0, Math.floor((now - entryAt) / 60000)) : 0;
  const entryAgeMinutes = n(positionState.entryAgeMinutes, derivedAgeMinutes);

  const base = buildBaseChecklist({ coin, btc, regime, mode });
  const {
    checklist,
    btcOk,
    btcHardAgainst,
  ...
  // HARDENING: spread checks niet hard laten falen bij missing snapshot
  const spreadOpenOk = obMissing ? true : spreadRaw < adaptiveConfig.openSpreadMax;

  const freshEntrySoftOk =
    freshOb ||
    obMissing || // missing snapshot -> niet negatief maken
    breakoutReady ||
  ...
  const spreadHoldOk = obMissing ? true : spreadRaw < adaptiveConfig.holdSpreadMax;
  ...
  const spreadWeakOk = obMissing ? true : spreadRaw < adaptiveConfig.weakHoldSpreadMax;
  ...
  const score = calcScore([
    ...
    persistenceOpenOk,
    breakoutOpenOk,
    depthOk, // depthOk is al "true" gemaakt bij obMissing
    spreadOpenOk,
    tradeCandidate,
    stageOk,
    freshEntrySoftOk,
  ...
  const softDamage = isSoftDamage({
    ...
    persistence,
    breakoutPressure,
    spread, // spread is clamped bij obMissing
  ...
  const holdOk =
    ...
    ((
      ...
      (
        winnerRunning &&
        // HARDENING: obMissing => spread condition is true already
        (obMissing ? true : spreadRaw < adaptiveConfig.weakHoldSpreadMax) &&
  ...
  const weakHoldLimit = winnerRunning ? maxWeakHoldCycles + 1 : maxWeakHoldCycles;
...
export function buildMainExecutionDecision({
  ...
  return buildExecutionDecision({
    ...
    config: {
      ...
      openQualityMin: 56,
      openPersistenceMin: 48,
      openBreakoutPressureMin: 50,
      openSpreadMax: 1.12,
      openScoreMin: 56,
      maxEntryAgeMinutes: 24,

      holdQualityMin: 44,
      holdPersistenceMin: 36,
      holdSpreadMax: 1.42,
      holdScoreMin: 40,

      weakHoldQualityMin: 38,
      weakHoldPersistenceMin: 30,
      weakHoldSpreadMax: 1.65,
      weakHoldScoreMin: 34,

      closeQualityFloor: 34,
      closePersistenceFloor: 26,
      closeBreakoutFloor: 24,

      emergencySpread: 2.00,
