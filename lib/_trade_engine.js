...
function buildExecutionDecision({
  ...
  // ENTRY: iets soepeler, omdat scanner al hard filtert
  ...
  // WEAK HOLD: nog steeds ok, maar waarschuwing
  ...
  if (!inPosition) {
    ...
    const watchOk =
      ...
      persistenceOpenOk &&
      depthOk &&
      spread < config.watchSpreadMax &&
      score >= config.watchScoreMin;

    if (watchOk) {
      return finalizeDecision({
        action: "WATCH",
        reason: "Scanner-setup is sterk; wacht op net iets betere timing",
        reasonCode: "watch_timing",
        mode,
        score,
        checklist,
        breakoutReady,
    ...
    return finalizeDecision({
      action: "IGNORE",
      reason: "Timing nog niet klaar voor trade funnel",
      reasonCode: "ignore_not_ready",
      mode,
      score,
      checklist,
      breakoutReady,
  ...
  // Emergency exits gaan altijd voor
  if (emergencyReason) {
    return finalizeDecision({
      action: "CLOSE",
  ...
  // Grace period: niet flippen op normale ruis
  if (graceActive) {
    return finalizeDecision({
      action: "HOLD",
  ...
  // WEAK HOLD: 1-2 zwakke cycles opvangen
  const weakHoldOk =
    tradeCandidate &&
    stageOk &&
    regimeOk &&
