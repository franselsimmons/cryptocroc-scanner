...
export function buildMainExecutionDecision({ coin, btc, regime, mode, coinProfile }) {
  ...
  const qualityOk = entryQuality >= 64; // gewijzigd van 66 naar 64
  checklist.push({
    name: "Entry quality",
    ok: qualityOk,
  ...
  const persistenceOk = persistence >= 56; // gewijzigd van 58 naar 56
  checklist.push({
    name: "Persistence",
    ok: persistenceOk,
...
export function buildMoonExecutionDecision({ coin, btc, regime, mode, coinProfile }) {
  ...
  const entryQuality = n(coin.entryQuality, 0);
  const qualityOk = entryQuality >= 60; // gewijzigd van 62 naar 60
  checklist.push({
    name: "Entry quality",
    ok: qualityOk,
    value: entryQuality.toFixed(1),
  ...
  const persistence = n(coin.persistenceScore, 0);
  const persistenceOk = persistence >= 50; // gewijzigd van 52 naar 50
  checklist.push({
    name: "Persistence",
    ok: persistenceOk,
    value: persistence.toFixed(1),
