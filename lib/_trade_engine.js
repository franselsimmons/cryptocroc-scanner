...
// - PnL: correct voor LONG en SHORT
// - Meta payload consistent voor Discord / analytics

import { calcPnlPct, hitStopOrTp } from "./_moon_core.js";
...
export function buildCoinProfile({ systemType, coin }) {
  ...
  return {
    ...
    name: c.name || "",
    image: c.image || "",
    side: up(c.side || sideFromMode(c.mode)),
    stage: up(c.stage || ""),
...
function buildExecutionDecisionCore({
  ...
  if (!inPosition) {
    ...
    // ✅ This is the key: engine allows scanner to open.
    return {
      action: "ALLOW_ENTRY",
  ...
  // PnL needs entry price. If your position manager stores entryPrice separately,
  // you should pass it into coin.entryPrice or coinProfile.entryPrice.
  const entryPrice = n(coin?.entryPrice, n(coin?.tradePlan?.entry, 0));
  const pnlPct = pnlPctForSide({ entryPrice, lastPrice: last, side: baseMeta.side });

  // Stop/TP hit check (uses your core util)
  // We pass a minimal shape to hitStopOrTp expected fields.
  const stopTp = plan
    ? hitStopOrTp({
        side: baseMeta.side,
        price: last,
  ...
  // We treat "thesis break" as combination of:
  // - breakout lost
  // - persistence drop
  // - ob flips against side
  ...
  // If weakSignals small -> HOLD
  if (weakSignals === 0) {
    return { action: "HOLD", meta: { ...baseMeta, pnlPct, reason: "healthy_hold" } };
  }

  // Weak hold mechanism: allow some weak holds before exit
...
export function buildMainExecutionDecision({
  ...
  // ensure side is always present
  if (!profile.side) profile.side = sideFromMode(mode);
