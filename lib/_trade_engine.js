...
//
// ✅ FIX (belangrijk voor jouw setup):
// 1) Engine accepteert nu ALLE gate-synoniemen die uit scanners kunnen komen:
//    - OPEN / WATCH / IGNORE
//    - ENTRY_READY / TRADE_READY / READY  => OPEN
//    - ENTRY / ENTRYREADY                 => OPEN
//    - ALMOST                              => WATCH
//    - BUILDUP / RADAR                     => IGNORE (maar kan als WATCH gebruikt worden als jij dat ooit wil)
// 2) Engine kan nu óók “execution scoring” meegeven (execution.score) zodat Trade Desk sortering strak is,
//    ook als coin.execution.score ontbreekt.
//
...
// ------------------------------------------------------
// ✅ Gate normalizer (maakt scanners compatibel)
// ------------------------------------------------------
function normalizeGate(gateLike) {
  ...
  // common synonyms that appear in UI/scanners
  if (g === "ENTRY" || g === "ENTRYREADY" || g === "ENTRY_READY") return "OPEN";
  if (g === "TRADE_READY" || g === "TRADEREADY" || g === "READY") return "OPEN";

  // sometimes stage names leak into gate field
  if (g === "ALMOST") return "WATCH";
  if (g === "BUILDUP" || g === "RADAR") return "IGNORE";

  // empty/unknown -> IGNORE
  return "IGNORE";
}

// ------------------------------------------------------
// ✅ Execution score (voor Trade Desk sortering)
// - Prioriteit: perfectCandidateScore (moon) / entryQuality / confidence
// - Bonus: gate OPEN/WATCH
// - Straft slechte spread
// ------------------------------------------------------
...
function buildExecutionDecisionCore({
  ...
  // ✅ normalize gate from ANY source
  ...
  if (!inPosition) {
    ...
    if (cfg.requireBreakoutForEntry && !breakoutOk(profile.breakout, cfg.minBreakoutPressure)) {
      return {
        action: "NO_TRADE",
        score: computeExecutionScore(profile, gate),
        meta: { ...baseMeta, reason: "breakout_not_ready" },
      };
    }
...
export function buildMainExecutionDecision({
  ...
  // ✅ side altijd forceren vanuit scan mode (bull=LONG, bear=SHORT)
