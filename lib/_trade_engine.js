...
export function buildCoinProfile({ systemType, coin }) {
  ...
  return {
    ...
    thresholds: { depthFloorUsd: n(th.depthFloorUsd, 0), depthOk: !!th.depthOk },
    breakout: { ready: !!br.ready, pressure: n(br.pressure, 0), breakoutPct: n(br.breakoutPct, 0) },
    compression: { isCompressed: !!comp.isCompressed, flatPct: n(comp.flatPct, 0) },
...
function buildExecutionDecisionCore({
  ...
  const baseMeta = {
    ...
    obScore,
    tradePlan: plan ? { entry: n(plan.entry, 0), sl: n(plan.sl, 0), tp: n(plan.tp, 0), rr: n(plan.rr, 0), tpPct: n(plan.tpPct, 0), slPct: n(plan.slPct, 0) } : null,
    inPosition,
    cyclesInTrade,
    minHoldCycles,
    partialExitDone,
  ...
  if (!inPosition) {
    if (entryTicketActive) {
      ...
      const cancelBecause = !plan ? "missing_trade_plan" : expired ? "entry_ticket_expired" : cfg.cancelEntryOnBadSpread && spreadPct > cfg.entryTicketMaxSpreadPct ? "spread_too_wide_for_entry" : null;
      if (cancelBecause) {
        return {
          action: "CANCEL_ENTRY",
          score: computeExecutionScore(profile, gate),
          meta: { ...baseMeta, reason: cancelBecause, cancelReason: cancelBecause, cancelEntry: true },
        };
      }
      ...
      if (!triggerReady) {
        return {
          ...
          score: computeExecutionScore(profile, gate),
          meta: { ...baseMeta, reason: "waiting_entry_trigger", keepPinned: true },
        };
      }
      return {
        action: "ALLOW_ENTRY",
        score: computeExecutionScore(profile, gate),
        meta: { ...baseMeta, reason: "watch_trigger_confirmed", triggerReason: "entry_trigger_confirmed" },
      };
    }

    if (gate === "WATCH") {
      if (!plan) return { action: "WATCH", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, reason: "watch_missing_trade_plan" } };
      if (cfg.blockBadSpread && isBadSpread(spreadPct, cfg.watchMaxSpreadPct)) return { action: "WATCH", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, reason: "watch_spread_too_wide" } };
      return {
        action: "ARM_ENTRY",
        score: computeExecutionScore(profile, gate),
        meta: { ...baseMeta, reason: "watch_ready_arm_entry", triggerReason: "scanner_watch_gate_passed", issueEntryTicket: true, entryTicketTtlMs },
      };
    }

    if (gate !== "OPEN") return { action: "NO_TRADE", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, reason: "gate_not_open" } };
    if (!plan) return { action: "NO_TRADE", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, reason: "missing_trade_plan" } };
    if (cfg.blockBadSpread && isBadSpread(spreadPct, cfg.maxSpreadPct)) return { action: "NO_TRADE", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, reason: "spread_too_wide" } };
    return {
      action: "ALLOW_ENTRY",
      score: computeExecutionScore(profile, gate),
      meta: { ...baseMeta, reason: "scanner_open_allow_entry", issueEntryTicket: true, entryTicketTtlMs },
    };
  }
  ...
  const netPnlPct = netPnlPctForSide({ entryPrice, lastPrice: last, side: baseMeta.side, spreadPct, feePct, slipPct });

  if (netPnlPct <= -Math.abs(cfg.emergencyExitNetPct)) {
    return { action: "EXIT", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "emergency_exit", exitReason: "emergency" } };
  }
  if (systemType === "main" && !partialExitDone && netPnlPct >= cfg.tp1NetPct) {
    return { action: "PARTIAL_EXIT", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "tp1_reached", tp1TakePct: cfg.tp1TakePct } };
  }
  const bePrice = entryPrice > 0 ? entryPrice * (1 + Math.abs(cfg.beBufferPct) / 100) : entryPrice;
  const effectiveSl = partialExitDone ? bePrice : n(plan?.sl, 0);
  const stopTp = plan ? hitStopOrTpLocal({ side: baseMeta.side, price: last, entry: n(plan.entry, entryPrice), sl: effectiveSl, tp: n(plan.tp, 0) }) : null;
  if (stopTp?.hit === "SL") {
    return { action: "EXIT", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: partialExitDone ? "breakeven" : "stop_loss", exitReason: partialExitDone ? "breakeven" : "sl", effectiveSl, bePrice } };
  }
  if (stopTp?.hit === "TP") {
    return { action: "EXIT", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "take_profit", exitReason: "tp" } };
  }
  if (cyclesInTrade < minHoldCycles) {
    return { action: "HOLD", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "min_hold_cycles" } };
  }
  const hardBreak = hardBreakDetected({ side: baseMeta.side, obScore, obContraExtremeAbs: cfg.obContraExtremeAbs, spreadPct, maxSpreadPctInTrade: cfg.maxSpreadPctInTrade });
  if (hardBreak.hit) {
    return { action: "EXIT", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: hardBreak.reason === "spread_spike" ? "spread_spike" : "sell_break", exitReason: hardBreak.reason === "spread_spike" ? "spread_spike" : "sell_break", obScore, spreadPct } };
  }
  const breakoutLost = !breakoutOk(profile.breakout, cfg.minBreakoutPressure || 52);
  const mildObAgainst = baseMeta.side === "LONG" ? obScore <= -(cfg.obContraAbs || 0.03) : obScore >= (cfg.obContraAbs || 0.03);
  if (breakoutLost && mildObAgainst && netPnlPct <= n(cfg.hardBreakMaxNetLossPct, -0.9)) {
    return { action: "EXIT", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "sell_break", exitReason: "sell_break", breakoutLost: true, obScore } };
  }
  if (cyclesInTrade >= cfg.timeoutBars && netPnlPct < cfg.timeoutMinNetPnlPct) {
    return { action: "EXIT", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "timeout", exitReason: "timeout" } };
  }
  return { action: "HOLD", score: computeExecutionScore(profile, gate), meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "hold_until_tp_or_sl" } };
}
...
const MAIN_ENGINE_CFG = {
  minHoldCycles: 5, timeoutBars: 12, timeoutMinNetPnlPct: 0.05,
  blockBadSpread: true, maxSpreadPct: 0.9, watchMaxSpreadPct: 1.0, maxSpreadPctInTrade: 1.2,
  requireBreakoutForEntry: false, minBreakoutPressure: 52, maxEntryDistancePct: 1.0, entryMinObScoreAbs: 0.0,
  obContraExtremeAbs: 0.04, obContraAbs: 0.03, emergencyExitNetPct: 2.2,
  feePct: 0.20, slipPct: 0.05, tp1NetPct: 0.60, tp1TakePct: 0.60, beBufferPct: 0.15,
  entryTicketTtlMs: 60 * 60 * 1000, cancelEntryOnBadSpread: true, entryTicketMaxSpreadPct: 1.25,
};

const MOON_ENGINE_CFG = {
  minHoldCycles: 6, timeoutBars: 16, timeoutMinNetPnlPct: 0.35,
  blockBadSpread: true, maxSpreadPct: 1.1, watchMaxSpreadPct: 1.10, maxSpreadPctInTrade: 1.35,
  requireBreakoutForEntry: false, minBreakoutPressure: 52, maxEntryDistancePct: 1.25, entryMinObScoreAbs: 0.0,
  obContraExtremeAbs: 0.05, obContraAbs: 0.03, emergencyExitNetPct: 2.4, hardBreakMaxNetLossPct: -0.9,
  feePct: 0.20, slipPct: 0.05, tp1NetPct: 999, tp1TakePct: 0, beBufferPct: 0.15,
  entryTicketTtlMs: 90 * 60 * 1000, cancelEntryOnBadSpread: true, entryTicketMaxSpreadPct: 1.6,
};

export function buildMainExecutionDecision({ coin, btc, regime, mode, coinProfile, positionState, scannerGate }) {
  const profile = coinProfile || buildCoinProfile({ systemType: "main", coin });
  profile.side = sideFromMode(mode);
  return buildExecutionDecisionCore({ systemType: "main", coin, btc, regime, mode, coinProfile: profile, positionState, scannerGate, cfg: MAIN_ENGINE_CFG });
}

export function buildMoonExecutionDecision({ coin, btc, regime, mode, coinProfile, positionState, scannerGate }) {
  const profile = coinProfile || buildCoinProfile({ systemType: "moon", coin });
  profile.side = sideFromMode(mode);
  return buildExecutionDecisionCore({ systemType: "moon", coin, btc, regime, mode, coinProfile: profile, positionState, scannerGate, cfg: MOON_ENGINE_CFG });
}