...
// ======================================================
// Nieuwe helpers voor slimmer hold/close
// ======================================================
function isWinnerRunning({ coin, mode }) {
  ...
  if (mode === "bull") {
    return (
      (ch1h >= 1.2 || breakoutReady || breakoutPressure >= 68) &&
      persistence >= 52 &&
      obScore >= -0.01
    );
  }
...
function buildExecutionDecision({
  ...
  // Nieuwe runtime signalen
  const winnerRunning = isWinnerRunning({ coin, mode });
  ...
  // ======================================================
  // GEEN POSITIE: ENTRY LOGIC (scannerGate regelt)
  // ======================================================
  if (!inPosition) {
  ...
  // ======================================================
  // BESTAANDE POSITIE: EMERGENCY EXIT (harde risico's)
  // ======================================================
  if (emergencyReason) {
    return finalizeDecision({
      action: "CLOSE",
  ...
  // ======================================================
  // HOLD: normale condities of winnaar-ruimte
  // ======================================================
  const holdOk =
    tradeCandidate &&
    stageOk &&
    ...
    (
      ...
      (
        winnerRunning &&
        spread < config.weakHoldSpreadMax &&
        score >= Math.max(config.weakHoldScoreMin, config.holdScoreMin - 4)
      )
    );
  ...
  // ======================================================
  // WEAK_HOLD: tijdelijke zwakte, winnaars krijgen extra cyclus
  // ======================================================
  const weakHoldOk =
    tradeCandidate &&
    stageOk &&
  ...
  // ======================================================
  // CLOSE: alleen na bevestigde zwakte of harde schade
  // ======================================================
  ...
  // Eerste keer dat softDamage optreedt -> nog geen close, eerst weak hold
  if (softDamage && weakHoldCount === 0 && closeReason !== "thesis_damage_confirmed") {
    return finalizeDecision({
      action: "WEAK_HOLD",
