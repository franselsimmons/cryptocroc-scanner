...
function pickGateLike(scannerGate, coin, profile) {
  return (
    scannerGate ||
    coin?.deskGate ||
    coin?.tradeDeskStatus ||
    coin?.engineGate ||
    coin?.scannerGate ||
    profile?.deskGate ||
    profile?.scannerGate ||
...
// BELANGRIJK:
// nieuwe scannerflow:
// universe -> radar -> warmup -> setup -> entry_ready
// entry_ready moet WATCH zijn voor de trade engine.
// Alleen expliciete OPEN blijft OPEN.
function normalizeGate(gateLike) {
  const g = up(gateLike);

  if (g === "OPEN" || g === "WATCH" || g === "IGNORE") return g;

  // nieuwe scannerflow / aliases -> WATCH
  if (
    g === "ENTRY" ||
    g === "ENTRYREADY" ||
    g === "ENTRY_READY" ||
...
export function buildCoinProfile({ systemType, coin }) {
  ...
  return {
    ...
    image: c.image || "",
    side: up(c.side || ""),
    stage: up(c.pipelineStage || c.stage || ""),
    pipelineStage: up(c.pipelineStage || c.stage || ""),
    stageLegacy: up(c.stageLegacy || ""),
    ...
    engineGate: up(c.engineGate || ""),
    deskGate: up(c.deskGate || c.tradeDeskStatus || ""),
    scannerGate: normalizeGate(rawGate),
  };
}
...
function buildExecutionDecisionCore({
  ...
  const gate = normalizeGate(pickGateLike(scannerGate, coin, profile));
  ...
  const baseMeta = {
    ...
    stage: profile.stage,
    pipelineStage: profile.pipelineStage || profile.stage,
    stageLegacy: profile.stageLegacy || "",
  ...
  if (!inPosition) {
    ...
    // Nieuwe scannerflow: ENTRY_READY komt als WATCH binnen
    if (gate === "WATCH") {
      if (!plan) {
        return {
          action: "WATCH",
