...
function buildExecutionDecisionCore({
  ...
  const tp1TakenPct = Math.max(0, Math.min(1, n(ps.tp1TakenPct, cfg.tp1TakePct))); // fraction already taken
  ...
  if (weakSignals === 0) {
    return { action: "HOLD", meta: { ...baseMeta, pnlPct: grossPnlPct, netPnlPct, reason: "healthy_hold" } };
  }
...
const MOON_ENGINE_CFG = {
  ...
  timeoutMinNetPnlPct: 0.00, // runners can breathe; timeout only if truly dead
