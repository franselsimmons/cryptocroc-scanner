...
export function getDiscordWebhookForSignal({ source, stage, kind }) {
  ...
  // Alleen gesloten trades naar trade-closed
  if (k === "trade_closed") {
    return process.env.DISCORD_WEBHOOK_TRADE_CLOSED || null;
  }

  // Position updates niet naar Discord
  if (k === "position_update") {
    return null;
  }

  // Alleen echte geopende trades naar trade-opened
  if (k === "trade_opened") {
  ...
  if (k === "signal") {
    if (s === "RADAR") {
      if (src === "moon") return process.env.DISCORD_WEBHOOK_RADAR_MOON || null;
