...
async function shouldSendDedupe({ kind, symbol, stage, source }) {
  // Spam preventie: lagere kanalen (RADAR/BUILDUP) krijgen cooldown van 12 uur (43200s)
  // Elite/Alerts houden we op 90s voor updates.
  const isLowTier = ["RADAR", "BUILDUP"].includes(up(stage));
  const expiry = isLowTier ? 43200 : 90; 
  
  const key = `dedupe:${kind}:${symbol}:${up(stage)}:${source}`;
...
export async function sendSignal(payload) {
  ...
  // Spam-filter: In Market Radar (vrij kanaal) alleen CORE doorlaten
  const isRadar = up(stage) === "RADAR";
  if (isRadar && source === "moon") {
     // Alleen sturen naar de specifieke MOON radar als die bestaat, anders blokkeren voor publiek
     if (!process.env.DISCORD_WEBHOOK_RADAR_MOON) return false;
  }
