...
export async function sendSignal(payload) {
  ...
  // Extra check: Geen ALPHA (Moon) in het algemene Market Radar kanaal (spam-preventie)
  if (up(stage) === "RADAR" && source === "moon" && !process.env.DISCORD_WEBHOOK_RADAR_MOON) {
    return false;
  }
