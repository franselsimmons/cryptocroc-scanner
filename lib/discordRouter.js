...
export function getDiscordWebhookForSignal({ source, stage, kind }) {
  ...
  // ELITE PRE-TRADE (WATCH-status coins uit Trade Desk)
  if (k === "elite_watch") {
    return process.env.DISCORD_WEBHOOK_PRE_TRADE || null;
  }

  // Gewone funnel-signalen (alleen RADAR en BUILDUP)
  if (k === "signal") {
    if (src === "main") {
      ...
      return null; // ALMOST en ELITE_* worden niet via "signal" gestuurd
    }
...
function getCooldownSec({ source, stage, kind }) {
  ...
  if (k === "elite_watch") {
    return n(process.env.DISCORD_COOLDOWN_PRE_TRADE_SEC, 10800); // aparte cooldown, default 3 uur
  }

  // Voor gewone signalen (RADAR, BUILDUP) gebruiken we de bestaande configuratie
  if (src === "main") {
    ...
    // ALMOST en ELITE* komen hier niet, maar voor de zekerheid:
    return 7200;
  }
