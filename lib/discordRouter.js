...
// Correct webhook mapping for YOUR existing env vars (screenshots)
// + adds ENTRY/TRADE_READY routing so you get entry notifications again.
...
// ✅ ENTRY-like stages (dit was je missing piece)
...
// --------------------------------
// ENV resolver (use EXACT names from your screenshots)
// --------------------------------
...
export function getDiscordWebhookForSignal({ source, stage, kind }) {
  ...
  // never
  if (k === "portfolio") return envFirst("DISCORD_WEBHOOK_PORTFOLIO"); // if you ever use it
  if (k === "position_update") return null;

  // Elite watch (pre-trade setup channel)
  if (k === "elite_watch") {
    return envFirst("DISCORD_WEBHOOK_PRE_TRADE");
  }
  ...
  // Scannerflow signals
  if (k === "signal") {
    // ✅ ENTRY/TRADE_READY/OPEN => PRO trade-alerts (dit geeft je “entry meldingen” terug)
    if (isEntryStage(s)) {
      return envFirst("DISCORD_WEBHOOK_TRADE_ALERTS");
    }

    // BUILDUP => setup-watch
    if (s === "BUILDUP") {
      return envFirst("DISCORD_WEBHOOK_SETUP_WATCH");
    }

    // ALMOST => high-alert
    if (s === "ALMOST") {
      return envFirst("DISCORD_WEBHOOK_HIGH_ALERT");
    }
    ...
    // RADAR => market-radar (moon optioneel eigen radar webhook)
    if (s === "RADAR") {
      if (src === "moon") {
        return envFirst("DISCORD_WEBHOOK_RADAR_MOON", "DISCORD_WEBHOOK_MARKET_RADAR");
      }
...
function getCooldownSec({ source, stage, kind }) {
  ...
  if (k === "signal") {
    // Entry-like => keep it shorter than almost/buildup typically
    if (isEntryStage(s)) return n(process.env.DISCORD_COOLDOWN_ENTRY_SEC, 1800);

    // PRO elite stages
    if (isMainProStage(src, s) || isMoonProStage(src, s)) {
      return n(process.env.DISCORD_COOLDOWN_PRO_SEC, 3600);
    }
