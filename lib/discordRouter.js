...
// FIXED: correct webhook mapping + MOON-specific vars + color usage + retries + logs
...
// (your screenshots show DISCORD_WEBHOOK_ALMOST etc)
// --------------------------------
...
export function getDiscordWebhookForSignal({ source, stage, kind }) {
  ...
  if (k === "elite_watch") {
    return envFirst(
      "DISCORD_WEBHOOK_PRE_TRADE",
      "DISCORD_WEBHOOK_ELITE" // fallback older
    );
  }
  ...
  // Scannerflow signals
  if (k === "signal") {
    // BUILDUP (FREE/Starter watch)
    if (s === "BUILDUP") {
      if (src === "moon") {
        return envFirst(
          "DISCORD_WEBHOOK_BUILDUP_MOON",     // your env
          "DISCORD_WEBHOOK_SETUP_WATCH"       // current code fallback
        );
      }
      return envFirst(
        "DISCORD_WEBHOOK_BUILDUP",            // your env
        "DISCORD_WEBHOOK_SETUP_WATCH"         // current code fallback
      );
    }

    // ALMOST (Starter high alert)
    if (s === "ALMOST") {
      if (src === "moon") {
        return envFirst(
          "DISCORD_WEBHOOK_ALMOST_MOON",      // your env
          "DISCORD_WEBHOOK_HIGH_ALERT"        // current code fallback
        );
      }
      return envFirst(
        "DISCORD_WEBHOOK_ALMOST",             // your env
        "DISCORD_WEBHOOK_HIGH_ALERT"          // current code fallback
      );
    }
    ...
    if (isMainProStage(src, s)) {
      return envFirst(
        "DISCORD_WEBHOOK_TRADE_ALERTS",       // current code
        "DISCORD_WEBHOOK_ELITE"              // your env (older)
      );
    }
    if (isMoonProStage(src, s)) {
      return envFirst(
        "DISCORD_WEBHOOK_TRADE_ALERTS",       // current code
        "DISCORD_WEBHOOK_ELITE_MOON",         // your env
        "DISCORD_WEBHOOK_ELITE"               // fallback
      );
    }

    // RADAR (Free market radar)
    if (s === "RADAR") {
      if (src === "moon") {
        return envFirst(
          "DISCORD_WEBHOOK_RADAR_MOON",        // current code
          "DISCORD_WEBHOOK_RADAR"              // your env
        );
      }
      return envFirst(
        "DISCORD_WEBHOOK_MARKET_RADAR",        // current code
        "DISCORD_WEBHOOK_RADAR"                // your env
      );
    }
  }
...
export async function sendSignal({
  ...
  // IMPORTANT: stage can be undefined sometimes for trade events
