...
function getDiscordWebhookForSignal({ source, stage, kind }) {
  ...
  if (k === "trade_opened") {
    return (
      process.env.DISCORD_WEBHOOK_TRADE_OPENED ||
  ...
  if (k === "trade_closed") {
    return (
      process.env.DISCORD_WEBHOOK_TRADE_CLOSED ||
      process.env.DISCORD_WEBHOOK_TRADE_ALERTS ||
      process.env.DISCORD_WEBHOOK_HIGH_ALERT
    );
  }
  ...
  // Moon elite / entry / almost -> zelfde alerts als main
  if (src === "moon") {
...
export async function sendSignal(payload) {
  const { source, stage, coin, kind = "signal", pnl, symbol: payloadSymbol } = payload;
  const symbol = up(coin?.symbol || payloadSymbol);
  ...
  const coinWithSymbol = { ...coin, symbol };
  const message = formatSignalMessage({ source, stage, kind, coin: coinWithSymbol, pnl });
