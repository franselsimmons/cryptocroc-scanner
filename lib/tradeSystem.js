...
// CHANGE 2: Increased lock TTL from 25s to 4 minutes
const DURABLE_LOCK_TTL_MS = Number(
  process.env.RUNNER_DURABLE_LOCK_TTL_MS || 4 * 60 * 1000
);
...
// Enige Discord-gate.
// Analyze blijft de source-of-truth voor micro-family berekening.
// A/B/C telt NIET mee voor Discord.

const DISCORD_ALLOWED_FAMILY_IDS = new Set([
  // Zet deze wekelijks handmatig op basis van Analyze.
  "LONG_36",
  "LONG_26",
  "LONG_16",
...
// CHANGE 1: New normalizeSide that recognizes short/sell/long/buy
...
// CHANGE 3: New helper functions for memory key normalization and seeding
...
function normalizeOpenPositionForMemory(pos) {
  ...
  return {
    ...
    discordEntryAllowed: Boolean(pos.discordEntryAllowed ?? pos.discordAllowed),
    discordEntryNotified: Boolean(pos.discordEntryNotified ?? pos.discordNotified),
...
function seedOpenPositionsFromOptions(options = {}) {
  const rows = [
    ...(
      Array.isArray(options.previousOpenPositions)
        ? options.previousOpenPositions
        : []
    ),
    ...(
      Array.isArray(options.openPositions)
  ...
  for (const row of rows) {
    ...
    if (pos.discordEntryNotified) {
      notifyState.set(key, Date.now());
      notifyState.set(buildEntryNotifyKey(pos), Date.now());
    }
  }
...
function buildEntryNotifyKey(pos) {
  const symbol = normalizeBaseSymbol(pos?.symbol);
  const side = normalizeSide(pos?.side);
  const entry = safeNumber(pos?.entry, 0);
...
function buildTradeCandidates(input) {
  ...
  // CHANGE 5: block scanner candidates if symbol already open
  for (const c of dedupeCandidates(accepted)) {
    ...
    if (hasAnyOpenPositionForSymbol(symbol)) {
      continue;
    }
  ...
  // CHANGE 6: replace memory injection with normalized positions
  for (const pos of memory.values()) {
    const normalized = normalizeOpenPositionForMemory(pos);
    if (!normalized) continue;
...
function buildPositionAction(action, reason, c, pos, ctx, extra = {}) {
  ...
  return {
    ...
    quality: pos.quality || null,
    market: pos.market || null,
    timing: pos.timing || null,

    qualityIndex: pos.qualityIndex ?? null,
    marketIndex: pos.marketIndex ?? null,
    timingIndex: pos.timingIndex ?? null,

    definition: pos.definition || null,
    labels: Array.isArray(pos.labels) ? pos.labels : [],
...
// CHANGE 8: Modified finalizeResult to include tradeId and family fields in openPositions output
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    // CHANGE 4: Normalize memory keys and seed from options
    normalizeOpenPositionMemoryKeys();

    const seededOpenPositions = seedOpenPositionsFromOptions(options);

    if (seededOpenPositions > 0) {
      vercelLog("warn", "OPEN_POSITIONS_SEEDED_FROM_LATEST_SCAN", {
        runId,
        seededOpenPositions,
    ...
    for (const raw of candidates) {
      ...
      try {
        ...
        position.discordFamilyId = discordDecision.familyId || null;
        entryPayload.discordFamilyId = discordDecision.familyId || null;
        ...
        // CHANGE 7: Improved Discord entry deduplication using entryNotifyKey
        const entryNotifyKey = buildEntryNotifyKey(position);
        const alreadyNotified =
          Boolean(position.discordEntryNotified) ||
          Boolean(notifyState.get(key)) ||
          Boolean(entryNotifyKey && notifyState.get(entryNotifyKey));
        ...
        if (!notify) {
          ...
          entryPayload.discordDeduped = true;
          vercelLog("info", "DISCORD_ENTRY_DEDUPED", buildTraceSnapshot(c, ctx, {
            reason: "ENTRY_ALREADY_NOTIFIED",
            finalRR,
            setupClass: setup.setupClass,
            entryType: setup.entryType,
            entryNotifyKey,
          ...
          try {
            ...
            if (entryNotifyKey) {
              notifyState.set(entryNotifyKey, Date.now());
            }
            ...
            if (entryNotifyKey) {
              notifyState.delete(entryNotifyKey);
            }
