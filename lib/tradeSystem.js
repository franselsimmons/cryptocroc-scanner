...
// - Final filter setpoints for highest PnL via Vercel logs
// - Runner Hot live-only routing; non-hot scanner tables are shadow-only
...
// ================= STRATEGY VERSION =================
// Niet bumpen tenzij je bewust runtime/KV-history wil resetten.
const STRATEGY_VERSION = "RUNNER_TS_V1_1_FINAL_FILTER_SETPOINTS";
...
// Global sniper mag niet te vroeg blokkeren.
// A/C setup gates blijven sniper beschermen.
...
const RUNNER_B_MIN_CONFLUENCE = 62;
...
// ================= SCANNER TABLE ROUTING =================
// Live trading alleen uit Runner Hot.
// Alles anders wordt shadow-only learning.

const SCANNER_TABLES = {
  RUNNER_HOT: "RUNNER_HOT",
...
const LIVE_SCANNER_TABLES = new Set([
  SCANNER_TABLES.RUNNER_HOT
]);

function normalizeScannerTable(raw) {
  const s = String(raw || "")
    .toUpperCase()
    .trim()
    .replace(/\s+/g, "_")
  ...
  if (!s) return SCANNER_TABLES.UNKNOWN;

  if (
    s === "RUNNER_HOT" ||
    s === "HOT_RUNNER" ||
    s === "HOT" ||
    s.includes("RUNNER_HOT")
  ) {
    return SCANNER_TABLES.RUNNER_HOT;
  }
  ...
  if (
    ...
    s === "SETUP" ||
    s === "SET_UP" ||
    s.includes("RUNNER_SETUP") ||
    s.includes("RUNNER_SET_UP")
  ) {
    return SCANNER_TABLES.RUNNER_SETUP;
  }

  if (s === "ALMOST" || s.includes("ALMOST")) {
    return SCANNER_TABLES.ALMOST;
  }

  if (s === "OPEN_POSITION") {
    return SCANNER_TABLES.OPEN_POSITION;
  }

  return SCANNER_TABLES.UNKNOWN;
}

function getCandidateScannerTable(raw) {
  if (raw?.fromRunnerHot || raw?.runnerHot || raw?.isRunnerHot) {
    return SCANNER_TABLES.RUNNER_HOT;
  }

  if (raw?.fromRunnerSetup || raw?.runnerSetup || raw?.isRunnerSetup) {
    return SCANNER_TABLES.RUNNER_SETUP;
  }

  const explicit = normalizeScannerTable(
    raw?.scannerTable ||
    raw?.scannerBucket ||
    raw?.scannerGroup ||
    raw?.scannerList ||
  ...
  if (explicit !== SCANNER_TABLES.UNKNOWN) return explicit;

  const stage = String(raw?.stage || "").toLowerCase();

  if (stage === "almost") return SCANNER_TABLES.ALMOST;

  return SCANNER_TABLES.UNKNOWN;
}

function decorateScannerCandidate(raw, forcedTable = null) {
  const scannerTable = forcedTable
    ? normalizeScannerTable(forcedTable)
    : getCandidateScannerTable(raw);

  const fromOpenPosition = Boolean(raw?.fromOpenPosition);
  const liveEligible =
    fromOpenPosition ||
    LIVE_SCANNER_TABLES.has(scannerTable);

  return {
    ...raw,
    scannerTable,
    liveEligible,
    shadowOnly: !liveEligible
  };
}
...
function tagCandidates(list, scannerTable) {
  return asArray(list).map(c => decorateScannerCandidate(c, scannerTable));
}

function extractTradeCandidates(input, defaultScannerTable = null) {
  if (Array.isArray(input)) {
    return input.map(c => {
      ...
      return decorateScannerCandidate(
        c,
        hasExplicitTable ? null : defaultScannerTable
      );
    });
  }

  const runnerHot = [
    ...tagCandidates(input?.tradeFunnel?.runnerHot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.tradeFunnel?.runner_hot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.tradeFunnel?.RUNNER_HOT, SCANNER_TABLES.RUNNER_HOT),

    ...tagCandidates(input?.runnerHot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.runner_hot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.RUNNER_HOT, SCANNER_TABLES.RUNNER_HOT),

    ...tagCandidates(input?.funnel?.runnerHot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.funnel?.runner_hot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.funnel?.RUNNER_HOT, SCANNER_TABLES.RUNNER_HOT),

    ...tagCandidates(input?.funnel?.bull?.runnerHot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.funnel?.bull?.runner_hot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.funnel?.bear?.runnerHot, SCANNER_TABLES.RUNNER_HOT),
    ...tagCandidates(input?.funnel?.bear?.runner_hot, SCANNER_TABLES.RUNNER_HOT)
  ];

  const runnerSetup = [
    ...tagCandidates(input?.tradeFunnel?.runnerSetup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.tradeFunnel?.runner_setup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.tradeFunnel?.RUNNER_SETUP, SCANNER_TABLES.RUNNER_SETUP),

    ...tagCandidates(input?.runnerSetup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.runner_setup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.RUNNER_SETUP, SCANNER_TABLES.RUNNER_SETUP),

    ...tagCandidates(input?.funnel?.runnerSetup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.funnel?.runner_setup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.funnel?.RUNNER_SETUP, SCANNER_TABLES.RUNNER_SETUP),

    ...tagCandidates(input?.funnel?.bull?.runnerSetup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.funnel?.bull?.runner_setup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.funnel?.bear?.runnerSetup, SCANNER_TABLES.RUNNER_SETUP),
    ...tagCandidates(input?.funnel?.bear?.runner_setup, SCANNER_TABLES.RUNNER_SETUP)
  ];

  const legacyUnknown = [
    ...tagCandidates(input?.funnel?.bull?.entry, SCANNER_TABLES.UNKNOWN),
    ...tagCandidates(input?.funnel?.bear?.entry, SCANNER_TABLES.UNKNOWN),
    ...tagCandidates(input?.funnel?.bull?.almost, SCANNER_TABLES.ALMOST),
    ...tagCandidates(input?.funnel?.bear?.almost, SCANNER_TABLES.ALMOST)
  ];
...
function dedupeCandidates(list) {
  ...
  for (const raw of Array.isArray(list) ? list : []) {
    ...
    const decorated = decorateScannerCandidate(raw);
    ...
    const normalized = {
      ...
      scannerTable: normalizeScannerTable(decorated.scannerTable),
      liveEligible: Boolean(decorated.liveEligible),
      shadowOnly: Boolean(decorated.shadowOnly)
    };
    ...
    if (normalized.liveEligible !== prev.liveEligible) {
      if (normalized.liveEligible) {
        map.set(key, normalized);
      }
...
function buildTradeCandidates(input) {
  ...
  for (const c of raw) {
    ...
    accepted.push(decorateScannerCandidate(c));
  }
  ...
  for (const [key, pos] of memory.entries()) {
    ...
    map.set(key, {
      ...
      stageSource: "memory",
      scannerTable: SCANNER_TABLES.OPEN_POSITION,
      liveEligible: true,
      shadowOnly: false,
      uiOnly: false,
  ...
  prefilter.liveRunnerHotCount = candidates.filter(c => c.liveEligible).length;
  prefilter.shadowOnlyCount = candidates.filter(c => c.shadowOnly).length;
...
function buildCommonPayload(c, ctx = {}) {
  ...
  return {
    ...
    scannerTable: normalizeScannerTable(c?.scannerTable),
    liveEligible: Boolean(c?.liveEligible || c?.fromOpenPosition),
    shadowOnly: Boolean(c?.shadowOnly && !c?.fromOpenPosition),
...
function buildWait(c, reason, ctx = {}) {
  ...
  const payload = {
    ...
    setupClass: ctx.setupClass ?? undefined,
    entryType: ctx.entryType ?? undefined,
    runnerEntryType: ctx.runnerEntryType ?? ctx.entryType ?? undefined,
    shadowOnly: Boolean(c?.shadowOnly || ctx.shadowOnly),
    liveEligible: Boolean(c?.liveEligible && !c?.shadowOnly),
...
async function buildShadowOnlyRunnerCandidate({
  c,
  ctx,
  ob,
  liquidity,
  ...
  if (!safeNumber(c.price, 0)) {
    return buildWait(c, "SHADOW_PRICE_INVALID", {
      ...ctx,
      shadowOnly: true
    });
  }
  ...
  if (ob.fetchFailed) {
    return buildWait(c, "SHADOW_ORDERBOOK_FETCH_FAILED", {
      ...ctx,
      shadowOnly: true
    });
  }
  ...
  if (ob.spoof) {
    return buildWait(c, "SHADOW_SPOOF_DETECTED", {
      ...ctx,
      shadowOnly: true
    });
  }
  ...
  try {
    ...
    vercelError("SHADOW_RISK_FAILED", err, {
      symbol: c.symbol,
      side: c.side,
      scannerTable: c.scannerTable
    });

    return buildWait(c, "SHADOW_RISK_FAILED", {
      ...ctx,
      shadowOnly: true
    });
  }
  ...
  if (!riskBase || !safeNumber(riskBase.sl, 0)) {
    return buildWait(c, "SHADOW_RISK_INVALID", {
      ...ctx,
      shadowOnly: true,
      risk: riskBase
    });
  ...
  const targetR = setup?.targetR || RUNNER_B_TARGET_R;
  const targets = buildRunnerTargetsFromRisk(c, riskBase, targetR);
  ...
  return buildWait(c, "SHADOW_ONLY_NOT_RUNNER_HOT", {
    ...ctx,
    shadowOnly: true,
    rr: finalRR || baseRR,
    risk: {
...
function optNormalizeOutcomeRow(row) {
  ...
  const normalized = {
    ...
    scannerTable: normalizeScannerTable(row?.scannerTable),
...
// Doel:
// - winnaars/verliezers herkennen per setup-combinatie
// - na 1 dag harde blocklist / allowlist uit Vercel logs halen
// - Discord-signalen terugbrengen naar high-quality runner setups
...
function buildCohortOutcomeLog(row) {
  ...
  return {
    ...
    scannerTable: normalized.scannerTable,
...
function buildFilterSetpointsFromBest(best) {
  ...
  return {
    ...
    constants: {
      ...
      MIN_DEPTH_USD_1P: f.MIN_DEPTH_USD_1P,
      SQUEEZE_MIN_DEPTH_USD_1P: f.MIN_DEPTH_USD_1P,
    ...
    codePatch: [
      ...
      `const MIN_DEPTH_USD_1P = ${f.MIN_DEPTH_USD_1P};`,
      `const SQUEEZE_MIN_DEPTH_USD_1P = ${f.MIN_DEPTH_USD_1P};`,
...
function maybeLogOptimizerReport(force = false) {
  ...
  const decision = buildFinalRunnerFilterDecision();
  const cohortLearning = buildCohortLearningReport();

  stats.lastOptimizerReportAt = now;

  console.log("RUNNER_OPTIMIZER_REPORT", JSON.stringify(report));
  console.log("RUNNER_FINAL_FILTER_DECISION", JSON.stringify(decision));

  console.log("RUNNER_COHORT_LEARNING_REPORT", JSON.stringify({
    tag: "RUNNER_COHORT_LEARNING_REPORT",
    strategyVersion: STRATEGY_VERSION,
    ts: Date.now(),
    sample: cohortLearning.sample,
    summary: cohortLearning.summary
  }));

  console.log("RUNNER_COHORT_ALLOWLIST_CANDIDATES", JSON.stringify({
    tag: "RUNNER_COHORT_ALLOWLIST_CANDIDATES",
    strategyVersion: STRATEGY_VERSION,
    ts: Date.now(),
    rows: cohortLearning.allowCandidates
  }));

  console.log("RUNNER_COHORT_BLOCKLIST_CANDIDATES", JSON.stringify({
    tag: "RUNNER_COHORT_BLOCKLIST_CANDIDATES",
    strategyVersion: STRATEGY_VERSION,
    ts: Date.now(),
    rows: cohortLearning.blockCandidates
  }));

  console.log("RUNNER_COHORT_WATCH_BLOCK_CANDIDATES", JSON.stringify({
    tag: "RUNNER_COHORT_WATCH_BLOCK_CANDIDATES",
    strategyVersion: STRATEGY_VERSION,
    ts: Date.now(),
    rows: cohortLearning.watchBlockCandidates
  }));

  console.log("RUNNER_COHORT_CODE_PATCH", JSON.stringify({
    tag: "RUNNER_COHORT_CODE_PATCH",
    strategyVersion: STRATEGY_VERSION,
    ts: Date.now(),
    codePatch: cohortLearning.codePatch
  }));

  if (decision?.setpoints) {
    console.log("RUNNER_FILTER_SETPOINTS", JSON.stringify({
      tag: "RUNNER_FILTER_SETPOINTS",
      strategyVersion: STRATEGY_VERSION,
      ts: Date.now(),
      decision: decision.decision,
      sample: decision.sample,
      deltaVsCurrent: decision.deltaVsCurrent || null,
      setpoints: decision.setpoints
    }));
  }

  if (Array.isArray(decision?.bestCohorts)) {
    console.log("RUNNER_BEST_COHORTS", JSON.stringify({
      tag: "RUNNER_BEST_COHORTS",
      strategyVersion: STRATEGY_VERSION,
      ts: Date.now(),
      rows: decision.bestCohorts.slice(0, 12)
    }));
  }

  if (Array.isArray(decision?.badCohorts)) {
    console.log("RUNNER_BAD_COHORTS", JSON.stringify({
      tag: "RUNNER_BAD_COHORTS",
      strategyVersion: STRATEGY_VERSION,
      ts: Date.now(),
      rows: decision.badCohorts.slice(0, 12)
    }));
  }
...
function finalizeResult(actions, candidates, btcState, runId, startedAt) {
  ...
  vercelLog("info", "RUN_DONE", {
    ...
    liveRunnerHotCount: candidates.filter(c => c.liveEligible).length,
    shadowOnlyCount: candidates.filter(c => c.shadowOnly).length,
  ...
  return {
    ...
    openPositions: Array.from(memory.values()).map(pos => ({
      ...
      setupClass: pos.setupClass,
      entryType: pos.entryType,
      scannerTable: normalizeScannerTable(pos.scannerTable),
      entry: pos.entry,
      sl: pos.sl,
      initialSl: pos.initialSl,
      tp: pos.tp,
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    const defaultScannerTable =
      options.scannerTable ||
      options.defaultScannerTable ||
      null;

    const rawCandidates = extractTradeCandidates(input, defaultScannerTable);
    ...
    for (const raw of candidates) {
      const c = {
        ...
        moveScore: safeNumber(raw.moveScore ?? raw.score, 0),
        scannerTable: normalizeScannerTable(raw.scannerTable),
      ...
      if (c.shadowOnly && !c.fromOpenPosition) {
        const payload = await buildShadowOnlyRunnerCandidate({
          c,
          ctx,
          ob,
          liquidity,
...
export function forceRunnerOptimizerLog() {
  return maybeLogOptimizerReport(true);
}
