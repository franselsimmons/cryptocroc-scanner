...
function isDiscordEntryAllowed({
  c,
  ctx,
  ob,
  setup,
  ...
  return DISCORD_ENTRY_FILTERS.some(filter => {
    if (!filter?.enabled) return false;
    if (normalizeSide(filter.side) !== side) return false;

    if (score < safeNumber(filter.minScore, 0)) return false;
    if (confluence < safeNumber(filter.minConfluence, 0)) return false;
    if (sniperScore < safeNumber(filter.minSniperScore, 0)) return false;
    if (rr < safeNumber(filter.minRR, 0)) return false;

    if (!listAllows(flow, filter.flows)) return false;
    if (!listAllows(rsiZone, filter.rsiZones)) return false;
    if (!listAllows(obBias, filter.obBiases)) return false;

    if (spreadPct > safeNumber(filter.maxSpreadPct, 0.05)) return false;
    if (depthMinUsd1p < safeNumber(filter.minDepthUsd1p, 0)) return false;

    if (runnerPressure < safeNumber(filter.minRunnerPressure, -999)) return false;
    if (runnerAcceleration < safeNumber(filter.minRunnerAcceleration, -999)) return false;
  ...
  try {
    ...
    for (const raw of candidates) {
      ...
      try {
        ...
        const discordAllowed = isDiscordEntryAllowed({
          c,
          ctx,
          ob,
          setup,
        ...
        position.discordEntryBlocked = !discordAllowed;

        if (notify && discordAllowed && !notifyState.get(key)) {
          await sendEntry({
            symbol: c.symbol,
            side: c.side,
            grade: setup.setupClass === "RUNNER_B" ? "B" : "A",
