...
// ================= STRATEGY VERSION =================
// Nu met universele optimizer die alle completed real+shadow rows gebruikt.
const STRATEGY_VERSION = "RUNNER_TS_V1_7_MAIN_STYLE_ULTRA_LOOSE";
...
// ================= RUNNER CONSTANTS =================
// MAIN-style: extreem los.
// Doel: zoveel mogelijk real + shadow outcome data verzamelen.
// Optimizer bepaalt later de harde live floors.

const MIN_SCORE = 0;
const MIN_TF_STRENGTH = 0;
const MIN_CONFLUENCE = 0;
const MIN_SNIPER_SCORE = 0;

const MAX_SPREAD_PCT = 0.05;
const SQUEEZE_MAX_SPREAD_PCT = 0.05;

const MIN_DEPTH_USD_1P = 0;
const SQUEEZE_MIN_DEPTH_USD_1P = 0;

const MIN_RUNNER_PRESSURE = -999;
const MIN_RUNNER_ACCELERATION = -999;

const RUNNER_A_MIN_CONFLUENCE = 0;
const RUNNER_A_MIN_SNIPER = 0;
const RUNNER_A_MIN_RR = 0.01;

const RUNNER_B_MIN_CONFLUENCE = 0;
const RUNNER_B_MIN_SNIPER = 0;
const RUNNER_B_MIN_RR = 0.01;

const RUNNER_C_MIN_CONFLUENCE = 0;
const RUNNER_C_MIN_SNIPER = 0;
const RUNNER_C_MIN_RR = 0.01;
...
// Main-style: geen hard behavioral blocks.
// Cohorts moeten eerst data krijgen.
const BLOCK_ENTRY_TYPES = new Set([]);

const WATCH_MID_RUNNING_NEUTRAL_MIN_CONFLUENCE = -999;
const WATCH_MID_RUNNING_NEUTRAL_MIN_SNIPER = -999;
const WATCH_MID_RUNNING_NEUTRAL_MIN_PRESSURE = -999;
...
const RUNNER_C_MIN_SCORE = 0;
const RUNNER_A_MIN_SCORE = 0;
const RUNNER_B_MIN_SCORE = 0;

const STRUCTURE_AGAINST_MIN_CONFLUENCE = -999;

const SPREAD_EXCEPTION_MIN_CONFLUENCE = -999;
const DEPTH_EXCEPTION_MIN_CONFLUENCE = -999;
const OB_AGAINST_MIN_CONFLUENCE = -999;

const FUNDING_EXTREME_ABS_MAX = 999;
const LONG_CROWDED_FUNDING_MAX = 999;
const SHORT_CROWDED_FUNDING_MIN = -999;
const FUNDING_MIN_CONFLUENCE = -999;

const BTC_STRONG_COUNTER_MIN_CONFLUENCE = -999;
const STRATEGY_SAFE_MIN_CONFLUENCE = -999;

const RSI_CONTINUATION_TEST_RR = 0.01;
...
// ================= RUNTIME GUARDS / TRACE =================
// In jouw geplakte bestand ontbreken deze guards. Zonder deze kan entry/exit path crashen.
...
const EXIT_NOTIFY_DEDUP_TTL_MS = Number(process.env.RUNNER_EXIT_NOTIFY_DEDUP_TTL_MS || 6 * 60 * 60 * 1000);

const DURABLE_LOCK_ATTEMPTS = Number(process.env.RUNNER_DURABLE_LOCK_ATTEMPTS || 3);
const DURABLE_LOCK_TTL_MS = Number(process.env.RUNNER_DURABLE_LOCK_TTL_MS || 25 * 1000);
const DURABLE_LOCK_RETRY_MS = Number(process.env.RUNNER_DURABLE_LOCK_RETRY_MS || 250);

// Zet RUNNER_TRACE=true in Vercel om te zien waar hij blijft hangen.
const RUNNER_TRACE = String(process.env.RUNNER_TRACE || "false").toLowerCase() === "true";
const RUNNER_TRACE_WAITS = String(process.env.RUNNER_TRACE_WAITS || process.env.RUNNER_TRACE || "false").toLowerCase() === "true";
...
// Main-style: geen scanner-hot live choke.
const LIVE_ONLY_HOT_RUNNER = false;
...
// ================= DISCORD ENTRY FILTER =================
// Exacte handmatige micro-family Discord gate.
// TradeSystem blijft alles analyseren/loggen/shadowen.
// Discord gaat ALLEEN af als één complete filter-family exact matcht.

const DISCORD_ENTRY_FILTERS = Object.freeze([
  {
    name: "LONG_36_Q4_M3_T2",
    enabled: true,

    side: "bull",

    setupClasses: ["RUNNER_B"],
    entryTypes: ["RUNNER_B_CONTINUATION"],

    stages: ["entry", "almost"],
    flows: ["TREND", "BUILDING"],
    rsiZones: ["LOWER_1", "LOWER_2", "LOWER_3", "MID"],
    obBiases: ["NEUTRAL"],
    ...
    // 8-16 bps = 0.0008 - 0.0016
    minSpreadPct: 0.0008,
    maxSpreadPct: 0.0016,

    minDepthUsd1p: 50000,
    maxDepthUsd1p: 100000,

    tfAlignedOnly: true,

    minRunnerPressure: -999,
    maxRunnerPressure: 999,

    minRunnerAcceleration: -999,
    maxRunnerAcceleration: 999
  },

  {
    name: "SHORT_26_Q3_M3_T2",
    enabled: true,
    ...
    stages: ["entry", "almost"],
    flows: ["TREND", "BUILDING"],
    rsiZones: ["UPPER_1", "UPPER_2", "UPPER_3", "MID"],
    obBiases: ["NEUTRAL"],
...
function listAllows(value, allowedList) {
...
function rangeAllows(value, min, max) {
  const n = safeNumber(value, 0);

  if (Number.isFinite(Number(min)) && n < Number(min)) return false;
  if (Number.isFinite(Number(max)) && n > Number(max)) return false;

  return true;
}

function getRangeFailReason(value, min, max, lowReason, highReason) {
  const n = safeNumber(value, 0);

  if (Number.isFinite(Number(min)) && n < Number(min)) return lowReason;
  if (Number.isFinite(Number(max)) && n > Number(max)) return highReason;

  return null;
}

function isTfAlignedForDiscord(tfAlignment, side) {
  const a = String(tfAlignment || "UNKNOWN").toUpperCase();
...
function getDiscordFilterFailure(filter, data) {
  if (!filter?.enabled) {
    return {
      reason: "DISCORD_FILTER_DISABLED",
      filter: filter?.name || "UNKNOWN"
    };
  }

  if (normalizeSide(filter.side) !== data.side) {
    return {
      reason: "DISCORD_SIDE_MISMATCH",
      filter: filter.name,
      side: data.side,
      requiredSide: normalizeSide(filter.side)
    };
  }

  if (!listAllows(data.setupClass, filter.setupClasses)) {
    return {
      reason: "DISCORD_SETUP_CLASS_BLOCKED",
      filter: filter.name,
      setupClass: data.setupClass,
      allowedSetupClasses: filter.setupClasses
    };
  }

  if (!listAllows(data.entryType, filter.entryTypes)) {
    return {
      reason: "DISCORD_ENTRY_TYPE_BLOCKED",
      filter: filter.name,
      entryType: data.entryType,
      allowedEntryTypes: filter.entryTypes
    };
  }

  if (!listAllows(data.stage, filter.stages)) {
    return {
      reason: "DISCORD_STAGE_BLOCKED",
      filter: filter.name,
      stage: data.stage,
      allowedStages: filter.stages
    };
  }

  if (!listAllows(data.flow, filter.flows)) {
    return {
      reason: "DISCORD_FLOW_BLOCKED",
      filter: filter.name,
      flow: data.flow,
      allowedFlows: filter.flows
    };
  }

  if (!listAllows(data.rsiZone, filter.rsiZones)) {
    return {
      reason: "DISCORD_RSI_ZONE_BLOCKED",
      filter: filter.name,
      rsiZone: data.rsiZone,
      allowedRsiZones: filter.rsiZones
    };
  }

  if (!listAllows(data.obBias, filter.obBiases)) {
    return {
      reason: "DISCORD_OB_BIAS_BLOCKED",
      filter: filter.name,
      obBias: data.obBias,
      allowedObBiases: filter.obBiases
    };
  }

  const scoreFail = getRangeFailReason(
    data.score,
    filter.minScore,
    filter.maxScore,
    "DISCORD_SCORE_TOO_LOW",
    "DISCORD_SCORE_TOO_HIGH"
  );
  ...
  const confluenceFail = getRangeFailReason(
    data.confluence,
    filter.minConfluence,
    filter.maxConfluence,
    "DISCORD_CONFLUENCE_TOO_LOW",
    "DISCORD_CONFLUENCE_TOO_HIGH"
  );

  if (confluenceFail) {
    return {
      reason: confluenceFail,
      filter: filter.name,
      confluence: data.confluence,
      minConfluence: filter.minConfluence,
      maxConfluence: filter.maxConfluence
    };
  }

  const sniperFail = getRangeFailReason(
    data.sniperScore,
    filter.minSniperScore,
    filter.maxSniperScore,
    "DISCORD_SNIPER_TOO_LOW",
    "DISCORD_SNIPER_TOO_HIGH"
  );
  ...
  if (sniperFail) {
    return {
      reason: sniperFail,
      filter: filter.name,
      sniperScore: data.sniperScore,
      minSniperScore: filter.minSniperScore,
      maxSniperScore: filter.maxSniperScore
    };
  }

  const rrFail = getRangeFailReason(
    data.rr,
    filter.minRR,
    filter.maxRR,
    "DISCORD_RR_TOO_LOW",
  ...
  const spreadFail = getRangeFailReason(
    data.spreadPct,
    filter.minSpreadPct,
    filter.maxSpreadPct,
    "DISCORD_SPREAD_TOO_LOW",
    "DISCORD_SPREAD_TOO_HIGH"
  );
  ...
  if (spreadFail) {
    return {
      ...
      minSpreadPct: filter.minSpreadPct,
      maxSpreadPct: filter.maxSpreadPct
    };
  }

  const depthFail = getRangeFailReason(
    data.depthMinUsd1p,
    filter.minDepthUsd1p,
    filter.maxDepthUsd1p,
    "DISCORD_DEPTH_TOO_LOW",
    "DISCORD_DEPTH_TOO_HIGH"
  );
  ...
  if (depthFail) {
    return {
      reason: depthFail,
      filter: filter.name,
      depthMinUsd1p: data.depthMinUsd1p,
      minDepthUsd1p: filter.minDepthUsd1p,
      maxDepthUsd1p: filter.maxDepthUsd1p
    };
  }

  const pressureFail = getRangeFailReason(
    data.runnerPressure,
    filter.minRunnerPressure,
    filter.maxRunnerPressure,
    "DISCORD_PRESSURE_TOO_LOW",
    "DISCORD_PRESSURE_TOO_HIGH"
  );
  ...
  if (pressureFail) {
    return {
      reason: pressureFail,
      filter: filter.name,
      runnerPressure: data.runnerPressure,
      minRunnerPressure: filter.minRunnerPressure,
      maxRunnerPressure: filter.maxRunnerPressure
    };
  }

  const accelerationFail = getRangeFailReason(
    data.runnerAcceleration,
    filter.minRunnerAcceleration,
    filter.maxRunnerAcceleration,
    "DISCORD_ACCELERATION_TOO_LOW",
    "DISCORD_ACCELERATION_TOO_HIGH"
  );

  if (accelerationFail) {
    return {
      reason: accelerationFail,
      filter: filter.name,
      runnerAcceleration: data.runnerAcceleration,
      minRunnerAcceleration: filter.minRunnerAcceleration,
      maxRunnerAcceleration: filter.maxRunnerAcceleration
    };
  }

  if (filter.tfAlignedOnly && !isTfAlignedForDiscord(data.tfAlignment, data.side)) {
    return {
      reason: "DISCORD_TF_NOT_ALIGNED",
      filter: filter.name,
      tfAlignment: data.tfAlignment
    };
  }
...
function getDiscordEntryDecision({
  c,
  ctx,
  ob,
  setup,
  ...
  const stage = String(c?.stage || "unknown").toLowerCase();

  const setupClass = String(setup?.setupClass || "UNKNOWN").toUpperCase();
  const entryType = String(setup?.entryType || setup?.runnerEntryType || "UNKNOWN").toUpperCase();
  ...
  const data = {
    ...
    rr: safeNumber(finalRR, 0),

    flow: normalizeFlow(ctx?.flow?.type || c?.flow || c?.scannerFlow),
    rsiZone: String(ctx?.rsiZone || "UNKNOWN").toUpperCase(),
    obBias: String(ob?.bias || "NEUTRAL").toUpperCase(),
    ...
    tfAlignment: String(c?.tfAlignment || ctx?.tfAlignment || "UNKNOWN").toUpperCase()
  };

  const failures = [];

  for (const filter of DISCORD_ENTRY_FILTERS) {
    const failure = getDiscordFilterFailure(filter, data);

    if (!failure) {
      return {
        allowed: true,
        reason: "DISCORD_MICRO_FAMILY_MATCH",
        filter: filter.name,
        matched: {
          ...data
        }
  ...
  return {
    allowed: false,
    reason: "DISCORD_NO_MICRO_FAMILY_MATCH",
    checkedFilters: failures.slice(0, 10),
    candidate: {
      ...data
    }
  };
...
function isDiscordEntryAllowed(args) {
  return getDiscordEntryDecision(args).allowed;
}
...
function getLiveFunnelBlockReason() {
  return null;
}
...
function buildWait(c, reason, ctx = {}) {
  ...
  try {
    ...
    vercelError("WAIT_FEATURE_ROW_FAILED", err, {
  ...
  try {
    ...
    vercelError("WAIT_SHADOW_CREATE_FAILED", err, {
      symbol: payload.symbol,
      side: payload.side,
      reason: payload.reason
    });
...
function classifyRunnerSetup({
  ...
  const plannedRR = safeNumber(rr, 0);
...
function getBadLiveCohortReason() {
  return null;
}
...
// ================= FINAL FILTER SETPOINT OPTIMIZER (legacy, niet gebruikt in master) =================
...
// ================= NIEUWE MASTER OPTIMIZER (UNIVERSEEL) =================
// Bouwt een universum van alle completed real + shadow rows.
// Per preset wordt gesimuleerd of de trade geopend zou zijn.
// Daarna worden statistieken berekend op basis van de werkelijke uitkomst.

function isCompletedOutcomeRow(row) {
  if (!row) return false;
  ...
  if (Number(row.entry || 0) <= 0) return false;
  if (Number(row.sl || row.initialSl || 0) <= 0) return false;
  if (Number(row.tp || 0) <= 0) return false;

  return true;
}

function normalizeOptimizerRow(row, source = "UNKNOWN") {
  return {
    source: String(row.source || source).toUpperCase(),

    symbol: String(row.symbol || "").toUpperCase(),
    side: String(row.side || "").toLowerCase() === "bear" ? "bear" : "bull",

    stage: String(row.stage || "entry").toLowerCase(),

    score: Number(row.score || row.moveScore || 0),
    confluence: Number(row.confluence || 0),
    sniperScore: Number(row.sniperScore || row.sniper || 0),

    rr: Number(row.rr || row.plannedRR || row.targetR || 0),
    plannedRR: Number(row.plannedRR || row.rr || row.targetR || 0),

    entry: Number(row.entry || 0),
    sl: Number(row.sl || row.initialSl || 0),
    tp: Number(row.tp || 0),

    exitR: Number(row.exitR),
    pnlPct: Number(row.pnlPct),

    mfeR: Number(row.mfeR || 0),
    maeR: Number(row.maeR || 0),

    flow: String(row.flow || row.scannerFlow || "UNKNOWN").toUpperCase(),
    scannerFlow: String(row.scannerFlow || row.flow || "UNKNOWN").toUpperCase(),

    rsiZone: String(row.rsiZone || "UNKNOWN").toUpperCase(),
    obBias: String(row.obBias || "UNKNOWN").toUpperCase(),

    spreadPct: Number(row.spreadPct || 0),
    depthMinUsd1p: Number(row.depthMinUsd1p || 0),

    runnerPressure: Number(row.runnerPressure || 0),
    runnerAcceleration: Number(row.runnerAcceleration || 0),
...
function buildOptimizerUniverse() {
  const realRows = (stats.closedTrades || [])
    .map(row => normalizeOptimizerRow(row, "REAL"))
    .filter(isCompletedOutcomeRow);

  const shadowRows = (stats.shadowRows || [])
    .filter(row => row?.status && row.status !== "OPEN")
    .map(row => normalizeOptimizerRow(row, "SHADOW"))
    .filter(isCompletedOutcomeRow);

  // Ook scan shadow rows zitten al in stats.shadowRows met source "SCAN_SHADOW"
  const allRows = [...realRows, ...shadowRows];

  // Filter alleen rows met minimale data
  return allRows
    .filter(row => row.score > 0)
    .filter(row => row.confluence >= 0)
    .filter(row => row.sniperScore >= 0)
    .slice(-RUNNER_MASTER_MAX_ROWS);
}

function inferRunnerSetupFromPreset(row, preset) {
  const flow = String(row.flow || row.scannerFlow || "UNKNOWN").toUpperCase();
  ...
  if (
    flow === "SQUEEZE" &&
    score >= preset.RUNNER_C_MIN_SCORE &&
    confluence >= preset.RUNNER_C_MIN_CONFLUENCE &&
    sniperScore >= preset.RUNNER_C_MIN_SNIPER &&
    rr >= preset.RUNNER_C_MIN_RR
  ) {
    return {
      setupClass: "RUNNER_C",
      liveGrade: "A",
  ...
  if (
    ...
    score >= preset.RUNNER_A_MIN_SCORE &&
    confluence >= preset.RUNNER_A_MIN_CONFLUENCE &&
    sniperScore >= preset.RUNNER_A_MIN_SNIPER &&
    rr >= preset.RUNNER_A_MIN_RR
  ) {
    return {
      setupClass: "RUNNER_A",
      liveGrade: "A",
  ...
  if (
    ["SQUEEZE", "RUNNING", "BREAKOUT", "BUILDING", "TREND"].includes(flow) &&
    score >= preset.RUNNER_B_MIN_SCORE &&
    confluence >= preset.RUNNER_B_MIN_CONFLUENCE &&
    sniperScore >= preset.RUNNER_B_MIN_SNIPER &&
    rr >= preset.RUNNER_B_MIN_RR
  ) {
    return {
      setupClass: "RUNNER_B",
      liveGrade: "B",
...
function wouldEnterUnderPreset(row, preset) {
  if (!row) return false;
  ...
  // Basis scores
  if (row.score < preset.MIN_SCORE) return false;
  if (row.confluence < preset.MIN_CONFLUENCE) return false;
  if (row.sniperScore < preset.MIN_SNIPER_SCORE) return false;

  if (row.runnerPressure < preset.MIN_RUNNER_PRESSURE) return false;
  if (row.runnerAcceleration < preset.MIN_RUNNER_ACCELERATION) return false;

  // Hot runner only
  if (preset.LIVE_ONLY_HOT_RUNNER) {
    const hotFlows = new Set(preset.LIVE_SCANNER_HOT_FLOWS || []);
    if (!hotFlows.has(String(row.scannerFlow || flow).toUpperCase())) return false;
  }

  // Flow moet runner zijn
  ...
  if (row.spreadPct > 0) {
    const maxSpread = flow === "SQUEEZE"
      ? preset.SQUEEZE_MAX_SPREAD_PCT
      : preset.MAX_SPREAD_PCT;

    if (row.spreadPct > maxSpread && row.confluence < preset.SPREAD_EXCEPTION_MIN_CONFLUENCE) {
      return false;
    }
  }

  if (row.depthMinUsd1p > 0) {
    const minDepth = flow === "SQUEEZE"
      ? preset.SQUEEZE_MIN_DEPTH_USD_1P
      : preset.MIN_DEPTH_USD_1P;

    if (row.depthMinUsd1p < minDepth && row.confluence < preset.DEPTH_EXCEPTION_MIN_CONFLUENCE) {
      return false;
    }
  }

  // RSI zone filter (geen bear entries in lower zones)
  if (
    row.side === "bear" &&
  ...
  const inferred = inferRunnerSetupFromPreset(row, preset);

  if (inferred.setupClass === "NONE") return false;

  if (
    Array.isArray(preset.BLOCK_ENTRY_TYPES) &&
    preset.BLOCK_ENTRY_TYPES.includes(inferred.entryType)
  ) {
    return false;
  }
...
function evaluatePresetOnAllCoins(rows, preset, target = "COMBINED") {
  const kept = [];

  for (const row of rows) {
    if (!wouldEnterUnderPreset(row, preset)) continue;
  ...
  const wins = kept.filter(row => row.exitR > 0).length;
  const losses = kept.filter(row => row.exitR < 0).length;
  const completed = wins + losses;

  const totalR = kept.reduce((sum, row) => sum + row.exitR, 0);
  const totalPnlPct = kept.reduce((sum, row) => sum + row.pnlPct, 0);

  const grossWinR = kept
    .filter(row => row.exitR > 0)
    .reduce((sum, row) => sum + row.exitR, 0);
  ...
  const directSL = kept.filter(row => row.exitR < 0 && row.mfeR < 0.25).length;

  const nearTpThenLoss = kept.filter(row => {
    return row.exitR < 0 && row.plannedRR > 0 && row.mfeR >= row.plannedRR * 0.80;
  }).length;

  const setupClassCounts = kept.reduce((acc, row) => {
    acc[row.inferredSetupClass] = (acc[row.inferredSetupClass] || 0) + 1;
    return acc;
  }, {});
  ...
  const winrate = completed ? wins / completed : 0;
  const avgR = completed ? totalR / completed : 0;
  const avgPnlPct = completed ? totalPnlPct / completed : 0;
  const directSLPct = completed ? directSL / completed : 0;
  ...
  const decisionScore =
    ...
    keepRatio * 5 -
    directSLPct * 25 -
    (nearTpThenLoss / Math.max(1, completed)) * 10;
  ...
  return {
    ...
    winrateNum: Number(winrate.toFixed(4)),
    ...
    directSL,
    directSLPct: `${(directSLPct * 100).toFixed(1)}%`,
...
function buildRunnerMasterSeedPresets(current, target) {
  const base = {
    ...current
  };
  ...
  const discovery = {
    ...
    MIN_SCORE: Math.min(base.MIN_SCORE, 54),
    MIN_CONFLUENCE: Math.min(base.MIN_CONFLUENCE, 62),
    MIN_SNIPER_SCORE: 0,

    MIN_RUNNER_PRESSURE: Math.min(base.MIN_RUNNER_PRESSURE, 0.25),
    MIN_RUNNER_ACCELERATION: Math.min(base.MIN_RUNNER_ACCELERATION, -0.85),

    RUNNER_A_MIN_SCORE: Math.min(base.RUNNER_A_MIN_SCORE, 64),
    RUNNER_A_MIN_CONFLUENCE: Math.min(base.RUNNER_A_MIN_CONFLUENCE, 66),
    RUNNER_A_MIN_SNIPER: Math.min(base.RUNNER_A_MIN_SNIPER, 46),
    RUNNER_A_MIN_RR: Math.min(base.RUNNER_A_MIN_RR, 1.05),

    RUNNER_B_MIN_SCORE: Math.min(base.RUNNER_B_MIN_SCORE, 58),
    RUNNER_B_MIN_CONFLUENCE: Math.min(base.RUNNER_B_MIN_CONFLUENCE, 64),
    RUNNER_B_MIN_SNIPER: Math.min(base.RUNNER_B_MIN_SNIPER, 42),
    RUNNER_B_MIN_RR: Math.min(base.RUNNER_B_MIN_RR, 1.00),

    RUNNER_C_MIN_SCORE: Math.min(base.RUNNER_C_MIN_SCORE, 68),
    RUNNER_C_MIN_CONFLUENCE: Math.min(base.RUNNER_C_MIN_CONFLUENCE, 72),
    RUNNER_C_MIN_SNIPER: Math.min(base.RUNNER_C_MIN_SNIPER, 55),
    RUNNER_C_MIN_RR: Math.min(base.RUNNER_C_MIN_RR, 1.10),

    MAX_SPREAD_PCT: Math.max(base.MAX_SPREAD_PCT, 0.0022),
    SQUEEZE_MAX_SPREAD_PCT: Math.max(base.SQUEEZE_MAX_SPREAD_PCT, 0.0035),

    BLOCK_ENTRY_TYPES: [],
    LIVE_ONLY_HOT_RUNNER: false
  };

  const quality = {
    ...base,

    MIN_SCORE: base.MIN_SCORE + 2,
    MIN_CONFLUENCE: base.MIN_CONFLUENCE + 2,

    MIN_RUNNER_PRESSURE: Number((base.MIN_RUNNER_PRESSURE + 0.10).toFixed(3)),

    RUNNER_A_MIN_CONFLUENCE: base.RUNNER_A_MIN_CONFLUENCE + 2,
    RUNNER_A_MIN_SNIPER: base.RUNNER_A_MIN_SNIPER + 2,

    RUNNER_B_MIN_CONFLUENCE: base.RUNNER_B_MIN_CONFLUENCE + 2,
    RUNNER_B_MIN_SNIPER: base.RUNNER_B_MIN_SNIPER + 2,

    RUNNER_C_MIN_CONFLUENCE: base.RUNNER_C_MIN_CONFLUENCE + 2,
    RUNNER_C_MIN_SNIPER: base.RUNNER_C_MIN_SNIPER + 2,

    MAX_SPREAD_PCT: Number((base.MAX_SPREAD_PCT * 0.90).toFixed(6)),
    SQUEEZE_MAX_SPREAD_PCT: Number((base.SQUEEZE_MAX_SPREAD_PCT * 0.90).toFixed(6))
  };
  ...
  if (target === "B") {
    return [base, bDiscovery, discovery];
  }

  return [base, discovery, quality];
}

function scoreRunnerEvalForSearch(result, minCompleted) {
...
function optimizeRunnerMasterTarget(rows, target) {
  ...
  const currentEval = evaluatePresetOnAllCoins(rows, currentPreset, target);
  ...
  let beam = buildRunnerMasterSeedPresets(currentPreset, target);

  for (let pass = 0; pass < RUNNER_MASTER_BEAM_PASSES + 1; pass++) {
    ...
    const evaluated = Array.from(candidateMap.values())
      .map(preset => evaluatePresetOnAllCoins(rows, preset, target))
      .sort((a, b) => {
        return scoreRunnerEvalForSearch(b, minCompleted) - scoreRunnerEvalForSearch(a, minCompleted);
      });
  ...
  return {
    ...
    decision: missingFilters.length
      ...
      : best.completed >= minCompleted
        ...
        : `BEST_RUNNER_${target}_LOW_SAMPLE_AFSTELLING`,
    ...
    deltaVsCurrent: {
      totalPnlPctDelta: Number((best.totalPnlPct - currentEval.totalPnlPct).toFixed(3)),
      totalRDelta: Number((best.totalR - currentEval.totalR).toFixed(3)),
      avgRDelta: Number((best.avgR - currentEval.avgR).toFixed(3)),
      avgPnlPctDelta: Number((best.avgPnlPct - currentEval.avgPnlPct).toFixed(3)),
      winrateDeltaPct: Number(((best.winrateNum - currentEval.winrateNum) * 100).toFixed(2)),
      profitFactorDelta: Number((best.profitFactorR - currentEval.profitFactorR).toFixed(3)),
      decisionScoreDelta: Number((best.decisionScore - currentEval.decisionScore).toFixed(3))
    },
...
function compactRunnerOptimizationResult(result) {
  return {
    ...
    afstelling: result.best?.filters || buildCurrentRunnerMasterPreset()
  };
}

function buildRunnerMasterBestAfstellingLog({ btcState, runId }) {
  const rows = buildOptimizerUniverse();
...
// ================= WRAPPER VOOR buildFinalRunnerFilterDecision (crashfix) =================
function buildFinalRunnerFilterDecision() {
  return buildRunnerMasterBestAfstellingLog({
    btcState: "STATS_SNAPSHOT",
    runId: "stats_snapshot"
  });
}
    ...
    for (const raw of candidates) {
      ...
      if (safeNumber(c.price, 0) > 0 && !ob.fetchFailed) {
        try {
          ...
          runnerTrace("RUNNER_RISK_PRECALC_FAILED", buildTraceSnapshot(c, ctx, {
            error: err?.message || String(err),
            hasLiquidationData
          }));
        }
      ...
      // ================= MAIN-STYLE ULTRA LOOSE ENTRY FILTERS =================
      // Alleen technische/harde guards blijven actief.
      // Alle edge-filters zijn bewust uit zodat optimizer data krijgt.
      ...
      runnerTrace("RUNNER_RISK_READY", buildTraceSnapshot(c, ctx, {
        entry: c.price,
        sl: riskBase.sl,
        tp: targets.tp,
        baseRR,
      ...
      try {
        ...
        position.discordEntryAllowed = Boolean(discordDecision.allowed);
        position.discordEntryNotified = false;
        position.discordEntryBlocked = !discordDecision.allowed;
        position.discordBlockReason = discordDecision.reason;
        position.discordDecision = discordDecision;

        entryPayload.discordAllowed = Boolean(discordDecision.allowed);
        entryPayload.discordNotified = false;
        entryPayload.discordBlockReason = discordDecision.reason;
        entryPayload.discordFilterName = discordDecision.filter || null;
        entryPayload.discordDecision = discordDecision;

        memory.set(key, position);

        if (!notify) {
          runnerTrace("DISCORD_ENTRY_SKIPPED", buildTraceSnapshot(c, ctx, {
            reason: "NOTIFY_DISABLED",
            discordDecision
          }));
        } else if (!discordDecision.allowed) {
          vercelLog("info", "DISCORD_ENTRY_BLOCKED", buildTraceSnapshot(c, ctx, {
            reason: discordDecision.reason,
            finalRR,
            setupClass: setup.setupClass,
            entryType: setup.entryType,
            discordDecision
          }));
        } else if (notifyState.get(key)) {
          vercelLog("info", "DISCORD_ENTRY_DEDUPED", buildTraceSnapshot(c, ctx, {
            reason: "NOTIFY_STATE_ALREADY_SET",
            finalRR,
            setupClass: setup.setupClass,
            entryType: setup.entryType,
            discordDecision
          ...
          try {
            await sendEntry({
              ...
              discordFilterName: discordDecision.filter || null,
              discordReason: discordDecision.reason
            });

            position.discordEntryNotified = true;
            position.discordEntryBlocked = false;
            position.discordNotifiedAt = Date.now();

            entryPayload.discordNotified = true;
            ...
            position.discordEntryNotified = false;
            position.discordEntryBlocked = false;
            position.discordNotifyFailed = true;
            position.discordNotifyError = err?.message || String(err);

            entryPayload.discordNotified = false;
            entryPayload.discordNotifyFailed = true;
            entryPayload.discordNotifyError = err?.message || String(err);
