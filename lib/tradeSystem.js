...
// Alleen Discord melding filteren.
// Trade systeem, analyze, memory, shadows en logs blijven verder exact hetzelfde.
...
function getDiscordEntryDecision({
  ...
  for (const filter of DISCORD_ENTRY_FILTERS) {
    ...
    if (normalizeSide(filter.side) !== side) continue;

    if (score < safeNumber(filter.minScore, 0)) {
      return { allowed: false, reason: "DISCORD_SCORE_TOO_LOW", score, required: filter.minScore };
    }

    if (confluence < safeNumber(filter.minConfluence, 0)) {
      return { allowed: false, reason: "DISCORD_CONFLUENCE_TOO_LOW", confluence, required: filter.minConfluence };
    }

    if (sniperScore < safeNumber(filter.minSniperScore, 0)) {
      return { allowed: false, reason: "DISCORD_SNIPER_TOO_LOW", sniperScore, required: filter.minSniperScore };
    }

    if (rr < safeNumber(filter.minRR, 0)) {
      return { allowed: false, reason: "DISCORD_RR_TOO_LOW", rr, required: filter.minRR };
    }

    if (!listAllows(flow, filter.flows)) {
      return { allowed: false, reason: "DISCORD_FLOW_BLOCKED", flow, allowedFlows: filter.flows };
    }

    if (!listAllows(rsiZone, filter.rsiZones)) {
      return { allowed: false, reason: "DISCORD_RSI_ZONE_BLOCKED", rsiZone, allowedRsiZones: filter.rsiZones };
    }

    if (!listAllows(obBias, filter.obBiases)) {
      return { allowed: false, reason: "DISCORD_OB_BIAS_BLOCKED", obBias, allowedObBiases: filter.obBiases };
    }

    if (spreadPct > safeNumber(filter.maxSpreadPct, 0.05)) {
      return { allowed: false, reason: "DISCORD_SPREAD_TOO_HIGH", spreadPct, maxSpreadPct: filter.maxSpreadPct };
    }

    if (depthMinUsd1p < safeNumber(filter.minDepthUsd1p, 0)) {
      return { allowed: false, reason: "DISCORD_DEPTH_TOO_LOW", depthMinUsd1p, minDepthUsd1p: filter.minDepthUsd1p };
    }

    if (runnerPressure < safeNumber(filter.minRunnerPressure, -999)) {
      return { allowed: false, reason: "DISCORD_PRESSURE_TOO_LOW", runnerPressure, minRunnerPressure: filter.minRunnerPressure };
    }

    if (runnerAcceleration < safeNumber(filter.minRunnerAcceleration, -999)) {
      return { allowed: false, reason: "DISCORD_ACCELERATION_TOO_LOW", runnerAcceleration, minRunnerAcceleration: filter.minRunnerAcceleration };
    }

    return {
      allowed: true,
      reason: "DISCORD_FILTER_MATCH",
      matched: {
        side,
        score,
        confluence,
        ...
        runnerPressure,
        runnerAcceleration,
        setupClass: setup?.setupClass || null,
  ...
  return {
    allowed: false,
    reason: "DISCORD_NO_SIDE_FILTER_MATCH",
    side
  };
}
