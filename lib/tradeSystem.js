...
function evaluatePresetOnAllCoins(rows, preset) {
  const kept = [];
  ...
  const directSL = kept.filter(row => {
    return row.exitR < 0 && row.mfeR < 0.25;
  }).length;
  ...
  // Score: totale PnL en R wegen zwaar, winrate minder, directSL penaliseert
  const decisionScore =
    totalPnlPct * 1.25 +
    totalR * 1.10 +
    avgR * 20 +
...
function optimizeRunnerMasterTarget(rows, target) {
  ...
  const currentEval = evaluatePresetOnAllCoins(rows, currentPreset);
  ...
  for (let pass = 0; pass < RUNNER_MASTER_BEAM_PASSES; pass++) {
    ...
    const evaluated = Array.from(candidateMap.values())
      .map(preset => evaluatePresetOnAllCoins(rows, preset))
