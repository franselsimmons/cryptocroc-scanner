...
//
// Live Discord entries worden nu gefilterd op exacte microFamilyKey.
// Zet wekelijks handmatig je winnaars in ENV:
//
// RUNNER_ALLOWED_MICRO_FAMILY_KEYS=LONG_36::RUNNER_MICRO_V1::Q4_STRONG::...,SHORT_26::RUNNER_MICRO_V1::...
//
// Backward alias:
...
function normalizeOpenPositionForMemory(pos) {
  ...
  const normalized = {
    ...
    microLabels: normalizeLabels(pos.microLabels || derivedMicro?.microLabels || pos.labels),
    ...
    labels: normalizeLabels(pos.labels || derivedMicro?.labels),
...
function getShadowDedupeKey(payload) {
  return [
    ...
    normalizeMicroFamilyKey(payload?.microFamilyKey || payload?.runnerMicroFamilyKey || "NO_MICRO"),
...
function enrichWithMicroFamily(payload) {
  if (!payload?.symbol || !payload?.side) return payload;

  const micro = getRunnerMicroMetaForRow(payload);
  if (!micro?.microFamilyKey) return payload;

  return applyFamilyMeta(payload, micro);
}
...
function buildRunnerScanObservation({
  ...
  return enrichWithMicroFamily(payload);
}
...
function buildWait(c, reason, ctx = {}) {
  ...
  const payload = enrichWithMicroFamily({
    ...buildCommonPayload(c, ctx),
  ...
  runnerWaitTrace(c, reason, ctx, {
    ...
    microFamilyKey: payload.microFamilyKey || null
  });
...
function buildAnalyzeOnly(c, reason, ctx = {}, extra = {}) {
  ...
  const payload = {
    ...
    macroFamilyId: discordDecision?.familyId || null,
    ...
    microFamilyId: discordDecision?.microFamilyId || null,
    runnerMicroFamilyId: discordDecision?.microFamilyId || null,
    analyzeMicroFamilyId: discordDecision?.microFamilyId || null,
    analysisMicroFamilyId: discordDecision?.microFamilyId || null,
    discordMicroFamilyId: discordDecision?.microFamilyId || null,

    microFamilyKey: discordDecision?.microFamilyKey || null,
    runnerMicroFamilyKey: discordDecision?.microFamilyKey || null,
    analyzeMicroFamilyKey: discordDecision?.microFamilyKey || null,
    analysisMicroFamilyKey: discordDecision?.microFamilyKey || null,
    discordMicroFamilyKey: discordDecision?.microFamilyKey || null,

    labels: discordDecision?.family?.labels || [],
    microLabels: discordDecision?.family?.microLabels || [],
    microDefinition: discordDecision?.family?.microDefinition || null,
    microBuckets: discordDecision?.family?.microBuckets || null,

    discordAllowed: Boolean(discordDecision?.allowed),
    discordNotified: false,
    discordEntryAllowed: false,
    discordEntryNotified: false,
...
function buildPositionAction(action, reason, c, pos, ctx, extra = {}) {
  ...
  return {
    ...
    macroFamilyId: pos.macroFamilyId || pos.familyId || null,
    ...
    runnerMicroFamilyId: pos.runnerMicroFamilyId || pos.microFamilyId || null,
    analyzeMicroFamilyId: pos.analyzeMicroFamilyId || pos.microFamilyId || null,
    analysisMicroFamilyId: pos.analysisMicroFamilyId || pos.microFamilyId || null,
    discordMicroFamilyId: pos.discordMicroFamilyId || pos.microFamilyId || null,

    microFamilyKey: pos.microFamilyKey || null,
    runnerMicroFamilyKey: pos.runnerMicroFamilyKey || pos.microFamilyKey || null,
    analyzeMicroFamilyKey: pos.analyzeMicroFamilyKey || pos.microFamilyKey || null,
    analysisMicroFamilyKey: pos.analysisMicroFamilyKey || pos.microFamilyKey || null,
    discordMicroFamilyKey: pos.discordMicroFamilyKey || pos.microFamilyKey || null,

    labels: normalizeLabels(pos.labels),
    microLabels: normalizeLabels(pos.microLabels),
    microDefinition: pos.microDefinition || null,
    microBuckets: pos.microBuckets || null,
...
function buildPositionFromEntry({
  ...
  const familyId = normalizeFamilyId(discordDecision?.familyId);
  const microFamilyKey = getMicroFamilyKeyFromMeta(discordDecision);
  const microFamilyId = getMicroFamilyIdFromMeta(discordDecision);
  const labels = normalizeLabels(discordDecision?.family?.labels);
  const microLabels = getFamilyLabelsFromMeta(discordDecision?.family);
...
function optNormalizeOutcomeRow(row) {
  ...
  const normalized = {
    ...
    microFamilyId: getMicroFamilyIdFromMeta(row),
    microFamilyKey: getMicroFamilyKeyFromMeta(row),
    labels: normalizeLabels(row?.labels),
    microLabels: getFamilyLabelsFromMeta(row),
...
function buildLearningCohortDimensions(row) {
  return {
    ...
    microFamilyId: getMicroFamilyIdFromMeta(row) || "UNKNOWN",
    microFamilyKey: getMicroFamilyKeyFromMeta(row) || "UNKNOWN",
...
function buildLearningCohortKey(dim) {
  return [
    `MICRO_KEY=${dim.microFamilyKey}`,
    `FAMILY=${dim.familyId}`,
    `MICRO_ID=${dim.microFamilyId}`,
    `TYPE=${dim.entryType}`,
...
function buildCohortRule(dim) {
  return {
    ...
    microFamilyId: dim.microFamilyId,
    microFamilyKey: dim.microFamilyKey,
...
function buildCohortOutcomeLog(row) {
  ...
  return {
    ...
    microFamilyId: normalized.microFamilyId,
    microFamilyKey: normalized.microFamilyKey,
...
function buildFinalRunnerFilterDecision() {
  const allowedMicroFamilyKeys = Array.from(getAllowedDiscordMicroFamilyKeys());
  ...
  return {
    ...
    decision: "LIVE_DISCORD_ONLY_EXACT_MICRO_FAMILY_KEY_GATE_ACTIVE",

    rules: {
      allCoinsGoToAnalyze: true,
      liveEntryRequiresDiscordFamilyMatch: RUNNER_LIVE_REQUIRES_DISCORD_FILTER,
      liveEntryRequiresExactMicroFamilyKey: true,
      liveEntryRequiresFamilySideMatch: true,
      liveEntryRequiresRedisLock: RUNNER_REQUIRES_REDIS_FOR_LIVE,
      ...
      maxDiscordEntriesPerSidePerRun: RUNNER_MAX_DISCORD_ENTRIES_PER_SIDE_PER_RUN,
      maxDiscordOpenPositions: RUNNER_MAX_DISCORD_OPEN_POSITIONS
    },

    env: {
      required: "RUNNER_ALLOWED_MICRO_FAMILY_KEYS",
      alias: "RUNNER_DISCORD_MICRO_FAMILY_KEYS",
      configuredMicroKeys: allowedMicroFamilyKeys.length
    },
...
export async function processTrades(input, options = {}) {
  ...
  const discordEntriesBySideThisRun = {
    bull: 0,
    bear: 0
  };
  ...
  try {
    vercelLog("info", "RUN_START", {
      ...
      liveRequiresExactMicroFamilyKey: true,
      liveRequiresFamilySideMatch: true,
      liveRequiresRedisLock: RUNNER_REQUIRES_REDIS_FOR_LIVE,
      maxDiscordEntriesPerRun: RUNNER_MAX_DISCORD_ENTRIES_PER_RUN,
      maxDiscordEntriesPerSidePerRun: RUNNER_MAX_DISCORD_ENTRIES_PER_SIDE_PER_RUN,
      allowedMicroKeys: getAllowedDiscordMicroFamilyKeys().size,
    ...
    for (const raw of candidates) {
      ...
      const redisBlockReason =
        RUNNER_REQUIRES_REDIS_FOR_LIVE && !durableRequired
          ? "REDIS_REQUIRED_FOR_LIVE"
          : null;

      const liveEntryBlockReason =
        redisBlockReason ||
        getLiveEntryBlockReason({
          c,
          notify,
          discordDecision,
          discordEntriesSentThisRun,
      ...
      try {
        if (!isDiscordDecisionFinalAllowed(discordDecision, c.side)) {
          throw new Error("FINAL_DISCORD_DECISION_NOT_ALLOWED");
        }

        if (hasAnyOpenPositionForSymbol(c.symbol)) {
          throw new Error(`FINAL_SYMBOL_ALREADY_OPEN_${getOpenPositionSideForSymbol(c.symbol)}`);
        }

        if (hasRecentEntryNotify(c.symbol, c.side, c.price)) {
          throw new Error("FINAL_DUPLICATE_ENTRY_NOTIFY");
        }
        ...
        await sendEntry({
          ...
          discordFilterName: discordDecision.microFamilyKey || discordDecision.familyId || null,
          discordReason: discordDecision.reason,
          discordFamilyId: discordDecision.familyId || null,
          discordMicroFamilyId: discordDecision.microFamilyId || null,
          discordMicroFamilyKey: discordDecision.microFamilyKey || null,
          discordMicroLabels: discordDecision.family?.microLabels || []
        });
        ...
        stats.entries++;
        discordEntriesSentThisRun++;
        discordEntriesBySideThisRun[normalizeSide(c.side)] =
          safeNumber(discordEntriesBySideThisRun[normalizeSide(c.side)], 0) + 1;
        ...
        actions.push(buildAnalyzeOnly(
          c,
          err?.message || "DISCORD_ENTRY_SEND_FAILED",
          ctx,
          {
            risk: riskBase,
            targets,
