...
const STRATEGY_VERSION = "RUNNER_TS_V1_8_WEEKLY_MICRO_ENTRY_FILTERS";

// ================= WEEKLY ACTIVE MICRO ENTRY FILTERS =================
// Dit is de enige wekelijkse entry-afstelling.
// Geen familyId. Geen whitelist. Alleen directe filterwaarden uit de analyzer.
// Elke week alleen dit blok aanpassen.

const ACTIVE_MICRO_ENTRY_FILTERS = Object.freeze({
  bull: {
    confluenceMin: 75,
    confluenceMax: 85,
    ...
    allowedStages: new Set(["entry", "almost"]),

    // FLOW_TREND_OR_BUILDING
    allowedFlows: new Set([
      "TREND",
      "BUILDING"
    ]),

    // RSI_LOWER_OR_MID
    rsiMode: "LOWER_OR_MID",

    // OB_REL_NEUTRAL
    requiredObBias: "NEUTRAL",
    ...
    // FUNDING_NEUTRAL
    fundingMode: "NEUTRAL",
    fundingAbsMax: 0.00015,

    // TF_ALIGNED
    requireTfAligned: true,

    // PULLBACK_OR_CONFIRMATION_OK
    requirePullbackOrConfirmationOk: true
  },
...
// Cohorten moeten eerst data krijgen.
...
function compactRuntimeRow(row) {
  ...
  return {
    ...
    activeMicroFilterVersion: row.activeMicroFilterVersion,
    activeMicroFilterMatched: row.activeMicroFilterMatched,
    activeMicroFilterSnapshot: row.activeMicroFilterSnapshot,
...
function inMinMax(value, min, max) {
...
function inMinMaxInclusive(value, min, max) {
  const n = safeNumber(value, NaN);
  if (!Number.isFinite(n)) return false;
...
function getMicroFlowValue(c, flow) {
  return String(
    c?.scannerFlow ||
    c?.flow ||
    flow?.type ||
...
function getMicroObBias(ob) {
  return String(ob?.bias || "NEUTRAL").toUpperCase();
}

function isMicroTfAligned(c, ctx = {}) {
  const value = String(
    c?.tfAlignment ||
    ctx?.tfAlignment ||
    "UNKNOWN"
...
function getMicroBtcRelation(side, btcState) {
...
function isMicroFundingMatched(fundingRate, filter) {
  const rate = safeNumber(fundingRate, 0);

  if (filter.fundingMode === "NEUTRAL") {
    return Math.abs(rate) <= safeNumber(filter.fundingAbsMax, 0.00015);
  }

  return true;
}

function isMicroRsiMatched(rsiZone, filter) {
  const zone = String(rsiZone || "MID").toUpperCase();

  if (filter.rsiMode === "LOWER_OR_MID") {
    return [
      "LOWER_3",
      "LOWER_2",
      "LOWER_1",
  ...
  if (filter.rsiMode === "UPPER_OR_MID") {
    return [
      "UPPER_3",
      "UPPER_2",
      "UPPER_1",
...
function isMicroPullbackOrConfirmationOk({ c, flow, rsiZone }) {
  const stage = String(c?.stage || "unknown").toLowerCase();
  const side = normalizeSide(c?.side);
  const flowValue = getMicroFlowValue(c, flow);
  const zone = String(rsiZone || "MID").toUpperCase();
  ...
  if (
    stage === "almost" &&
    ["TREND", "BUILDING", "RUNNING", "BREAKOUT"].includes(flowValue)
  ) {
    return true;
  }
  ...
  if (
    side === "bull" &&
    ["LOWER_3", "LOWER_2", "LOWER_1", "MID"].includes(zone)
  ) {
    return true;
  }
  ...
  if (
    side === "bear" &&
    ["UPPER_3", "UPPER_2", "UPPER_1", "MID"].includes(zone)
  ) {
    return true;
  }
...
function checkActiveMicroEntryFilter({
  c,
  ctx,
  flow,
  ob,
  ...
  const side = normalizeSide(c?.side);
  const filter = ACTIVE_MICRO_ENTRY_FILTERS[side];

  if (!filter) {
    return {
      ok: false,
      reason: `MICRO_NO_ACTIVE_FILTER_FOR_${side}`
    };
  }
  ...
  const stage = String(c?.stage || "unknown").toLowerCase();
  const flowValue = getMicroFlowValue(c, flow);
  const obBias = getMicroObBias(ob);
  const spreadPct = normalizeSpread(ob?.spreadPct);
  const depthUsd = safeNumber(ob?.depthMinUsd1p, 0);
  const btcRelation = getMicroBtcRelation(side, btcState);
  const tfAligned = isMicroTfAligned(c, ctx);

  const pullbackOrConfirmationOk = isMicroPullbackOrConfirmationOk({
    c,
    flow,
    rsiZone
  });

  if (!inMinMax(confluence, filter.confluenceMin, filter.confluenceMax)) {
    return {
      ok: false,
      reason: `MICRO_CONFLUENCE_${safeNumber(confluence, 0).toFixed(2)}_NOT_IN_${filter.confluenceMin}_${filter.confluenceMax}`
    };
  }

  if (!inMinMax(sniperScore, filter.sniperMin, filter.sniperMax)) {
    return {
      ok: false,
      reason: `MICRO_SNIPER_${safeNumber(sniperScore, 0).toFixed(2)}_NOT_IN_${filter.sniperMin}_${filter.sniperMax}`
    };
  }

  if (!inMinMax(score, filter.scoreMin, filter.scoreMax)) {
    return {
      ok: false,
      reason: `MICRO_SCORE_${score.toFixed(2)}_NOT_IN_${filter.scoreMin}_${filter.scoreMax}`
    };
  }

  if (!inMinMax(finalRR, filter.rrMin, filter.rrMax)) {
    return {
      ok: false,
      reason: `MICRO_RR_${safeNumber(finalRR, 0).toFixed(2)}_NOT_IN_${filter.rrMin}_${filter.rrMax}`
    };
  }

  if (!filter.allowedStages.has(stage)) {
    return {
      ok: false,
      reason: `MICRO_STAGE_${stage}_NOT_ALLOWED`
    };
  }

  if (!filter.allowedFlows.has(flowValue)) {
    return {
      ok: false,
      reason: `MICRO_FLOW_${flowValue}_NOT_ALLOWED`
    };
  }

  if (!isMicroRsiMatched(rsiZone, filter)) {
    return {
      ok: false,
      reason: `MICRO_RSI_${rsiZone}_NOT_${filter.rsiMode}`
    };
  }

  if (obBias !== filter.requiredObBias) {
    return {
      ok: false,
      reason: `MICRO_OB_${obBias}_NOT_${filter.requiredObBias}`
    };
  }

  if (!inMinMaxInclusive(spreadPct, filter.spreadMinPct, filter.spreadMaxPct)) {
    return {
      ok: false,
      reason: `MICRO_SPREAD_${spreadPct}_NOT_IN_${filter.spreadMinPct}_${filter.spreadMaxPct}`
    };
  }

  if (!inMinMaxInclusive(depthUsd, filter.depthMinUsd1p, filter.depthMaxUsd1p)) {
    return {
      ok: false,
      reason: `MICRO_DEPTH_${depthUsd}_NOT_IN_${filter.depthMinUsd1p}_${filter.depthMaxUsd1p}`
    };
  }

  if (btcRelation !== filter.btcRelation) {
    return {
      ok: false,
      reason: `MICRO_BTC_REL_${btcRelation}_NOT_${filter.btcRelation}`
    };
  }

  if (!isMicroFundingMatched(funding?.rate, filter)) {
    return {
      ok: false,
      reason: `MICRO_FUNDING_${safeNumber(funding?.rate, 0)}_NOT_${filter.fundingMode}`
    };
  }

  if (filter.requireTfAligned && !tfAligned) {
    return {
      ok: false,
      reason: "MICRO_TF_NOT_ALIGNED"
    };
  }

  if (filter.requirePullbackOrConfirmationOk && !pullbackOrConfirmationOk) {
    return {
      ok: false,
      reason: "MICRO_PULLBACK_OR_CONFIRMATION_NOT_OK"
    };
  }

  return {
    ok: true,
    reason: "MICRO_ENTRY_FILTER_MATCHED",
    snapshot: {
      side,
      confluence: safeNumber(confluence, 0),
      sniperScore: safeNumber(sniperScore, 0),
      score,
      finalRR: safeNumber(finalRR, 0),
      stage,
      flowValue,
      rsiZone,
      obBias,
...
function buildPositionAction(action, reason, c, pos, ctx, extra = {}) {
  return {
    ...
    activeMicroFilterVersion: pos.activeMicroFilterVersion,
    activeMicroFilterMatched: pos.activeMicroFilterMatched,
    activeMicroFilterSnapshot: pos.activeMicroFilterSnapshot,
...
function buildPositionFromEntry({
  ...
  entryType,
  setupClass,
  activeMicroFilterSnapshot = null
}) {
  const now = Date.now();
  ...
  return {
    ...
    activeMicroFilterVersion: STRATEGY_VERSION,
    activeMicroFilterMatched: true,
    activeMicroFilterSnapshot,
...
export async function processTrades(input, options = {}) {
  ...
  try {
    ...
    for (const raw of candidates) {
      ...
      // ================= WEEKLY MICRO ENTRY FILTERS =================
      // Eerst technische guards.
      // Daarna moet de setup exact matchen met ACTIVE_MICRO_ENTRY_FILTERS.
      // Geen match = geen entry.
      ...
      const microEntryFilter = checkActiveMicroEntryFilter({
        c,
        ctx,
        flow,
        ob,
      ...
      if (!microEntryFilter.ok) {
        actions.push(buildWait(c, microEntryFilter.reason, {
          ...ctx,
          rr: finalRR,
          risk: {
            ...riskBase,
          ...
          requiredRR: "ACTIVE_MICRO_ENTRY_FILTER"
        }));
        continue;
      }
      ...
      const position = buildPositionFromEntry({
        ...
        activeMicroFilterSnapshot: microEntryFilter.snapshot
      });
