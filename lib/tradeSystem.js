...
const STRATEGY_VERSION = "RUNNER_TS_V2_1_EXACT_MICRO_FAMILY_GATE";
...
const DISCORD_ALLOWED_MICRO_FAMILIES = {
  LONG_36: {
    side: "bull",
    labels: [
      "Q4_STRONG",
...
const DISCORD_ALLOWED_FAMILY_IDS = new Set(
  Object.keys(DISCORD_ALLOWED_MICRO_FAMILIES)
);
...
function getMicroFamilyProfile(familyId) {
  return DISCORD_ALLOWED_MICRO_FAMILIES[normalizeFamilyId(familyId)] || null;
}
...
function getFamilyLabelsFromMeta(meta = {}) {
  return normalizeLabels(
    meta.labels ||
      ...
      meta?.discordDecision?.microMatch?.actualLabels ||
      meta?.microMatch?.actualLabels ||
      []
  );
}

function getMicroLabelGroup(label) {
  const l = String(label || "").toUpperCase();

  if (/^Q\d+_/.test(l)) return "QUALITY";
  if (/^M\d+_/.test(l)) return "MARKET";
  if (/^T\d+_/.test(l)) return "TIMING";

  if (l.startsWith("CONF_")) return "CONF";
  if (l.startsWith("SNIPER_")) return "SNIPER";
  if (l.startsWith("RR_")) return "RR";
  if (l.startsWith("SCORE_")) return "SCORE";
  if (l.startsWith("STAGE_")) return "STAGE";
  if (l.startsWith("FLOW_")) return "FLOW";
  if (l.startsWith("RSI_")) return "RSI";
  if (l.startsWith("OB_REL_")) return "OB_REL";
  if (l.startsWith("SPREAD_")) return "SPREAD";
  if (l.startsWith("DEPTH_")) return "DEPTH";
  if (l.startsWith("BTC_REL_")) return "BTC_REL";
  if (l.startsWith("FUNDING_")) return "FUNDING";
  if (l.startsWith("TF_")) return "TF";
  if (l.startsWith("PULLBACK_")) return "PULLBACK";

  return null;
}

function buildMicroSignature(labels) {
  const signature = new Map();

  for (const label of normalizeLabels(labels)) {
    const group = getMicroLabelGroup(label);
    if (!group) continue;

    signature.set(group, label);
  }

  return signature;
}

function getMicroSignatureMismatches(actualLabels, expectedLabels) {
  const actualSignature = buildMicroSignature(actualLabels);
  const expectedSignature = buildMicroSignature(expectedLabels);

  const missing = [];
  const mismatched = [];

  for (const [group, expectedLabel] of expectedSignature.entries()) {
    const actualLabel = actualSignature.get(group);
...
function microFamilyMatchesExactly(familyMeta, side) {
  const familyId = normalizeFamilyId(familyMeta?.familyId);
  const profile = getMicroFamilyProfile(familyId);

  if (!profile) {
    return {
      ok: false,
      reason: "DISCORD_MICRO_PROFILE_MISSING",
      familyId
    };
  }

  const actualSide = normalizeSide(side);

  if (actualSide !== profile.side) {
    return {
      ok: false,
      reason: "DISCORD_MICRO_SIDE_MISMATCH",
      familyId,
      expectedSide: profile.side,
      actualSide
    };
  }

  const actualLabels = getFamilyLabelsFromMeta(familyMeta);
  const check = getMicroSignatureMismatches(actualLabels, profile.labels);

  if (!check.ok) {
    return {
      ok: false,
      reason: "DISCORD_MICRO_SIGNATURE_MISMATCH",
      familyId,
      missing: check.missing,
      mismatched: check.mismatched,
      actualLabels,
      expectedLabels: profile.labels
    };
  }

  return {
    ok: true,
    reason: "DISCORD_EXACT_MICRO_FAMILY_MATCH",
    familyId,
    actualLabels,
    expectedLabels: profile.labels
  };
...
function isExactAllowedMicroFamilyForSide(familyMeta, side, allowedFamilies = getAllowedDiscordFamilyIds()) {
  const familyId = normalizeFamilyId(familyMeta?.familyId);

  if (!familyId) return false;
  if (!allowedFamilies.has(familyId)) return false;
  if (!familyIdMatchesTradeSide(familyId, side)) return false;

  return microFamilyMatchesExactly(familyMeta, side).ok === true;
}
...
function compactFamilyMeta(family) {
  ...
  return {
    familyId: normalizeFamilyId(family.familyId),
    runnerFamilyId: normalizeFamilyId(family.familyId),
    analyzeFamilyId: normalizeFamilyId(family.familyId),
    analysisFamilyId: normalizeFamilyId(family.familyId),
    ...
    labels: getFamilyLabelsFromMeta(family)
  };
}
...
function getDiscordEntryDecision(args) {
  ...
  const familyMeta = family ? compactFamilyMeta(family) : null;
  const familyId = normalizeFamilyId(family?.familyId);
  const allowed = getAllowedDiscordFamilyIds();
  ...
  const microMatch = microFamilyMatchesExactly(family, row.side);

  if (!microMatch.ok) {
    return {
      ...baseDecision,
      allowed: false,
      reason: microMatch.reason,
      microMatch
    };
  }
...
function isDiscordDecisionFinalAllowed(discordDecision, side) {
  ...
  return (
    ...
    discordDecision?.reason === "DISCORD_EXACT_MICRO_FAMILY_MATCH" &&
    discordDecision?.microMatch?.ok === true &&
    familyId &&
    familyIdMatchesTradeSide(familyId, side) &&
    getAllowedDiscordFamilyIds().has(familyId)
  );
}
...
function normalizeOpenPositionForMemory(pos) {
  ...
  const familyAllowed = isExactAllowedMicroFamilyForSide(
    {
      ...pos,
      familyId,
      labels: getFamilyLabelsFromMeta(pos)
    },
    side,
    allowedFamilies
  );
...
function seedOpenPositionsFromOptions(options = {}) {
  ...
  for (const row of rows) {
    ...
    if (
      !isExactAllowedMicroFamilyForSide(
        {
          ...pos,
          familyId,
          labels: getFamilyLabelsFromMeta(pos)
...
function buildAnalyzeOnly(c, reason, ctx = {}, extra = {}) {
  ...
  const payload = {
    ...
    labels: getFamilyLabelsFromMeta(discordDecision?.family),
...
function buildPositionFromEntry({
  ...
  const entry = safeNumber(c.price, 0);
  const familyId = normalizeFamilyId(discordDecision?.familyId);
  const labels = getFamilyLabelsFromMeta(discordDecision?.family);
...
function optNormalizeOutcomeRow(row) {
  ...
  const normalized = {
    ...
    labels: getFamilyLabelsFromMeta(row),
...
function buildLearningCohortDimensions(row) {
  return {
    ...
    microSignature: normalizeLabels(row?.labels).join("|")
  };
}
...
function buildLearningCohortKey(dim) {
  return [
    `FAMILY=${dim.familyId}`,
    `MICRO=${dim.microSignature || "UNKNOWN"}`,
    `TYPE=${dim.entryType}`,
...
function buildCohortRule(dim) {
  return {
    ...
    microSignature: dim.microSignature,
...
function buildFinalRunnerFilterDecision() {
  ...
  return {
    ...
    decision: "LIVE_DISCORD_ONLY_EXACT_MICRO_FAMILY_GATE_ACTIVE",

    rules: {
      allCoinsGoToAnalyze: true,
      liveEntryRequiresDiscordFamilyMatch: RUNNER_LIVE_REQUIRES_DISCORD_FILTER,
      liveEntryRequiresExactMicroSignature: true,
...
export async function processTrades(input, options = {}) {
  ...
  try {
    vercelLog("info", "RUN_START", {
      ...
      liveRequiresExactMicroSignature: true,
    ...
    for (const raw of candidates) {
      ...
      try {
        ...
        await sendEntry({
          ...
          discordMicroLabels: getFamilyLabelsFromMeta(discordDecision.family)
        });
