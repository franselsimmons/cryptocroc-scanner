...
function getDiscordEntryDecision({
  ...
  for (const filter of DISCORD_ENTRY_FILTERS) {
    ...
    if (!listAllows(setupClass, filter.setupClasses)) {
      tested.push({ filterName, reason: "SETUP_CLASS_BLOCKED", setupClass, allowed: filter.setupClasses });
      continue;
    }

    if (!listAllows(entryType, filter.entryTypes)) {
      tested.push({ filterName, reason: "ENTRY_TYPE_BLOCKED", entryType, allowed: filter.entryTypes });
      continue;
    }
