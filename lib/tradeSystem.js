...
function updatePositionMetrics(pos, price) {
  ...
  if (normalizeSide(pos.side) === "bull") {
...
function optCurrentFilterSet() {
  return {
    ...
    MIN_RUNNER_PRESSURE: 0.05,
    MIN_RUNNER_ACCELERATION: -0.65,
...
function optBuildPresetGrid(rows) {
  ...
  for (const MIN_CONFLUENCE of confluenceValues) {
    for (const MIN_SNIPER_SCORE of sniperValues) {
      for (const RUNNER_MIN_RR of rrValues) {
        for (const MAX_SPREAD_PCT_VALUE of spreadValues) {
          for (const MIN_DEPTH_USD_1P_VALUE of depthValues) {
            for (const MIN_RUNNER_PRESSURE of pressureValues) {
              for (const MIN_RUNNER_ACCELERATION of accelerationValues) {
...
function buildFilterSetpointsFromBest(best) {
  ...
  return {
    ...
    constants: {
      ...
      RUNNER_B_MIN_CONFLUENCE: Math.max(confluence - 4, 62),
      RUNNER_B_MIN_SNIPER: Math.max(sniper - 4, 58),
    ...
    behavioralFilters: {
      ...
      BLOCK_ENTRY_TYPES: f.BLOCK_ENTRY_TYPES || []
    },
    ...
    codePatch: [
      ...
      `const RUNNER_B_MIN_CONFLUENCE = ${Math.max(confluence - 4, 62)};`,
      `const RUNNER_B_MIN_SNIPER = ${Math.max(sniper - 4, 58)};`,
...
function buildFinalRunnerFilterDecision() {
  ...
  const badCohorts = cohorts
    .filter(row => row.avgR < 0 || row.directSLPctNum >= 0.45)
