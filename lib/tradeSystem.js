...
const STRATEGY_VERSION = "RUNNER_TS_V1_8_MICRO_FAMILY_DISCORD_ONLY";
...
// SetupClass (RUNNER_A/B/C) wordt niet gebruikt voor filtering, alleen de onderstaande velden.
...
const DISCORD_ENTRY_FILTERS = Object.freeze([
  {
    ...
    // Geen restrictie op setupClass/entryType meer
    setupClasses: [],
    entryTypes: [],
...
function numberInRange(value, min = -Infinity, max = Infinity) {
...
function getDiscordEntryDecision({
  ...
  // setupClass en entryType worden niet meer gebruikt voor filtering
  ...
  const flow = normalizeFlow(ctx?.flow?.type || c?.flow || c?.scannerFlow);
  const rsiZone = String(ctx?.rsiZone || "UNKNOWN").toUpperCase();
  const obBias = String(ob?.bias || "NEUTRAL").toUpperCase();

  const spreadPct = normalizeSpread(ob?.spreadPct);
  const depthMinUsd1p = safeNumber(ob?.depthMinUsd1p, 0);

  const runnerPressure = safeNumber(c?.runnerPressure, 0);
  const runnerAcceleration = safeNumber(c?.runnerAcceleration, 0);
  ...
  for (const filter of DISCORD_ENTRY_FILTERS) {
    ...
    const filterName = filter.name || "UNNAMED_FILTER";

    if (normalizeSide(filter.side) !== side) {
      tested.push({ filterName, reason: "SIDE_MISMATCH", side, required: filter.side });
      continue;
    }

    // Geen checks op setupClass/entryType meer
    // if (!listAllows(setupClass, filter.setupClasses)) { ... }
    // if (!listAllows(entryType, filter.entryTypes)) { ... }

    if (!listAllows(flow, filter.flows)) {
      tested.push({ filterName, reason: "FLOW_BLOCKED", flow, allowed: filter.flows });
      continue;
    }

    if (!listAllows(rsiZone, filter.rsiZones)) {
      tested.push({ filterName, reason: "RSI_ZONE_BLOCKED", rsiZone, allowed: filter.rsiZones });
      continue;
    }

    if (!listAllows(obBias, filter.obBiases)) {
      tested.push({ filterName, reason: "OB_BIAS_BLOCKED", obBias, allowed: filter.obBiases });
      continue;
    }

    if (!numberInRange(score, filter.minScore, filter.maxScore)) {
      tested.push({
        filterName,
    ...
    if (!numberInRange(confluence, filter.minConfluence, filter.maxConfluence)) {
      tested.push({
        filterName,
        reason: "CONFLUENCE_OUT_OF_RANGE",
        confluence,
        min: filter.minConfluence,
        max: filter.maxConfluence
      });
      continue;
    }

    if (!numberInRange(sniperScore, filter.minSniperScore, filter.maxSniperScore)) {
      tested.push({
        filterName,
        reason: "SNIPER_OUT_OF_RANGE",
        sniperScore,
        min: filter.minSniperScore,
        max: filter.maxSniperScore
      });
      continue;
    }

    if (!numberInRange(rr, filter.minRR, filter.maxRR)) {
      tested.push({
        filterName,
        reason: "RR_OUT_OF_RANGE",
        rr,
        min: filter.minRR,
        max: filter.maxRR
      });
    ...
    if (!numberInRange(spreadPct, filter.minSpreadPct, filter.maxSpreadPct)) {
      tested.push({
        filterName,
        reason: "SPREAD_OUT_OF_RANGE",
        spreadPct,
        min: filter.minSpreadPct,
        max: filter.maxSpreadPct
      });
      continue;
    }

    if (!numberInRange(depthMinUsd1p, filter.minDepthUsd1p, filter.maxDepthUsd1p)) {
      tested.push({
        filterName,
        reason: "DEPTH_OUT_OF_RANGE",
        depthMinUsd1p,
        min: filter.minDepthUsd1p,
        max: filter.maxDepthUsd1p
      });
      continue;
    }

    if (!numberInRange(runnerPressure, filter.minRunnerPressure, filter.maxRunnerPressure)) {
      tested.push({
        filterName,
        reason: "PRESSURE_OUT_OF_RANGE",
        runnerPressure,
        min: filter.minRunnerPressure,
        max: filter.maxRunnerPressure
      });
      continue;
    }

    if (!numberInRange(runnerAcceleration, filter.minRunnerAcceleration, filter.maxRunnerAcceleration)) {
      tested.push({
        filterName,
        reason: "ACCELERATION_OUT_OF_RANGE",
        runnerAcceleration,
        min: filter.minRunnerAcceleration,
        max: filter.maxRunnerAcceleration
      });
      continue;
    }
  ...
  try {
    ...
    for (const raw of candidates) {
      ...
      try {
        ...
        entryPayload.discordFilterName = discordDecision.filterName || null;
        ...
        if (!notify) {
          ...
          try {
            await sendEntry({
              ...
              discordFilterName: discordDecision.filterName || null,
