...
function getLifecycleAction(event) {
  const explicitEntry = hasExplicitEntryAction(event);
  const explicitExit = hasExplicitExitAction(event);

  if (explicitExit && !explicitEntry) return "EXIT";
...
async function persistToRedis(events) {
  ...
  try {
    ...
    await redisCommand(["DEL", RUNNER_ANALYZE_REDIS_LIST_KEY]);
...
export async function clearRunnerAnalyzeEvents() {
  ...
  if (hasRedis()) {
    try {
      await redisCommand(["DEL", RUNNER_ANALYZE_REDIS_LIST_KEY]);
      await redisCommand(["DEL", RUNNER_ANALYZE_REDIS_META_KEY]);
      await redisCommand(["DEL", RUNNER_ANALYZE_REDIS_BASE_KEY]).catch(() => null);
