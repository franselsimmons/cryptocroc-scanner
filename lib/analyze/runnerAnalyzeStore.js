...
// - Redis gebruikt LIST + RPUSH byte-safe batches + LTRIM.
...
// - Redis payload wordt compact + byte-batched om Upstash max request size te voorkomen.
...
const REDIS_MAX_REQUEST_BYTES = readNumberEnv(
  "RUNNER_ANALYZE_REDIS_MAX_REQUEST_BYTES",
  6 * 1024 * 1024
);

const REDIS_MAX_EVENT_BYTES = readNumberEnv(
  "RUNNER_ANALYZE_REDIS_MAX_EVENT_BYTES",
  64 * 1024
);
...
function getTradeId(event) {
  ...
  if (symbol && side && entry && openedAt > 0) {
...
function compactPersistEvent(event) {
  const row = safeObject(event);

  return cleanObject({
    profile: SYSTEM_PROFILE,
    runnerProfile: row.runnerProfile || SYSTEM_PROFILE,
    ...
    action: row.action,
    analyzeLifecycle: row.analyzeLifecycle,
    analyzeSource: row.analyzeSource,
    analyzeTs: row.analyzeTs,
    ts: row.ts,
    ...
    rsiHTF: row.rsiHTF,
    rsiZone: row.rsiZone,
    rsiContinuationScore: row.rsiContinuationScore,
    ...
    obQualityScore: row.obQualityScore,
    ...
    syntheticAnalyzeEntry: row.syntheticAnalyzeEntry,
    orphanExit: row.orphanExit,

    storedAt: row.storedAt,
    updatedAt: row.updatedAt,
    lastExitTs: row.lastExitTs,

    filterSnapshot: cleanObject({
      familyId: row.filterSnapshot?.familyId || row.familyId,
      runnerFamilyId: row.filterSnapshot?.runnerFamilyId || row.runnerFamilyId,
      analyzeFamilyId: row.filterSnapshot?.analyzeFamilyId || row.analyzeFamilyId,
      analysisFamilyId: row.filterSnapshot?.analysisFamilyId || row.analysisFamilyId,

      profile: SYSTEM_PROFILE,
      runnerProfile: SYSTEM_PROFILE,

      side: row.filterSnapshot?.side || row.side,
      setupClass: row.filterSnapshot?.setupClass || row.setupClass,
      entryType: row.filterSnapshot?.entryType || row.entryType,
      runnerEntryType: row.filterSnapshot?.runnerEntryType || row.runnerEntryType,

      qualityIndex: row.filterSnapshot?.qualityIndex,
      marketIndex: row.filterSnapshot?.marketIndex,
      timingIndex: row.filterSnapshot?.timingIndex,

      qualityBucket: row.filterSnapshot?.qualityBucket,
      marketBucket: row.filterSnapshot?.marketBucket,
      timingBucket: row.filterSnapshot?.timingBucket,

      stage: row.filterSnapshot?.stage || row.stage,
      scannerStage: row.filterSnapshot?.scannerStage || row.scannerStage,

      flow: row.filterSnapshot?.flow || row.flow,
      scannerFlow: row.filterSnapshot?.scannerFlow || row.scannerFlow,

      confluence: row.filterSnapshot?.confluence || row.confluence,
      sniperScore: row.filterSnapshot?.sniperScore || row.sniperScore,
      rr: row.filterSnapshot?.rr || row.rr,
      baseRR: row.filterSnapshot?.baseRR || row.baseRR,
      plannedRR: row.filterSnapshot?.plannedRR || row.plannedRR,
      score: row.filterSnapshot?.score || row.score,
      moveScore: row.filterSnapshot?.moveScore || row.moveScore,

      runnerPressure: row.filterSnapshot?.runnerPressure || row.runnerPressure,
      runnerAcceleration: row.filterSnapshot?.runnerAcceleration || row.runnerAcceleration,

      rsi: row.filterSnapshot?.rsi || row.rsi,
      rsiHTF: row.filterSnapshot?.rsiHTF || row.rsiHTF,
      rsiZone: row.filterSnapshot?.rsiZone || row.rsiZone,

      obBias: row.filterSnapshot?.obBias || row.obBias,
      spreadPct: row.filterSnapshot?.spreadPct || row.spreadPct,
      spreadBps: row.filterSnapshot?.spreadBps || row.spreadBps,
      depthMinUsd1p: row.filterSnapshot?.depthMinUsd1p || row.depthMinUsd1p,

      btcState: row.filterSnapshot?.btcState || row.btcState,
      fundingRate: row.filterSnapshot?.fundingRate || row.fundingRate,

      tfScore: row.filterSnapshot?.tfScore || row.tfScore,
      tfStrength: row.filterSnapshot?.tfStrength || row.tfStrength,

      structure: row.filterSnapshot?.structure || row.structure,
      structureAligned: row.filterSnapshot?.structureAligned || row.structureAligned,

      strategyVersion: row.filterSnapshot?.strategyVersion || row.strategyVersion,
      syntheticAnalyzeEntry: row.syntheticAnalyzeEntry,
    }),
  });
}

function stringifyPersistEvent(event) {
  const compact = compactPersistEvent(event);
  const json = JSON.stringify(compact);

  if (byteLength(json) <= REDIS_MAX_EVENT_BYTES) {
    return json;
  }

  const smaller = cleanObject({
    profile: SYSTEM_PROFILE,
    runnerProfile: SYSTEM_PROFILE,
    ...
    analyzeLifecycle: compact.analyzeLifecycle,
    analyzeSource: compact.analyzeSource,
    analyzeTs: compact.analyzeTs,
    ts: compact.ts,

    familyId: compact.familyId,
    runnerFamilyId: compact.runnerFamilyId,
    analyzeFamilyId: compact.analyzeFamilyId,
    analysisFamilyId: compact.analysisFamilyId,

    openedAt: compact.openedAt,
    entryTs: compact.entryTs,
    entry: compact.entry,
    entryPrice: compact.entryPrice,
    openPrice: compact.openPrice,
    ...
    rr: compact.rr,
    baseRR: compact.baseRR,
    finalRR: compact.finalRR,
    plannedRR: compact.plannedRR,
    targetR: compact.targetR,
    ...
    realizedR: compact.realizedR,
    pnlR: compact.pnlR,
    exitR: compact.exitR,
    resultR: compact.resultR,
    outcomeR: compact.outcomeR,
    rMultiple: compact.rMultiple,
    pnlPct: compact.pnlPct,

    exitReason: compact.exitReason,

    setupClass: compact.setupClass,
    entryType: compact.entryType,
    runnerEntryType: compact.runnerEntryType,

    confluence: compact.confluence,
    sniperScore: compact.sniperScore,
    moveScore: compact.moveScore,
    score: compact.score,

    stage: compact.stage,
    scannerStage: compact.scannerStage,
    flow: compact.flow,
    scannerFlow: compact.scannerFlow,

    runnerPressure: compact.runnerPressure,
    runnerAcceleration: compact.runnerAcceleration,

    rsi: compact.rsi,
    rsiHTF: compact.rsiHTF,
    rsiZone: compact.rsiZone,

    obBias: compact.obBias,
    spreadPct: compact.spreadPct,
    spreadBps: compact.spreadBps,
    depthMinUsd1p: compact.depthMinUsd1p,

    btcState: compact.btcState,
    fundingRate: compact.fundingRate,

    tfScore: compact.tfScore,
    tfStrength: compact.tfStrength,

    regime: compact.regime,
    structure: compact.structure,
    structureAligned: compact.structureAligned,

    partialTaken: compact.partialTaken,
    breakEvenMoved: compact.breakEvenMoved,
    trailingActive: compact.trailingActive,
    adds: compact.adds,
    ...
    syntheticAnalyzeEntry: compact.syntheticAnalyzeEntry,
    orphanExit: compact.orphanExit,

    storedAt: compact.storedAt,
    updatedAt: compact.updatedAt,
    lastExitTs: compact.lastExitTs,
...
function buildRedisRpushBatches(rows) {
  const batches = [];
  let current = [];
  let currentBytes = byteLength(JSON.stringify(["RPUSH", RUNNER_ANALYZE_REDIS_LIST_KEY]));

  for (const row of rows) {
    const rowBytes = byteLength(row) + 16;

    if (
      current.length > 0 &&
      (
        current.length >= REDIS_RPUSH_BATCH_SIZE ||
        currentBytes + rowBytes > REDIS_MAX_REQUEST_BYTES
      )
    ) {
      batches.push(current);
      current = [];
      currentBytes = byteLength(JSON.stringify(["RPUSH", RUNNER_ANALYZE_REDIS_LIST_KEY]));
    }
...
async function persistToRedis(events) {
  ...
  try {
    ...
    const rows = trimmed.map(stringifyPersistEvent);
    const batches = buildRedisRpushBatches(rows);
    ...
    await redisCommand([
      ...
      JSON.stringify({
        ...
        redisMaxEventBytes: REDIS_MAX_EVENT_BYTES,
        persistedAt: Date.now(),
        mode: "redis_list_byte_batched_compact",
      }),
    ]);
...
async function persistToFile(events) {
  try {
    ...
    const rows = trimToMax(events).map(compactPersistEvent);
...
async function persistEvents(events) {
  const trimmed = trimToMax(events).map(compactPersistEvent);

  globalStore.events = trimmed;
  globalStore.lastPersistAt = Date.now();

  const [redis, file] = await Promise.allSettled([
  ...
  const redisResult =
    redis.status === "fulfilled"
      ...
      : {
          ...
          source: "redis",
          key: RUNNER_ANALYZE_REDIS_LIST_KEY,
          error: redis.reason?.message || "redis_persist_rejected",
        };
  ...
  const fileResult =
    file.status === "fulfilled"
      ...
      : {
          ...
          source: "file",
          path: RUNNER_ANALYZE_FILE_PATH,
          error: file.reason?.message || "file_persist_rejected",
        };
