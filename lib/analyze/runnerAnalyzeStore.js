...
// - Secondary matching via symbol + side + entry werkt als tradeId verschilt.
...
function stablePriceKey(value) {
  const n = Number(value);
  return Number.isFinite(n) && n > 0 ? Number(n).toPrecision(12) : "";
}
...
function getEntryPrice(event) {
  return nullableNumber(event?.entry ?? event?.entryPrice ?? event?.openPrice);
}

function getFallbackPrice(event) {
...
function getTradeId(event) {
  ...
  const entry = getEntryPrice(event);
  const fallbackPrice = getFallbackPrice(event);
  ...
  if (symbol && side && entry && openedAt > 0) {
    return `RUNNER_${symbol}_${side}_${openedAt}_${stablePriceKey(entry)}`;
  }
  ...
  if (symbol && side && entry && closedAt > 0) {
    return `RUNNER_${symbol}_${side}_${closedAt}_${stablePriceKey(entry)}`;
  }

  if (symbol && side && fallbackPrice && closedAt > 0) {
    return `RUNNER_${symbol}_${side}_${closedAt}_${stablePriceKey(fallbackPrice)}`;
  }

  return "";
}

function buildMatchKey(event) {
  const symbol = normalizeSymbol(event?.symbol);
  const side = normalizeSide(event?.side || event?.direction || event?.tradeSide);
  const entry = getEntryPrice(event);
  ...
  return `${symbol}|${side}|${stablePriceKey(entry)}`;
}
...
function buildIndexes(records) {
  ...
  for (const record of safeArray(records)) {
    ...
    const matchKey = buildMatchKey(record);
    ...
    if (record.closed !== true) {
      ...
      if (matchKey) openByMatchKey.set(matchKey, record);
    }
  }
...
function mergeExitIntoEntry(entry, exit) {
  ...
  const exitSnapshot = safeObject(exit.filterSnapshot);
  ...
  entry.familyId = entry.familyId || exit.familyId || exit.runnerFamilyId || null;
  entry.runnerFamilyId = entry.runnerFamilyId || exit.runnerFamilyId || exit.familyId || null;
  entry.analyzeFamilyId = entry.analyzeFamilyId || exit.analyzeFamilyId || entry.familyId || null;
  entry.analysisFamilyId = entry.analysisFamilyId || exit.analysisFamilyId || entry.familyId || null;

  entry.filterSnapshot = cleanObject({
    ...safeObject(entry.filterSnapshot),
    ...exitSnapshot,
    familyId: entry.familyId || exitSnapshot.familyId,
    runnerFamilyId: entry.runnerFamilyId || exitSnapshot.runnerFamilyId,
    analyzeFamilyId: entry.analyzeFamilyId || exitSnapshot.analyzeFamilyId,
    profile: SYSTEM_PROFILE,
...
export async function appendRunnerAnalyzeEvents(events, context = {}) {
  ...
  for (const rawEvent of received) {
    ...
    const action = getLifecycleAction(event);
    const tradeId = getTradeId(event);
    const matchKey = buildMatchKey(event);
    ...
    if (action === "ENTRY") {
      const existingEntry =
        indexes.entryByTradeId.get(tradeId) ||
        indexes.openByMatchKey.get(matchKey);

      if (existingEntry && event.closed === true && existingEntry.closed !== true) {
        mergeExitIntoEntry(existingEntry, event);

        indexes.openByTradeId.delete(getTradeId(existingEntry));
        indexes.openByTradeId.delete(tradeId);

        if (matchKey) indexes.openByMatchKey.delete(matchKey);
        if (eventKey) indexes.eventKeySet.add(eventKey);

        indexes.entryByTradeId.set(tradeId, existingEntry);
      ...
      if (storedEntry.closed !== true) {
        ...
        if (matchKey) indexes.openByMatchKey.set(matchKey, storedEntry);
      }
    ...
    if (action === "EXIT") {
      const openEntry =
        indexes.openByTradeId.get(tradeId) ||
        indexes.openByMatchKey.get(matchKey);

      if (openEntry) {
        mergeExitIntoEntry(openEntry, event);

        indexes.openByTradeId.delete(getTradeId(openEntry));
        indexes.openByTradeId.delete(tradeId);

        if (matchKey) indexes.openByMatchKey.delete(matchKey);
        if (eventKey) indexes.eventKeySet.add(eventKey);

        indexes.entryByTradeId.set(tradeId, openEntry);
