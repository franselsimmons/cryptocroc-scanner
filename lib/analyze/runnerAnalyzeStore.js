...
// - Closed ENTRY records met dezelfde tradeId sluiten bestaande open ENTRY.
...
function hasExitFields(event) {
  return (
    ...
    hasValue(event?.exit) ||
    hasValue(event?.executionPrice) ||
    hasValue(event?.closedAt) ||
    hasValue(event?.exitedAt) ||
    hasValue(event?.exitAt) ||
    hasValue(event?.exitTs) ||
    hasValue(event?.realizedR) ||
    hasValue(event?.pnlR) ||
    hasValue(event?.exitR) ||
    hasValue(event?.resultR) ||
    hasValue(event?.outcomeR) ||
    hasValue(event?.pnlPct)
  );
}
...
function hasEntryFields(event) {
  return (
    ...
    hasValue(event?.familyId) ||
    hasValue(event?.runnerFamilyId) ||
    hasValue(event?.analyzeFamilyId) ||
    hasValue(event?.analysisFamilyId) ||
    hasValue(event?.filterSnapshot?.familyId) ||
    hasValue(event?.filterSnapshot?.runnerFamilyId) ||
    hasValue(event?.filterSnapshot?.analyzeFamilyId)
  );
}
...
function getLifecycleAction(event) {
  ...
  // - Als een row ENTRY + closed/outcome bevat, sluiten we de ENTRY als closed ENTRY.
...
function normalizeAnalyzeEvent(event, fallbackTs = Date.now()) {
  ...
  return cleanObject({
    ...
    rsiValid: event.rsiValid,
    rsiBlocked: event.rsiBlocked,
    rsiExhaustedAgainstSide: event.rsiExhaustedAgainstSide,
...
export async function loadRunnerAnalyzeStore(options = {}) {
  ...
  return {
    ...
    primary: hasRedis() ? "redis_list" : "file_or_memory",
...
export async function appendRunnerAnalyzeEvents(events, context = {}) {
  ...
  for (const rawEvent of received) {
    ...
    if (action === "ENTRY") {
      ...
      } else if (storedEntry.syntheticAnalyzeEntry) {
