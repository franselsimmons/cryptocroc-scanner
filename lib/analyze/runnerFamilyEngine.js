...
function finalizeFamily(family, minClosed) {
  ...
  // Runner-score is PnL-first.
  // Balance-score is geschikt voor main Discord-signals.
  // RunnerPnlScore is geschikt voor runner-picks.
...
function getBestBalanceFamilies(families, limit = 10) {
  return [...families]
...
function getBestRunnerPnlFamilies(families, limit = 10) {
  return [...families]
    .filter(family => family.closed > 0)
    .sort((a, b) => {
...
export function buildRunnerAnalyzeReport(events = [], options = {}) {
  ...
  const bestBalance = getBestBalanceFamilies(finalizedAll, 10);
  const bestRunnerPnl = getBestRunnerPnlFamilies(finalizedAll, 10);
