...
function hasClosedSignal(row) {
  ...
  // Cruciaal: open ENTRY met resultR:null / pnlPct:null mag nooit closed worden.
...
function buildFamilyStats(def, rows, options = {}) {
  ...
  const closedRows = familyRows.filter(row => row.closed === true && row.resultR !== null);
  ...
  const out = {
    ...
    open: familyRows.filter(row => row.closed !== true).length,
    pending: familyRows.filter(row => row.closed !== true || row.resultR === null).length,
