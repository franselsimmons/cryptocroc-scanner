...
const DEFAULT_SELECTION_MIN_WINRATE = 40;
const DEFAULT_SELECTION_MIN_AVG_R = 0.05;
const DEFAULT_SELECTION_MIN_PROFIT_FACTOR = 1.15;
...
function canonicalFamilyId(side, index) {
  return `${side}_${index}`;
}

function prefixedRunnerFamilyId(side, index) {
  return `RUNNER_${side}_${index}`;
}
...
function parseFamilyId(value) {
  ...
  return {
    ...
    marketIndex,
    timingIndex,
    familyId: canonicalFamilyId(side, index),
    runnerFamilyId: canonicalFamilyId(side, index),
    prefixedRunnerFamilyId: prefixedRunnerFamilyId(side, index),
  };
}
...
function createFamily(side, q, m, t) {
  ...
  const id = canonicalFamilyId(side, index);
...
function setupClassBonus(event) {
  const setupClass = normalizeText(
    firstString(event, ["setupClass"], "")
  );
  ...
  if (setupClass === "RUNNER_B" || entryType.includes("RUNNER_B")) return -0.10;
...
function getStructureRelative(event, side) {
  ...
  if (raw.includes("WITH")) return "WITH";
  if (raw.includes("AGAINST")) return "AGAINST";
  if (raw.includes("RANGE") || raw.includes("NEUTRAL") || raw.includes("UNKNOWN")) return "NEUTRAL";
...
function isRsiTimed(event, side) {
  ...
  if (zone) {
    ...
    if (side === "LONG") {
      ...
      return zone.includes("LOWER") || zone.includes("OVERSOLD") || zone.includes("UPPER_1");
    }

    if (side === "SHORT") {
      if (zone.includes("LOWER_3")) return false;
      return zone.includes("UPPER") || zone.includes("OVERBOUGHT") || zone.includes("LOWER_1");
    }
  }
...
function isTradeRecord(event) {
  ...
  const kind = normalizeText(event?.analyzeKind || event?.type);
  const profile = normalizeText(event?.profile || event?.runnerProfile || event?.filterSnapshot?.profile || "");
...
function getFamilyDecision(family, minClosed) {
  ...
  if (family.avgR >= 0.55 && family.profitFactorR >= 1.8 && family.winrateNum >= 45) {
    return "ALLOW_PRIORITY";
  }

  if (family.avgR >= 0.35 && family.profitFactorR >= 1.45 && family.winrateNum >= 40) {
    return "ALLOW";
  }

  if (family.avgR >= 0.15 && family.profitFactorR >= 1.15 && family.winrateNum >= 35) {
...
function scoreFamilyStatus(family, minClosed) {
  ...
  if (family.avgR >= 0.55 && family.profitFactorR >= 1.8 && family.winrateNum >= 45) return "HOT";
  if (family.avgR >= 0.35 && family.profitFactorR >= 1.45 && family.winrateNum >= 40) return "GOOD";
  if (family.avgR >= 0.15 && family.profitFactorR >= 1.15 && family.winrateNum >= 35) return "STABLE";
...
function finalizeFamily(family, minClosed) {
  ...
  family.balanceScore = round(
    family.avgR * 45 +
      family.winrateNum * 0.35 +
      Math.min(family.profitFactorR, 10) * 2.5 +
      Math.log10(Math.max(family.closed, 1)) * 4,
    3
  );
  ...
  family.runnerPnlScore = round(
    family.avgR * 70 +
      family.avgPnlPct * 8 +
      Math.min(family.profitFactorR, 10) * 3 +
      Math.log10(Math.max(family.closed, 1)) * 3,
    3
  );

  family.runnerScore = round(
    family.runnerPnlScore * 0.65 + family.balanceScore * 0.35,
    3
  );
...
function getQualifiedFamilies(families, options = {}) {
  const minClosed = safeNumber(options.minClosed, DEFAULT_MIN_CLOSED);
  const minWinrate = safeNumber(options.minWinrate, DEFAULT_SELECTION_MIN_WINRATE);
  const minAvgR = safeNumber(options.minAvgR, DEFAULT_SELECTION_MIN_AVG_R);
  const minProfitFactor = safeNumber(options.minProfitFactor, DEFAULT_SELECTION_MIN_PROFIT_FACTOR);

  return safeArray(families).filter(family => {
    if (family.closed < minClosed) return false;
    if (!["HOT", "GOOD", "STABLE"].includes(family.status)) return false;
    if (family.avgR < minAvgR) return false;
    if (family.winrateNum < minWinrate) return false;
    if (family.profitFactorR < minProfitFactor) return false;
...
function getBestFamilies(families, limit = 10) {
  return sortRankedFamilies(
    safeArray(families).filter(family =>
...
function getBestRunnerPnlFamilies(families, limit = 10, minClosed = 1) {
  return [...safeArray(families)]
    .filter(family => family.closed >= minClosed)
    .sort((a, b) => {
      const scoreDiff = b.runnerPnlScore - a.runnerPnlScore;
      if (scoreDiff !== 0) return scoreDiff;

      const avgRDiff = b.avgR - a.avgR;
      if (avgRDiff !== 0) return avgRDiff;

      const pnlDiff = b.avgPnlPct - a.avgPnlPct;
...
function buildSelection(finalizedAll, options = {}) {
  const minClosed = safeNumber(options.minClosed, DEFAULT_MIN_CLOSED);
  const minWinrate = safeNumber(options.minWinrate, DEFAULT_SELECTION_MIN_WINRATE);
  const minAvgR = safeNumber(options.minAvgR, DEFAULT_SELECTION_MIN_AVG_R);
  const minProfitFactor = safeNumber(options.minProfitFactor, DEFAULT_SELECTION_MIN_PROFIT_FACTOR);

  const qualified = getQualifiedFamilies(finalizedAll, {
    minClosed,
    minWinrate,
    minAvgR,
    minProfitFactor,
  ...
  const runnerPnlQualified = getBestRunnerPnlFamilies(qualified, 10, minClosed);
  const balanceQualified = getBestBalanceFamilies(qualified, 10, minClosed);
  const rankedQualified = sortRankedFamilies(qualified);

  const primaryRunnerPnlFamily = runnerPnlQualified[0] || null;
  const primaryBalancedFamily = balanceQualified[0] || null;
  const primaryRankedFamily = rankedQualified[0] || null;

  const allowedFamilyIds = uniqueArray(rankedQualified.map(family => family.id));

  const allowedRunnerFamilyIds = uniqueArray(
    rankedQualified.flatMap(family => [
      family.id,
      family.runnerFamilyId,
      family.prefixedRunnerFamilyId,
      `RUNNER_${family.id}`,
    ])
  );
  ...
  return {
    ...
    objective:
      "Kies alleen runner families met genoeg closed trades, positieve Avg R, fatsoenlijke winrate, profit factor >= minimum en status HOT/GOOD/STABLE.",
    ...
    blockedFamilyIds: finalizedAll
      .filter(family => family.closed >= minClosed && family.status === "BAD")
      .map(family => family.id),

    blockedRunnerFamilyIds: finalizedAll
      .filter(family => family.closed >= minClosed && family.status === "BAD")
      .flatMap(family => [
        family.runnerFamilyId || family.id,
        family.prefixedRunnerFamilyId || `RUNNER_${family.id}`,
      ]),
  };
}
...
export function buildRunnerAnalyzeReport(events = [], options = {}) {
  ...
  const bestBalanceQualified = getBestBalanceFamilies(qualified, 10, minClosed);
  const bestRunnerPnlQualified = getBestRunnerPnlFamilies(qualified, 10, minClosed);

  const selection = buildSelection(finalizedAll, {
    minClosed,
    minWinrate,
    minAvgR,
    minProfitFactor,
  ...
  return {
    ...
    config: {
      ...
      objective: "RUNNER_PNL_FIRST_WITH_BALANCED_SELECTION_GATE",
    },
...
export function selectRunnerFamilies(events = [], options = {}) {
  const report = buildRunnerAnalyzeReport(events, options);
