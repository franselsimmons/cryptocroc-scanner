...
function getQualifiedFamilies(families, minClosed) {
...
function buildSelection(finalizedAll, minClosed) {
  const qualified = getQualifiedFamilies(finalizedAll, minClosed);
  ...
  return {
    ...
    minClosed,
    objective: "Kies pas families met genoeg closed trades, positieve Avg R, profit factor >= 1.15 en status HOT/GOOD/STABLE.",
    ...
    allowedFamilyIds: rankedQualified.map(family => family.id),
    allowedRunnerFamilyIds: rankedQualified.map(family => family.runnerFamilyId || family.id),
