...
const MICRO_FAMILY_VERSION = "RUNNER_MICRO_V1";
const MICRO_LIVE_FILTER_FIELD = "microFamilyKey";
...
function marketIndexFromRow(row) {
  ...
  if (depth <= 0) scores.push(3);
  else if (depth < 10000) scores.push(1);
...
function isTimedRow(row) {
  ...
  const tf = tfGroupLabel(row);
  const tfOk = tf === "TF_ALIGNED" || tf === "TF_UNKNOWN";
...
function buildFamilyDefinition(side, qualityIndex, marketIndex, timingIndex) {
  ...
  return {
    ...
    macroFamilyId: `${side}_${familyNumber}`,
...
export function buildRunnerMicroBucketMap(row, familyArg = null) {
  const family = familyArg || getRunnerFamilyForRow(row);
  if (!family?.familyId) return null;

  return {
    microVersion: MICRO_FAMILY_VERSION,

    macroFamilyId: family.familyId,
    ...
    confluenceBucket: confBucketLabel(row),
    sniperBucket: sniperBucketLabel(row),
    rrBucket: rrBucketLabel(row),
    scoreBucket: scoreBucketLabel(row),

    stageGroup: stageGroupLabel(row),
    stageExact: stageExactLabel(row),

    flowGroup: flowGroupLabel(row),
    flowExact: flowExactLabel(row),

    rsiSideGroup: rsiSideGroupLabel(row),
    rsiExact: rsiExactLabel(row),

    obRelation: obRelLabel(row),
    spreadBucket: spreadBucketLabel(row),
    depthBucket: depthBucketLabel(row),
    btcRelation: btcRelLabel(row),
    fundingState: fundingLabel(row),

    tfGroup: tfGroupLabel(row),
    tfExact: tfExactLabel(row),
    tfStrength: tfStrengthLabel(row),

    confirmationGroup: confirmationGroupLabel(row),
    confirmationPullback: row.pullbackConfirmed === true ? "PULLBACK_ON" : "PULLBACK_OFF",
    confirmationSweep: row.sweepConfirmed === true ? "SWEEP_ON" : "SWEEP_OFF",
    confirmationRetest: row.retestConfirmed === true ? "RETEST_ON" : "RETEST_OFF",
    confirmationStructure: row.structureAligned === true ? "STRUCTURE_ALIGNED" : "STRUCTURE_NOT_ALIGNED",
    confirmationRsiPullback: row.rsiPullbackAllowed === true ? "RSI_PULLBACK_ALLOWED" : "RSI_PULLBACK_BLOCKED",
    confirmationRsiContinuation: row.rsiContinuationAllowed === true ? "RSI_CONTINUATION_ALLOWED" : "RSI_CONTINUATION_BLOCKED",

    regime: regimeLabel(row),
    volatility: volatilityLabel(row),

    setupClass: setupClassLabel(row),
    entryType: entryTypeLabel(row)
  };
}

export function buildRunnerMicroLabels(row, familyArg = null) {
  const buckets = buildRunnerMicroBucketMap(row, familyArg);
  if (!buckets) return [];

  return [
    buckets.microVersion,
    ...
    buckets.depthBucket,
    buckets.btcRelation,
    buckets.fundingState,

    buckets.tfGroup,
    buckets.tfExact,
    buckets.tfStrength,

    buckets.confirmationGroup,
    buckets.confirmationPullback,
    buckets.confirmationSweep,
    buckets.confirmationRetest,
    buckets.confirmationStructure,
    buckets.confirmationRsiPullback,
    buckets.confirmationRsiContinuation,
...
export function getRunnerMicroFamilyForRow(row, familyArg = null) {
  ...
  const microBuckets = buildRunnerMicroBucketMap(row, family);
  const microLabels = buildRunnerMicroLabels(row, family);

  if (!microLabels.length) return null;
  ...
  return {
    ...
    microVersion: MICRO_FAMILY_VERSION,
    microFilterField: MICRO_LIVE_FILTER_FIELD,
...
export function getRunnerMicroFamilyKeyForRow(row) {
  const micro = getRunnerMicroFamilyForRow(row);
  return micro?.microFamilyKey || "";
}

export function getRunnerFamilyGateForRow(row) {
  const family = getRunnerFamilyForRow(row);
  const micro = getRunnerMicroFamilyForRow(row, family);

  if (!family?.familyId || !micro?.microFamilyKey) return null;
  ...
  return {
    ...
    discordFilterField: MICRO_LIVE_FILTER_FIELD,
    discordFilterValue: micro.microFamilyKey,
...
export function normalizeRunnerAnalyzeRow(raw) {
  ...
  const normalizedCore = {
    ...
    pullbackConfirmed: merged.pullbackConfirmed === true,
    sweepConfirmed: merged.sweepConfirmed === true,
    retestConfirmed: merged.retestConfirmed === true,
    structureAligned: merged.structureAligned === true,
    rsiPullbackAllowed: merged.rsiPullbackAllowed === true,
    rsiContinuationAllowed: merged.rsiContinuationAllowed === true
  };
  ...
  const family = getRunnerFamilyForRow(normalizedCore);
  if (!family?.familyId) return null;

  const micro = getRunnerMicroFamilyForRow(normalizedCore, family);
  ...
  return {
    ...
    microFamilyId: micro.microFamilyId,
    runnerMicroFamilyId: micro.microFamilyId,
    analyzeMicroFamilyId: micro.microFamilyId,
    analysisMicroFamilyId: micro.microFamilyId,
    ...
    confluence: normalizedCore.confluence,
    sniperScore: normalizedCore.sniperScore,
    score: normalizedCore.score,
    moveScore: normalizedCore.moveScore,

    flow: normalizedCore.flow,
    scannerFlow: normalizedCore.scannerFlow,

    rsi: getFirstNumber(merged.rsi),
    rsiZone: normalizedCore.rsiZone,

    obBias: normalizedCore.obBias,
    spreadPct: normalizedCore.spreadPct,
    spreadBps: normalizedCore.spreadBps,
    depthMinUsd1p: normalizedCore.depthMinUsd1p,

    funding: normalizedCore.funding,
    fundingRate: normalizedCore.fundingRate,

    btcState: normalizedCore.btcState,
    regime: normalizedCore.regime,
    volatility: normalizedCore.volatility,

    tfScore: normalizedCore.tfScore,
    tfStrength: normalizedCore.tfStrength,
    tfAlignment: normalizedCore.tfAlignment,

    pullbackConfirmed: normalizedCore.pullbackConfirmed,
    sweepConfirmed: normalizedCore.sweepConfirmed,
    retestConfirmed: normalizedCore.retestConfirmed,
    structureAligned: normalizedCore.structureAligned,
    rsiPullbackAllowed: normalizedCore.rsiPullbackAllowed,
    rsiContinuationAllowed: normalizedCore.rsiContinuationAllowed,
...
export function buildRunnerFamilyAnalysis(rawRows, options = {}) {
  ...
  const bestLongByPnl =
    longFamilies.filter(row => row.observed > 0).sort(sortByPnl)[0] || null;

  const bestShortByPnl =
    shortFamilies.filter(row => row.observed > 0).sort(sortByPnl)[0] || null;

  const bestLongMicroByPnl =
    longMicroFamilies.filter(row => row.observed > 0).sort(sortByPnl)[0] || null;

  const bestShortMicroByPnl =
    shortMicroFamilies.filter(row => row.observed > 0).sort(sortByPnl)[0] || null;

  return {
    rows,

    config: {
      macroFamilyCountPerSide: FAMILY_COUNT_PER_SIDE,
      microVersion: MICRO_FAMILY_VERSION,
      liveDiscordFilterField: MICRO_LIVE_FILTER_FIELD,
      minClosed,
      minMicroClosed,
      breakevenREps,
      objective: "PnL-first: totalPnlPct -> totalR -> avgR -> profitFactor -> closed"
    },
    ...
    microBest: {
      ...
      allowedDiscordMicroFamilyKeysSuggestion: [
        bestLongMicroByPnl?.microFamilyKey,
        bestShortMicroByPnl?.microFamilyKey
      ].filter(Boolean),

      allowedDiscordMicroFamilyIdsSuggestion: [
        bestLongMicroByPnl?.microFamilyId,
        bestShortMicroByPnl?.microFamilyId
      ].filter(Boolean)
    },
    ...
    microWinnerCandidateSummary: {
      ...
      objective: "highest_total_pnl_pct_then_total_r",
      liveGateField: MICRO_LIVE_FILTER_FIELD,
...
export default {
  ...
  buildRunnerMicroBucketMap,
  buildRunnerMicroLabels,
  getRunnerMicroFamilyForRow,
  getRunnerMicroFamilyKeyForRow,
  getRunnerFamilyGateForRow,
