// lib/analyze/runnerFamilyEngine.js
// ================= RUNNER FAMILY ENGINE =================
// 50 LONG families + 50 SHORT families.
// Objective: RUNNER_PNL_FIRST_WITH_BALANCE_VIEW.
...
function isTradeRecord(event) {
  ...
  return Boolean(
    hasValue(event.entry) ||
      ...
      event.closed === true ||
      event.isClosed === true ||
      hasValue(event.exitPrice) ||
      hasValue(event.exit) ||
      hasValue(event.closedAt) ||
      hasValue(event.exitAt) ||
      hasValue(event.exitedAt) ||
...
function hasExitSignal(event) {
  ...
  if (hasValue(event.exitPrice)) return true;
  if (hasValue(event.exit)) return true;
  if (hasValue(event.closedAt) || hasValue(event.exitAt) || hasValue(event.exitedAt) || hasValue(event.exitTs)) return true;
...
function getBestBalanceFamilies(families, limit = 10, minClosed = DEFAULT_MIN_CLOSED) {
  return [...families]
    .filter(family => family.closed >= minClosed)
    .filter(family => family.avgR > 0)
    .filter(family => family.profitFactorR > 1)
    .filter(family => family.winrateNum >= 35)
    .sort((a, b) => {
...
function getBestRunnerPnlFamilies(families, limit = 10, minClosed = DEFAULT_MIN_CLOSED) {
  return [...families]
    .filter(family => family.closed >= minClosed)
    .filter(family => family.avgR > 0)
    .filter(family => family.avgPnlPct > 0)
    .filter(family => family.profitFactorR > 1)
    .filter(family => family.winrateNum >= 35)
    .sort((a, b) => {
...
export function buildRunnerAnalyzeReport(events = [], options = {}) {
  ...
  const bestBalance = getBestBalanceFamilies(finalizedAll, 10, minClosed);
  const bestRunnerPnl = getBestRunnerPnlFamilies(finalizedAll, 10, minClosed);
  ...
  return {
    ...
    selection: {
      primaryRunnerPnlFamily: bestRunnerPnl[0] || null,
      primaryBalancedFamily: bestBalance[0] || null,
      primaryStatusFamily: best[0] || null,
      avoidFamily: worst[0] || null,
      canSelectRealFamilies: bestRunnerPnl.length > 0 || bestBalance.length > 0 || best.length > 0,
    },
