...
export async function runMoonAll({ base, token, fetchFn, sleepMs = 2000, maxMs = 25000 }) {
  ...
  const obUrl = ensureAbsoluteUrl(
    base,
    `/api/moon/ob-sampler?token=${encodeURIComponent(token)}&_t=${t}`
  );

  // FIX 7: eerste twee scans parallel
  const [scanBull1Res, scanBear1Res] = await Promise.all([
    fetchFn(scanBullUrl),
    fetchFn(scanBearUrl),
  ]);
  ...
  // FIX 7: tweede scans parallel
  const [scanBull2Res, scanBear2Res] = await Promise.all([
    fetchFn(scanBullUrl),
    fetchFn(scanBearUrl),
  ]);
  ...
  return {
    ...
    steps: {
      scanBull1: {
        ok: scanBull1Res.ok,
        status: scanBull1Res.status,
        preview: scanBull1Text.slice(0, 200),
      },
      scanBear1: {
        ok: scanBear1Res.ok,
        status: scanBear1Res.status,
        preview: scanBear1Text.slice(0, 200),
      },
      obSampler: {
        ok: obRes.ok,
        status: obRes.status,
      ...
      scanBull2: {
        ok: scanBull2Res.ok,
        status: scanBull2Res.status,
        preview: scanBull2Text.slice(0, 200),
      },
      scanBear2: {
        ok: scanBear2Res.ok,
        status: scanBear2Res.status,
        preview: scanBear2Text.slice(0, 200),
      },
    },
  };
}
