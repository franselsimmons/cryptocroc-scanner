...
/**
 * PURE BULL MOON CORE
 * - buildup / almost opener zodat winners minder blijven hangen
 * - elite strenger op kwaliteit, persistence en OB
 */
...
export function decideMoonStage({
  ...
  const neutralOverrideOk =
    ...
    n(obx.score, 0) >= 0.05 &&
    n(volAcc.short, 1) >= 1.08 &&
    n(volAcc.medium, 1) >= 1.12 &&
    entryQuality >= 72 &&
    persistenceScore >= 60;
  ...
  if (mode === "bull") {
    if (
      ...
      n(coin.change1h, 0) >= Math.max(0.10, n(cfg.minCh1hAlmost, 0) - 0.2) &&
      n(coin.change24, 0) >= Math.max(0.5, n(cfg.minCh24Almost, 0) - 0.8) &&
      n(coin.vm, 0) >= Math.max(0.10, n(cfg.minVmAlmost, 0) - 0.02) &&
      velocity >= Math.max(0.03, n(cfg.strongVelocity, 0) - 0.05)
    ) {
      stage = "ALMOST";
    } else if (
  ...
  if (isMoonEliteStage(stage) && !breakout.ready && entryQuality < 74) {
    stage = "ALMOST";
    eliteType = null;
  }
