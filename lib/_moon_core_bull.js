 ...
 * PURE BULL MOON CORE
 * - strengere variant
 * - neutral BTC override ingebouwd
 * - geen fetch / geen kv
 */
...
export function decideMoonStage({
  ...
  const neutralOverrideOk =
    ...
    n(obx.score, 0) >= 0.055 &&
    n(volAcc.short, 1) >= 1.12 &&
    n(volAcc.medium, 1) >= 1.18 &&
    breakout.ready === true &&
    entryQuality >= 74 &&
    persistenceScore >= 60;
  ...
  if (isMoonEliteStage(stage) && !btcMomentumOk && up(regime) !== "EXPANSION" && !neutralOverrideOk) {
    return {
      stage: "ALMOST",
