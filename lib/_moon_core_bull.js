...
export function adjustMoonConfigForRegime(baseCfg, regime) {
  ...
  if (r === "DRY") {
    ...
    cfg.minCh24Almost = Math.max(0, n(cfg.minCh24Almost, 0) - 1.0);
    cfg.strongVelocity = Math.max(0, n(cfg.strongVelocity, 0) - 0.015);
  }
  if (r === "EXPANSION") {
    cfg.minVmElite = Math.max(0, n(cfg.minVmElite, 0) - 0.02);
    cfg.minCh24Ignition = Math.max(0, n(cfg.minCh24Ignition, 0) - 1.0);
  }
  if (r === "HEADWIND") {
...
export function isBullExhausted(coin) {
  ...
  if (ch24 >= 45 && vm < 0.2) return true;
  return false;
}
export function isBearBounceTrap() { return false; }
...
export function isLateBearEntry() { return false; }
...
export function computeMoonProbabilities({ mode, coin, moveScore, velocity, compression, persistenceScore = 0 }) {
  ...
  const moonProbability = mode === "bull"
    ? Math.max(0, Math.min(1,
        (moveScore * 0.34)/100 + (velScore * 0.18)/100 + (vmScore * 0.14)/100 +
        (compScore * 0.08)/100 + (persist * 0.18)/100 + (obScore > 0.05 ? 0.08 : 0)
      ))
    : 0;
...
export function computeMoonRisk({ mode, price, range24, confidence, depthOk, tier, regime = "TREND", persistenceScore = 50 }) {
  ...
  if (tier?.name === "small") { tpPct += 1.4; slPct += 0.5; }
  if (tier?.name === "large") { tpPct -= 1.2; slPct -= 0.4; }
  if (reg === "EXPANSION") tpPct += 1.8;
  if (reg === "DRY") tpPct -= 1.2;
  if (reg === "HEADWIND") { tpPct -= 1.6; slPct += 0.4; }
...
export function computePerfectCandidateScore({ qualityScore, liquidityScore, timingScore, marketScore }) {
  const total = n(qualityScore,0)*0.4 + n(timingScore,0)*0.3 + n(liquidityScore,0)*0.2 + n(marketScore,0)*0.1;
  return clamp(Math.round(total), 0, 100);
}

// ====================== NIEUWE EXPORTS VOOR LICHTERE SCAN ======================

// Drempels voor desk gate (hysteresis)
const DESK_THRESHOLDS_MOON = {
  watchConfirmScans: 1,
  openConfirmScans: 1,
  watchMinHoldMs: 20 * 60 * 1000,
  openMinHoldMs: 12 * 60 * 1000,
  watchEnterEQ: 68,
  watchEnterPS: 54,
  watchEnterPressure: 50,
  watchEnterObScore: 0.004,
  watchStayEQ: 60,
  watchStayPS: 48,
  openEnterEQ: 72,
  openEnterPS: 56,
...
export function decideDeskGateHysteresis({
  ...
  const wantWatchFinal = wantWatch === true && hasTradePlan === true &&
    (isEliteStageForDesk ||
      (n(entryQuality,0) >= T.watchEnterEQ && n(persistenceScore,0) >= T.watchEnterPS &&
       (brReady || brPressure >= T.watchEnterPressure) && n(obScore,0) >= T.watchEnterObScore));

  const canStayWatch = hasTradePlan === true &&
    n(entryQuality,0) >= T.watchStayEQ && n(persistenceScore,0) >= T.watchStayPS;

  const wantOpenFinal = wantOpen === true && isEliteStageForDesk && hasTradePlan === true &&
    n(entryQuality,0) >= T.openEnterEQ && n(persistenceScore,0) >= T.openEnterPS &&
    (brReady || brPressure >= T.openEnterPressure);

  const canStayOpen = hasTradePlan === true &&
    n(entryQuality,0) >= T.openStayEQ && n(persistenceScore,0) >= T.openStayPS;
...
export function computeDeskGate({ mode, stage, entryQuality, persistenceScore, breakout, obScore, tradePlan, now, prevGate, prevMeta, isEliteStageForDesk }) {
  const aPlus = tradePlan && entryQuality >= 72 && persistenceScore >= 56 && breakout?.ready && obScore >= 0.02;
  const nearAPlus = tradePlan && entryQuality >= 66 && persistenceScore >= 50 && (breakout?.ready || breakout?.pressure >= 50);
  ...
  const uiGate = engineGate; // voor moon gebruiken we dezelfde
  return { engineGate, uiGate, deskMeta: hyst.meta };
}

export function splitFunnels(coins) {
  const funnel = { elite_expansion: [], elite_ignition: [], almost: [], buildup: [], radar: [], hold: [] };
...
export function computeThesisDamage(coin, prevState, mode) {
  ...
  const reasons = {};
  const obScore = n(coin?.ob?.score, 0);
  if (mode === "bull" && obScore < -0.02) { damage += 2; reasons.obContra = true; }
  if (mode === "bear" && obScore > 0.02) { damage += 2; reasons.obContra = true; }
  const v1 = n(coin?.volAcc?.short, 1);
  const v2 = n(coin?.volAcc?.medium, 1);
  if (v1 < 1.01 && v2 < 1.04) { damage += 1; reasons.volDead = true; }
  if (!coin?.breakout?.ready) { damage += 1; reasons.breakoutLost = true; }
  const ps = n(coin?.persistenceScore, 0);
  const prevPs = n(prevState?.persistenceScore, 0);
  if (ps < prevPs - 15) { damage += 2; reasons.persistDrop = true; }
...
export function decideMoonStage({
  ...
  const persistenceScore = CORE.computePersistenceScore({ priceHist, volHist, stageHist: prev?.stageHist || [], mode });
  ...
  const entryQuality = CORE.computeEliteQuality({
    moveScore, velocity, vm: coin.vm, obScore: obx.score, compression, volAcc, persistenceScore, regime
  });
  ...
  if (mode === "bull") {
    if (n(coin.change1h,0) >= n(cfg.minCh1hExpansion,0) && n(coin.change24,0) >= n(cfg.minCh24Expansion,0) &&
        n(coin.vm,0) >= n(cfg.minVmElite,0) && n(obx.score,0) >= n(cfg.minObStrong,0) &&
        velocity >= n(cfg.explosiveVelocity,0) && entryQuality >= 76 && persistenceScore >= n(cfg.minPersistenceExpansion,70)) {
      stage = "ELITE_EXPANSION"; eliteType = "expansion";
    } else if (n(coin.change1h,0) >= n(cfg.minCh1hIgnition,0) && n(coin.change24,0) >= n(cfg.minCh24Ignition,0) &&
               n(coin.vm,0) >= n(cfg.minVmElite,0) && n(obx.score,0) >= n(cfg.minObStrong,0) &&
               velocity >= n(cfg.strongVelocity,0) && entryQuality >= 66 && persistenceScore >= n(cfg.minPersistenceIgnition,60)) {
      stage = "ELITE_IGNITION"; eliteType = "ignition";
    } else if (n(coin.change1h,0) >= Math.max(0.9, n(cfg.minCh1hAlmost,0)-0.3) &&
      ...
      stage = "ALMOST";
    } else if (n(coin.change1h,0) >= n(cfg.minCh1hBuildup,0) && n(coin.change24,0) >= n(cfg.minCh24Buildup,0) &&
               n(coin.vm,0) >= n(cfg.minVmBuildup,0) && velocity >= n(cfg.minVelocity,0)) {
      stage = "BUILDUP";
    }
  } else {
    // bear logic (wordt in bear core anders geïmplementeerd)
    stage = "RADAR";
  }

  if (isMoonEliteStage(stage) && !breakout.ready && entryQuality < 82) {
    stage = "ALMOST"; eliteType = null;
  }
  ...
  function hasEliteFollowThrough(prev, currentStage) {
    ...
    const eliteLike = tail.filter(s => { const x=up(s); return x==="ELITE_IGNITION"||x==="ELITE_EXPANSION"||x==="ELITE_CASCADE"; }).length;
    return eliteLike >= 1;
  }
...
export function buildMoonTradePlan({ CORE, price, mode, confidence, range24, depthOk, tier, regime, persistenceScore }) {
  const risk = CORE.computeMoonRisk({ mode, price, range24, confidence, depthOk, tier, regime, persistenceScore });
  if (!risk) return null;
  return {
