...
export function formatSignalMessage({ source, stage, kind, coin, pnl }) {
  ...
  // Bepaal niveau
  if (k.includes("elite") || k.includes("trade_")) {
  ...
  // Trade Closed is een kort bericht
  if (k === "trade_closed") {
    return [
      header,
      `🔴 **TRADE CLOSED — ${symbol}**`,
  ...
  // Voeg alleen beschrijving toe voor de Pro/Elite niveaus
  let descriptionText = "";
  if (niveau === "APEX PRIORITY" || niveau === "PRIME SETUP") {
  ...
  const rows = [
    ...
    descriptionText,
    "---",
    `📥 **Enter:** ${fmtNum(coin?.entry || coin?.price)}`,
    `🎯 **TP:** ${fmtNum(coin?.tp || "-")}`,
    `❌ **SL:** ${fmtNum(coin?.sl || "-")}`
  ];
