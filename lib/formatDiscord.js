...
// Optie A labels: MAIN => Cashflow, MOON => Runner
// Extra: bij ELITE en trade_opened/trade_closed altijd TP/SL/RR tonen (als beschikbaar)
...
function cleanReason(reason) {
  ...
  if (r === "btc_hard_against") return "BTC draaide hard tegen de positie.";
  if (r === "regime_panic") return "Marktregime werd te riskant.";
  if (r === "depth_failed") return "Orderboek-depth viel weg.";
  if (r === "spread_explosion") return "Spread liep te ver op.";
  if (r === "trade_candidate_lost") return "Coin verloor trade-candidate status.";
  if (r === "stage_lost") return "Elite-structuur viel weg.";
  if (r === "quality_collapse") return "Entry quality zakte te ver weg.";
  if (r === "persistence_collapse") return "Persistence verloor te veel kracht.";
  if (r === "timeout_no_followthrough") return "Geen follow-through na de entry.";
  if (r === "thesis_damage_confirmed") return "Setup verloor op meerdere punten overtuiging.";
  if (r === "grace_hold") return "Trade zit nog in grace hold.";
  if (r === "hold_valid") return "Positie blijft valide.";
  if (r === "weak_hold") return "Tijdelijke zwakte zonder bevestigde break.";
  if (r === "entry_confirmed") return "Scanner-setup bevestigd.";
  if (r === "entry_early_strength") return "Vroege entry op sterke scanner-setup.";
  if (r === "watch_timing") return "Timing nog net niet optimaal.";
...
// helper: altijd TP/SL tonen bij ELITE of trade events
...
export function formatSignalMessage({
  ...
  const planLine =
    showPlan ? formatTpSl(tradePlan) : (tp != null || sl != null || rr != null ? formatTpSl(tradePlan) : "");
  ...
  if (kind === "position_update") {
    ...
    return [
      ...
      `Status: **${mood}**`,
      `PnL: **${pnlText}** • Prijs: **${price}**`,
      entry != null ? `📍 Entry ${fmtNum(entry)}` : "",
      planLine,
      `📊 Quality ${conf} • Persistence ${persistence} • Breakout ${breakoutPressure}`,
      `🌊 Spread ${spread} • Depth ${depth} • OB ${obScore}`,
      `₿ BTC ${btcState || "-"} • 1u ${ch1h} • 24u ${ch24}`,
      reason ? `📝 Note: ${cleanReason(reason)}` : "",
    ]
      .filter(Boolean)
      .join("\n");
  }
