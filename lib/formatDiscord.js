...
export function getDiscordColor({ kind, pnl, reason }) {
  ...
  if (k === "trade_closed") {
    ...
    if (r === "btc_hard_against" || r === "regime_panic" || r === "spread_explosion" || r === "depth_failed") {
      return 15548997; // rood
    }
    if (Number.isFinite(p) && p >= 0) return 5763719;
    return 15105570; // oranje / amber
  }
...
export function formatSignalMessage({
  ...
  const breakoutPressure =
    coin?.breakout?.pressure != null ? fmtNum(coin.breakout.pressure, 1) : "-";
