...
export function formatSignalMessage({
  ...
  if (kind === "trade_closed") {
    const entry = tradePlan.entry ? fmtNum(tradePlan.entry) : "-";
    const exit = coin?.price ? fmtNum(coin.price) : "-";
    const pnlText = pnl != null ? fmtNum(pnl, 2) : "-";

    let message = "De trade is afgesloten.";

    const r = String(reason || "").toLowerCase();

    if (r === "take_profit") {
      message = "Doel bereikt, we sluiten de positie netjes af.";
    } else if (r === "stop_loss") {
      message = "De stop loss werd geraakt, dus we zijn uit de trade.";
    } else if (r === "thesis_break") {
      message = "De setup verloor kracht, daarom sluiten we de trade.";
    } else if (r === "timeout") {
      message = "De trade bleef te lang stil liggen, dus we nemen hem van tafel.";
    }

    return [
      `📊 **Trade afgerond – ${symbol}**`,
      ``,
      message,
      ``,
      `Entry: ${entry}`,
