...
/**
 * Strakkere entry-trigger voor funnel WATCH -> ENTRY.
 * Doel:
 * - alleen sterkere candidates naar echte entry
 * - breakout druk omhoog
 * - entry distance kleiner
 * - OB richting verplicht zodra minObScoreAbs gezet is
 */
export function entryTriggerOk({
  ...
  const minObAbs = Math.abs(n(minObScoreAbs, 0));
  if (minObAbs > 0) {
    if (s === "SHORT") {
      if (obs > -minObAbs) return false;
    } else {
      if (obs < minObAbs) return false;
    }
  }
 ...
 * Trade tunnel sterker:
 * - spread spike sneller hard exit
 * - contrair OB direct serieus nemen
 */
