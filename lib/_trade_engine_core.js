 ...
 * Iets losser gemaakt voor main sample-opbouw:
 * - default breakout pressure lager
 * - entry distance iets ruimer
 */
export function entryTriggerOk({
  ...
  minBreakoutPressure = 28,
  maxEntryDistancePct = 1.6,
}) {
  const s = up(side);
  const p = n(price, 0);
  const e = n(entry, 0);
 ...
 * Iets minder snel op spread spikes alleen.
 */
export function hardBreakDetected({
  ...
  if (spread > n(maxSpreadPctInTrade, 999) * 1.1) {
